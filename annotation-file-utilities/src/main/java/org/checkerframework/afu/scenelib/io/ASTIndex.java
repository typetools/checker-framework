...
public class ASTIndex extends WrapperMap<Tree, ASTRecord> {
  ...
  private ASTIndex(CompilationUnitTree root) {
    ...
    cut.accept(new AstIndexVisitor(), null);
  }
  ...
  // This is an instance class because it accesses `cut` and `formals`.
  ...
  private class AstIndexVisitor extends SimpleTreeVisitor<Void, ASTRecord> {
    ...
    private void saveAll(Iterable<? extends Tree> nodes, ASTRecord rec, Kind kind, String sel) {
      if (nodes != null) {
        int i = 0;
        for (Tree node : nodes) {
