...
public class MustCallVisitor extends BaseTypeVisitor<MustCallAnnotatedTypeFactory> {
  ...
  /**
   * Checks whether the loop either:
   * - initializes entries of a resource array. In that case the loop is marked in a special
   *   datastructure (where? TODO)
   * - calls a method on each element of an array. In this case, we need to be more careful and
   *   check whether the bounds actually guarantee that the loop reaches over all elements. Mark
   *   the loop as calling a method on a certain @OwningArray in the CFG (how to access the CFG? Some class probably
   *   offers a getCFG method or similar TODO)
   */
  @Override
  public Void visitForLoop(ForLoopTree tree, Void p) {
    ...
    if (blockT.getStatements().size() == 1) {
      if (blockT.getStatements().get(0) instanceof ExpressionStatementTree) {
        ExpressionTree exprT = ((ExpressionStatementTree) blockT.getStatements().get(0)).getExpression();
        if (exprT instanceof AssignmentTree) {
          // pattern match opening an obligation
          AssignmentTree assgn = (AssignmentTree) exprT;
          ...
          if (lhs instanceof ArrayAccessTree) {
            ...
            if (lhsIsOwningArray) {
              // check whether RHS has any mustcall first TODO
              MustCallAnnotatedTypeFactory.createArrayObligationForAssignment(assgn);
            }
          }
        } else if (exprT instanceof MemberSelectTree) {
          ...
          boolean lhsIsOwningArray =
            atypeFactory.getDeclAnnotation(lhsElt, OwningArray.class) != null;
          if (lhsIsOwningArray) {
            MustCallAnnotatedTypeFactory.fulfillArrayObligationForMethodAccess((MemberSelectTree) exprT);
          }
        }
      }
    }
