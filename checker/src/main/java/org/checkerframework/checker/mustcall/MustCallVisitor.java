...
public class MustCallVisitor extends BaseTypeVisitor<MustCallAnnotatedTypeFactory> {
  ...
  //   *
  //   * <p>This logic lives in the Must Call visitor because matching must complete before
  // collection
  //   * ownership transfer runs.
  //   */
  //  @Override
  //  public Void visitForLoop(ForLoopTree tree, Void p) {
  //    boolean singleLoopVariable = tree.getUpdate().size() == 1 && tree.getInitializer().size() ==
  // 1;
  ...
  //   *
  //   * <p>The visitor records only the loop tree here. RLCC resolves the desugared iterator CFG
  // shape
  ...
  //   * #collectionVarNameForBailout} names the collection variable whose writes should invalidate
  // the
  //   * match when present.
  //   */
  ...
  //   *
  //   * <p>This method performs AST matching plus the small amount of CFG lookup needed to identify
  // the
  ...
  //   * non-empty collection loops such as {@code while (!q.isEmpty())}, {@code while (q.size() >
  // 0)},
  ...
  //   * lambda expression.
  //   *
  //   * <p>The per-method loop-state refactor records only loops that are part of the enclosing
  // method
  ...
  //
  //  /**
  //   * Returns the conditional successor reached from the given block, if one is immediately
  // visible.
  //   *
  ...
  //   *
  //   * <p>This is needed because loop conditions such as {@code iterator.hasNext()} may be
  // represented
  ...
  //
  //  /**
  //   * Recovers the collection expression from an iterator receiver in a header such as {@code
  // while
  //   * (it.hasNext())}.
  //   *
  ...
  //   *
  //   * <p>The extraction call is the expression that removes or advances to the next element, such
  // as
  ...
  //   * collection variable itself, because such writes invalidate the header/body correspondence
  // used
  ...
  // getLoopBodyStatements(tree.getStatement());
  ...
  //      // analyze it later.
  //      // MustCallConsistencyAnalyzer.analyzeResolvedPotentiallyFulfillingCollectionLoop will
  // then
  ...
  //
  //  /**
  //   * Conservatively decides whether a loop iterates over all elements of some collection, using
  // the
  //   * following rules:
  //   *
  //   * <ul>
  ...
  //   * @param identifierInHeader collection name if loop condition is {@code i <
  // collection.size()} or
  //   *     {@code i < arr.length} and {@code n} if loop condition is {@code i < n}
  //   * @param iterator the name of the loop iterator variable
  //   * @return {@code null} if the loop body writes to the iterator or collection variable;
  // otherwise
  ...
  //
  //  /**
  //   * Returns the simple name of the identifier referenced by the given expression, or {@code
  // null}
  ...
  //   * @return the name of the identifier declared or referenced by the statement, or {@code null}
  // if
  //   *     none
  //   */
