 ...
 * section 4.2</a>. A {@linkplain BinaryName binary name} is conceptually the name for the class or
 * interface in a compiled binary, but the actual representation of that name in its class file is
 * slightly different.
 *
...
@ImplicitFor(
    stringPatterns =
            "(^(?![BCDFIJSZ]$)[A-Za-z_][A-Za-z_0-9]*(/[A-Za-z_][A-Za-z_0-9]*)*(\\$[A-Za-z_0-9]+)*$)|^\\[+([BCDFIJSZ]|L[A-Za-z_][A-Za-z_0-9]*(/[A-Za-z_][A-Za-z_0-9]*)*(\\$[A-Za-z_0-9]+)*;)$"
)
