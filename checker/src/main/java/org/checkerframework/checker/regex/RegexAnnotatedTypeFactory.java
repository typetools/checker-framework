...
public class RegexAnnotatedTypeFactory extends BaseAnnotatedTypeFactory {
  ...
  protected final ExecutableElement enhancedRegexValueElement =
      TreeUtils.getMethod(
          "org.checkerframework.checker.regex.qual.EnhancedRegex", "value", 0, processingEnv);

  /** The {@code Matcher.group} method. */
  protected final ExecutableElement group =
      TreeUtils.getMethod(
          java.util.regex.Matcher.class.getCanonicalName(), "group", processingEnv, "int");
  ...
  public static List<Integer> getNonNullGroups(@Regex String regexp) {
    ...
    for (int i = 0; i < regexp.length(); i++) {
      if (!escaped && !squareBracketOpen && regexp.charAt(i) == '(') {
        ...
        if (i != regexp.length() - 1 && "*?|".contains(Character.toString(regexp.charAt(i + 1)))) {
  ...
  private class RegexTreeAnnotator extends TreeAnnotator {
    ...
    public Void visitCompoundAssignment(CompoundAssignmentTree node, AnnotatedTypeMirror type) {
      if (TreeUtils.isStringCompoundConcatenation(node)) {
        ...
        if (lhsNonNullGroups != null && rhsNonNullGroups != null) {
          type.removeAnnotationInHierarchy(ENHANCEDREGEX);
          List<Integer> lNonNullGroups = getNonNullGroups(lhs.getAnnotation(EnhancedRegex.class));
          List<Integer> rNonNullGroups = getNonNullGroups(rhs.getAnnotation(EnhancedRegex.class));
          List<Integer> concatNonNullGroups = new ArrayList<>();
    ...
    private List<Integer> getMinimumNonNullGroups(final AnnotatedTypeMirror type) {
      ...
      if (anno == null) {
        switch (type.getKind()) {
          case TYPEVAR:
            return getMinimumNonNullGroups(((AnnotatedTypeVariable) type).getUpperBound());

          case WILDCARD:
            return getMinimumNonNullGroups(((AnnotatedWildcardType) type).getExtendsBound());
