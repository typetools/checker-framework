...
public class UpperBoundUtils {
    ...
    public static boolean hasValue(AnnotatedTypeMirror type, String name) {
        String[] rgst = getValue(type.getAnnotation(LtLength.class));
        for (String st : rgst) {
            if (st.equals(name)) {
                return true;
            }
    ...
    public static boolean hasValueMethod(AnnotationMirror anno) {
        boolean fLTL = AnnotationUtils.areSameByClass(anno, LtLength.class);
        boolean fLTEL = AnnotationUtils.areSameByClass(anno, LteLength.class);
        return fLTL || fLTEL;
    }
}
