...
public class UpperBoundUtils {
    ...
    static ExecutableElement getValueMethod(AnnotationMirror anno) {
        if (AnnotationUtils.areSameIgnoringValues(anno, UpperBoundAnnotatedTypeFactory.LTL)) {
            return TreeUtils.getMethod("org.checkerframework.checker.upperbound.qual.LessThanLength", "value", 0, UpperBoundAnnotatedTypeFactory.env);
        }
        if (AnnotationUtils.areSameIgnoringValues(anno, UpperBoundAnnotatedTypeFactory.EL)) {
            return TreeUtils.getMethod("org.checkerframework.checker.upperbound.qual.EqualToLength", "value", 0, UpperBoundAnnotatedTypeFactory.env);
        }
        if (AnnotationUtils.areSameIgnoringValues(anno, UpperBoundAnnotatedTypeFactory.LTEL)) {
            return TreeUtils.getMethod("org.checkerframework.checker.upperbound.qual.LessThanOrEqualToLength", "value", 0, UpperBoundAnnotatedTypeFactory.env);
        }
        return null;
    }
    ...
    static String[] getIndexValue(AnnotationMirror anno, ExecutableElement valueElement) {
        return (String[]) AnnotationUtils.getElementValuesWithDefaults(anno).get(valueElement).getValue();
    }
