...
for PACKAGE in "${PACKAGES[@]}"; do
  ...
  (cd "${PACKAGEDIR}" && (./gradlew ${IS_CI:+"--no-daemon"} --console=plain -PcfLocal compileJava || (sleep 60 && ./gradlew ${IS_CI:+"--no-daemon"} --console=plain -PcfLocal compileJava))) || failing_packages="${failing_packages} ${PACKAGE}"
done
