...
GITHUB_ORGANIZATION=eisop

git -C /tmp/plume-scripts pull > /dev/null 2>&1 \
    || git -C /tmp clone --depth 1 -q https://github.com/plume-lib/plume-scripts.git
eval `/tmp/plume-scripts/ci-info ${GITHUB_ORGANIZATION}`
...
if [ -d ../annotation-tools ] ; then
    ...
    REPO=`/tmp/plume-scripts/git-find-fork ${CI_ORGANIZATION} ${GITHUB_ORGANIZATION} annotation-tools`
