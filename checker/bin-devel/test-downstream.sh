...
REPO=`/tmp/plume-scripts/git-find-fork ${CI_ORGANIZATION} ${GITHUB_ORGANIZATION} checker-framework.demos`
...
# Guava
REPO=`/tmp/plume-scripts/git-find-fork ${CI_ORGANIZATION} ${GITHUB_ORGANIZATION} guava`
...
if [ $BRANCH = "master" ] ; then
  ...
  REPO=https://github.com/${GITHUB_ORGANIZATION}/guava.git
  BRANCH=`/tmp/plume-scripts/git-find-branch ${REPO} ${CI_BRANCH} cf-master`
  if [ $BRANCH = "master" ] ; then
