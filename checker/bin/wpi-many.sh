...
if [ "${has_java8}" = "no" ] && [ "${has_java11}" = "no" ] && [ "${has_java17}" = "no" ] && [ "${has_java21}" = "no" ] && [ "${has_java24}" = "no" ] && [ "${has_java25}" = "no" ] && [ "${has_java26}" = "no" ]; then
  if [ "${has_java_home}" = "yes" ]; then
    ...
    echo "No Java 8, 11, 17, 21, 24, 25, or 26 JDK found. At least one of JAVA_HOME, JAVA8_HOME, JAVA11_HOME, JAVA17_HOME, JAVA21_HOME, JAVA24_HOME, JAVA25_HOME, or JAVA26_HOME must be set."
  fi
