...
of the generic type parameter (known as the ``bound'') restricts
how the class may be instantiated.
Both the Java type and the type parameter restrict 
For example, given the definition
\verb|class G<T extends Number> {...}|,
the upper bound is \<Number>
and a client can instantiate it as \code{G<Integer>} but not \code{G<Date>}.
Similarly, type qualifiers on the bound can restrict
how the class may be instantiated.  For example, a generic list class might
be declared 
\verb|class MyList<T extends @NonNull Object> {...}|
to indicate that it can hold only non-null values:
...
have annotaions for both the Regex and the Tainting checker, there must be a
different copy of each annotation so that the Checker Framework can determine
the checker that an annotaiton belongs to.
...
% LocalWords:  wildcards holderExtends holderSuper getTaintedString s2
% LocalWords:  getUntaintedString Typesystem Param ClassTaintingParam h1
% LocalWords:  arg param MethodTaintingParam Util meth value1 h2
