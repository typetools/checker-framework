...
argument \<-AuseConservativeDefaultsForUnannotatedSourceCode>.  This causes the
...
\<-AuseConservativeDefaultsForUnannotatedSourceCode> command-line argument is
...
using the \<-AuseConservativeDefaultsForUnannotatedSourceCode> command-line
...
%     typing those 17 characters later.
  ...
  build process, such as a Maven or Ant buildfile, so that every time it runs the
  compiler, it passes the \<-AuseConservativeDefaultsForUnannotatedSourceCode>
  command-line option and runs every pluggable type-checker for which any
  annotations exist.  You are not adding a new buld target, but modifying
  any existing targets.
  The reason to run every type-checker is so that all
  of the annotations you wrote are correct, and all other type uses are
  defaulted properly.
...
\item
  ...
  Because of the changes that you made in step 1, this will run the Checker
  Framework.  If there are any compiler warnings, fix them and re-compile.
