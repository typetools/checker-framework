...
\subsection{Using annotated libraries from the Checker Framework distribution\label{annotated-libraries-using}}
...
\subsection{Annotating the JDK when developing from the Checker Framework source\label{annotating-jdk}}
\begin{enumerate}
\item
...
\item Unzip the contatined \<src.zip> (resulting in folders: com, java, javax, launcher, org)
\end{enumerate}

\item
Set Up Checker Framework (replace \textit{mychecker} with the name of your checker)
\begin{enumerate}
\item Add a folder \<\$JSR308/checker-framework/checker/jdk/\textit{mychecker}> 

\item In  \<\$JSR308/checker-framework/checker/jdk/\textit{mychecker}> add a symbolic link to   \newline
\<\$JSR308/checker-framework/checker/jdk/MakeFile> (like the ones in lock and nullness)


\item Add \textit{mychecker} to \<\$JSR308/checker-framework/checker/jdk/MakeFile> in the following 2 places:
\begin{verbatim}
Line: ''CHECKER_DIRS = nullness lock'' etc
Becomes: ''CHECKER_DIRS = nullness lock mychecker '' etc 

Line: ANNOTATED_CLASSES = \
	$(patsubst nullness/build/%.class,annotated/%.class, \ 
      $(patsubst lock/build/%.class,annotated/%.class, \ 
          $(ANNOTATED_CLASSES_BY_DIR)))
Becomes:ANNOTATED_CLASSES = \
	$(patsubst nullness/build/%.class,annotated/%.class, \
     $(patsubst lock/build/%.class,annotated/%.class, \
     $(patsubst mychecker/build/%.class,annotated/%.class, \
        $(ANNOTATED_CLASSES_BY_DIR))))
\end{verbatim}

Don't forget to add another closing paren!

\end{enumerate}

\item
Add your Annotated Files \newline
	As you add annotations to the jdk copy annotated files into the folder \<\$JSR308/checker-framework/checker/jdk/\textit{mychecker}/src>
	In an identical directory structure as the jdk, look in nullness for an example \newline
	
	\textbf{Always fully annotated every class you add!}

\item
Build the checker \newline
	Run ant in the \<\$JSR308/checker-framework/checker> to apply your new annotated jdk.
\end{enumerate}
