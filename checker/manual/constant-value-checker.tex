...
\item \code{``Test.java:12: warning: [method.evaluation.exception]
  ...
  run-time.
\item \code{''Basics.java:154: error: [too.many.values.given] An
  annotation was written containing more 10 values, the maximum length
  allowed. Reduce the number of values given or change the annotation
  to @UnknownVal.''} means an annotation was written with more than
...
\item \code{''Basics.java:4: warning: [too.many.values.accumulated] An
  annotations value array became longer than 10, the maximum length
  allowed. Now treating the type as @UnknownVal.''} means a type had
