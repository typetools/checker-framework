...
\begin{itemize}
  \item
    Option 1:
    Add directory
    \code{.../checker-framework-1.8.1/checker/bin} to your path, \emph{before} any other
...
\begin{Verbatim}
  export CHECKERFRAMEWORK=${HOME}/checker-framework-1.8.1
...
\begin{Verbatim}
  ...
  # Windows
  set CHECKERFRAMEWORK = C:\Program Files\checker-framework-1.8.1\
...
There are two ways to do this. The first is to point Maven to a
script that executes the Checker Framework's copy of javac, and
...
\subsection{Pointing Maven to the Checker Framework script\label{maven-javac-script}}
...
\item Point the Maven compiler plugin to the javac\_maven script. Find
...
On Windows, javac\_maven.bat is used instead of the javac\_maven script, but it is not necessary to
include the .bat extension in the absolute path above.

This script assumes that the Checker Framework is the only annotation processor being run.
If this is not the case, please modify the javac\_maven script accordingly.

\item Create a text file named `argfile' in the same directory as the pom.xml and include in it the
command-line parameters to pass to the Java compiler.
...
Due to limitations of the pom.xml syntax, some command-line options are difficult or impossible
to pass via the pom.xml, hence the need for this file.

If you would like to call this file something other than 'argfile', please modify the javac\_maven
script accordingly.
