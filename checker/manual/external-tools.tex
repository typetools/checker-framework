...
\item Use Maven properties to hold the locations of the
  annotated JDK and (if using Java 7 and/or annotations in comments), the type annotations compiler.  Both were declared as Maven dependencies above.
...
<properties>
    ...
    <!-- Use the type annotations compiler if using Java 7 or annotations
    in comments. -->
    ...
    <plugin>
        ...
        <configuration>
            ...
            <compilerArgs>
                ...
                <!-- Uncomment the following line if using Java 7 or annotations in comments. -->
...
\item
  ...
  % Without the ENTER key, the setting won't be saved.
   ...
   %% This seems obvious, so I removed it to keep the text shorter
   % To remove a checker, simply select it from this list and click
   % the red "-" button.
