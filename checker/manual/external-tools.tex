...
\begin{itemize}
  \item
    Option 1:
    Add directory
    \code{.../checker-framework-1.9.7/checker/bin} to your path, \emph{before} any other
...
\begin{Verbatim}
  export CHECKERFRAMEWORK=${HOME}/checker-framework-1.9.7
...
\begin{Verbatim}
  ...
  # Windows
  set CHECKERFRAMEWORK = C:\Program Files\checker-framework-1.9.7\
...
the Maven artifacts are retrieved correctly.
...
\item Use a Maven property to hold the location of the
  annotated JDK (which was declared as a Maven dependency above).
To set the value of this property automaticaly, use the Maven Dependency plugin.
...
<properties>
    ...
    <jsr308Javac>$\{org.checkerframework:compiler:jar\}</jsr308Javac>
</properties>
\end{alltt}
    ...
    <plugin>
        ...
        <configuration>
            ...
            <compilerArgs>
                ...
                <arg>-J-Xbootclasspath/p:$\{jsr308Javac\}</arg>
            </compilerArgs>
        </configuration>
    </plugin>
\end{alltt}
...
so your Maven build should be reproducible in any server.
...
% Framework was deprecated in favor of a script. That in turn
% was deprecated in favor of the above approach in November 2015.
