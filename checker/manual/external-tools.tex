...
See the demonstration \code{examples/GradleExamples/} for examples of the use of
these instructions in a Gradle project.  These examples can be used to verify that
Gradle is correctly downloading and executing the Checker Framework on your machine.
...
\subsection{Gradle with JDK 8\label{gradle-jdk-8}}
These instructions explain how to configure Gradle to use the Checker Framework using JDK 8.
See Section~\ref{gradle-jdk-7} for JDK 7 instructions.
...
\item Add dependency configurations for the annotated JDK and \<checker.jar>:
...
\subsection{Gradle with JDK 7\label{gradle-jdk-7}}
These instructions explain how to configure Gradle to use the Checker Framework using JDK 7.
See Section~\ref{gradle-jdk-8} for JDK 8 instructions.
...
Alternatively, create a new task to run the desired checkers:

\begin{Verbatim}
task checkNullness(type: JavaCompile, group: 'build', description: 'Check for null pointer dereferences') {
