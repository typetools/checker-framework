...
If you are developing Checkers and wish to use the latest builds of your source files compiled by
Eclipse, you can use the development version of javac instead. This version will use compiled
...
If you use the Checker Framework's \code{javac} compiler, this step is optional.

However, if you use the Type Annotations \code{javac} compiler you will have to supply both of these
jars in the classpath using the \code{-cp} option whenever you run \code{javac} and use a checker
plugin. Adding the two jars to the classpath makes it easier to use.
