...
Framework.  The following command installs them for Ubuntu 16.04:
...
\subsection{Code style\lable{code-style}}
...
style by running <ant check-format/> in the top level directory.  If this fails
because of improper formatting, run <ant reformat/>.  If this fail with
"Argument list too long" error, then create a local.properties file in the top
level directory and set a propertie named "maxparallel" to 4000.  Rerun
check-format or reformat.  If the error is still issued, reduce the number and
try again.  Setting maxparallel will slow down the formatting Ant targets.
Another way to fix this error is reduce the characters in the full path to the
