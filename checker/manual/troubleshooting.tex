...
\begin{sloppypar}
  ...
  variable/option, or by ensuring that the \code{checker.jar} file is on
  your classpath.
\end{sloppypar}
...
but some library on your classpath was compiled against JDK 7, and the
compiled library refers to a class that only appears in JDK 7.  (If only
...
declaration locations, see Section~\ref{java7-classfiles}.
...
\noindent
indicates that you are compiling using a Java 7 JDK, but your code
...
Framework, but still compile and run your code in a Java 7 JVM, by
following the instructions in Section~\ref{java7-classfiles}.

If you ignore the error and run your code in a Java 7 JVM, then you will get a run-time error:
...
that are created, such as \<javac -target 7 ...>.
...
\item[\ahref{\url{https://ece.uwaterloo.ca/~wdietl/ownership/}}{Generic Universe Types}]
...
Another implementation of Universe Types and \ahref{\url{http://www.cs.rpi.edu/~huangw5/cf-inference/}}{ownership types} is  described in
...
\ahref{\url{http://findbugs.sourceforge.net/}}{FindBugs},
\ahref{\url{http://jlint.sourceforge.net/}}{Jlint}, or
\ahref{\url{http://pmd.sourceforge.net/}}{PMD}.
...
Also see the
\ahref{\url{http://types.cs.washington.edu/jsr308/}}{JSR 308}~\cite{JSR308-2008-09-12}
