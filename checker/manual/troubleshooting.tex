...
Once \<checker.jar> is built, you may wish to put the Checker Framework \<javac> even earlier on
your path. The Checker Framework's \code{javac} ensures that \<checker.jar> 
is on your classpath and bootclasspath, but is otherwise
...
If you are developing a checker and wish to use the latest builds of your source files compiled by
an IDE (such as Eclipse), you can use the development version of javac instead. This version will use compiled
classes from \<dataflow/build>, \<javacutil/build>, \<framework/build>, and \<checker/build>
in the \<checker-framework> directory instead of checker.jar, and by default will print stack 
traces for all errors:
\begin{Verbatim}
export PATH=$JSR308/checker-framework/checker/bin-devel:$JSR308/jsr308-langtools/dist/bin:${PATH}
\end{Verbatim}

\item Configuring classpath

If you want use the Type Annotations \code{javac} compiler to run a checker, you will have to supply
\code{tools.jar} and \code{checker.jar} using the \code{-cp} option or put the two jars on the classpath:
...
If you use the Checker Framework's \code{javac} compiler to run a checker, you do not have to add
\code{checker.jar} to the classpath as the Checker Framework's \code{javac} compiler will put
\code{checker.jar} on the classpath.
