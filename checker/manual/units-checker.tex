...
\code{-Aunits} must be accessible to the compiler during compilation in the
classpath. In other words, they must already be compiled (and, typically, be on
the javac bootclasspath) before you run the Units Checker with \code{javac}. It
...
that contain units qualifiers using the \code{-AunitsDirs} option, for example:
