...
If you want to create a type alias or typedef, you have two options.
Which one you choose depends on which operations you wish your new type to support.

\begin{itemize}
\item
  Use the Fake Enum Checker (\chapterpageref{fenum-checker}) if you wish to
  prevent all operations on the elements of the new type.  Suppose you
  create a new fenum type \<CompassDirection> with \<int> values \<NORTH>,
  \<SOUTH>, \<EAST>, and \<WEST>.  The operation \<NORTH + SOUTH> should be
  illegal.

\item
  Use the Subtyping Checker if you wish to permit operations on elements of the new type.  By default, the Subtyping Checker does not forbid any operations.
\end{itemize}
...
To make the code type-chekc, it is necessary to write an explicit
...
%%  LocalWords:  ElementType myqualpath sendOverInternet myModule
%%  LocalWords:  SubtypeOf LiteralKind DefaultFor TypeUseLocation
%%  LocalWords:  DefaultQualifierInHierarchy
