...
Two expressions might be aliased when they have the same non-primitive value.
Two expressions are aliased if they are references to the identical Java object
...
\code{-processor org.checkerframework.checker.aliasing.AliasingChecker}
...
There are two possible types for an expression in the program:

\begin{description}

\item[\refqualclass{checker/aliasing/qual}{MaybeAliased}]
...
\item[\refqualclass{checker/aliasing/qual}{Unique}]
...
constructor's body does not creates a new alias to the constructed object.

\end{description}

There are also two annotations, which are currently trusted instead of verified,
that can be used on parameters of a constructor or method declaration (including
the receiver parameter, \<this>):

\begin{description}

\item[\refqualclass{checker/aliasing/qual}{NonLeaked}]
is used to identify method parameters that are not leaked nor
...
\item[\refqualclass{checker/aliasing/qual}{LeakedToResult}]
...
assigns the argument to the formal parameter. Expressions returned by methods
are effectively assigned to the method call itself and are also
pseudo-assignments. Like assignments, the left-hand side and right-hand side of
...
reference to a method does not necessarily create an alias, because the
argument and the parameter might not be in scope at the same program point.
...
creating aliases for the argument unless the corresponding formal
parameters are marked as
@\refqualclass{checker/aliasing/qual}{NonLeaked} or
@\refqualclass{checker/aliasing/qual}{LeakedToResult}
...
\item
The LHS is a \code{@NonLeaked} method parameter.
(That can only happen when the RHS is an argument in a method call.)
\item
The LHS is a \code{@LeakedToResult} method parameter, and the method's return
value is discarded --- that is, the method call is written syntactically as a
statement rather than as a part of a larger expression or statement.
\end{itemize}
...
void bar() {
    String s = new String("MyUniqueString");  // s is refined to @Unique.

    StringBuffer sb = new StringBuffer();     // sb is refined to @Unique.

    sb.append(s);
    // Both s and sb stay @Unique, as no aliases are created.

    StringBuffer sb2 = sb.append(s);
    // sb is leaked and becomes @MaybeAliased.

    // Both sb and sb2 have aliases and because of that have type @MaybeAliased,
    // but s remains @Unique.
}
