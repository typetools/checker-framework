...
\section{Assumptions\label{lock-assumptions}}

The Lock Checker assumes that if an expression \emph{expr} is used to acquire a lock, then
...
\item[\refqualclass{checker/lock/qual}{GuardedBy}{\small(\emph{exprSet})}]
  ...
  write an explicit locking type qualifier (except CLIMB-to-top locations, see Section~\ref{climb-to-top}).
