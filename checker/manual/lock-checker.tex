...
\subsection{Declaration annotations for objects protected by locks}
...
\item[\refqualclass{checker/lock/qual}{EnsuresLockHeldIf}\small{(String[] expressions, boolean result)}]
  ...
  a locked state (i.e. they are annotated with \code{@LockHeld}, see
  section~\ref{lock-checker-internals}) after the
  method returns; this is useful for annotating a method that takes a lock.
...
\subsection{A note on Lock Checker internals\label{lock-checker-internals}}
