...
command-line option to javac.  Note that the \<-AconcurrentSemantics>
command-line option is automatically enabled (for the Lock Checker
only) when the Lock Checker is run (see Section~\ref{faq-concurrency}).
...
types (except CLIMB-to-top locations, see Section~\ref{climb-to-top}).
}
...
\item[\refqualclass{checker/lock/qual}{GuardSatisfied}{\small(\emph{index})}]
  ...
  It may not be written on fields.
...
\item[\code{@GuardSatisfied}]
  ...
  \item
    If the left-hand side is a formal parameter has type
    \<@GuardSatisfied(\emph{index})>, the right-hand-side must have
    identical \<@GuardSatisfied(\emph{index})> type.
  \end{itemize}

  Suppose that a method signature has multiple formal parameters with the
  same \<@GuardSatisfied(index)> type.
  As a consequence of the flat type hierarchy and the fact that
  \<@GuardedByUnknown> is not assignable to \<@GuardSatisfied>,
  at any call, all the corresponding actual arguments have the same
  \<@GuardedBy> type or \<@GuardedByBottom>.
...
It might seem that \<@GuardSatisfied> is would be a better default for
