...
\paragraph{Operations on primitive types}

  Primitive values are not guarded.  Instead, the variables that store them are.
\textcolor{red}{JT: Do I need to justify why primitives are guarded by name?
I could explain that this is the way primitives are most commonly guarded in
practice and could even explain why that is the case, but it seems unnecessary.}
...
\textcolor{red}{JT: The user may still want to guard primitive variables
against data races, such as a bank account balance.  I think we should
just mention that this guarding is by-name.}
