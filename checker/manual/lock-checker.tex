...
\item[\refqualclass{checker/lock/qual}{UnknownGuard}]
  If a variable \<x> has type \code{@UnknownGuard}, then
  ...
  \code{String(String s)}, is annotated with \code{@UnknownGuard} since
  ...
  arbitrary code.
  Note that \code{@UnknownGuard}
  has a distinct meaning from \code{@GuardedBy(\{\})}.  \code{@GuardedBy(\{\})}
  ...
  for details on when to use \code{@UnknownGuard}.
...
\item[\refqualclass{checker/interning/qual}{PolyGuardedBy}]
  ...
  See Section~\ref{lock-checker-library-methods}
  for an example of a use of \code{@PolyGuardedBy}.

\end{description}

\begin{figure}
\includeimage{lock-guardedby}{3.5cm}
...
\paragraph{Using the \refqualclass{checker/lock/qual}{UnknownGuard} and \refqualclass{common/aliasing/qual}{NonLeaked} annotations\label{lock-checker-library-methods-unknownguard-and-nonleaked}}
...
The \refqualclass{common/aliasing/qual}{NonLeaked} annotation is defined
...
With the \code{@NonLeaked} and \code{@UnknownGuard} annotations in place, the
...
\paragraph{Using the \refqualclass{checker/lock/qual}{PolyGuardedBy} and \refqualclass{common/aliasing/qual}{LeakedToResult} annotations\label{lock-checker-library-methods-polyguardedby-and-leakedtoresult}}
...
\code{@NonLeaked} because the receiver is leaked to the
return value.  Instead, the \code{@LeakedToResult} annotation is needed.
\refqualclass{common/aliasing/qual}{LeakedToResult} is defined
in Chapter \ref{aliasing-checker}.  Also, \code{@PolyGuardedBy} is used
...
With the \code{@LeakedToResult} and \code{@PolyGuardedBy} annotations in place, the
...
The \code{@LockingFree}, \code{@NonLeaked},
\code{@LeakedToResult}, \code{@UnknownGuard} and \code{@PolyGuardedBy} annotations allow the programmer
