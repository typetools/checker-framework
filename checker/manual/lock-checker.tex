...
\item[\refqualclass{checker/lock/qual}{GuardSatisfied}]
  If a method parameter or receiver \<x> has type \code{@GuardSatisfied}, then
...
\item[\refqualclass{checker/interning/qual}{PolyGuardedBy}]
  ...
  \refqualclass{checker/interning/qual}{PolyGuardedBy}, see
  Section~\ref{qualifier-polymorphism}.  Unlike with \code{@GuardSatisfied},
  a method parameter or receiver annotated with \code{@PolyGuardedBy}
  can never be dereferenced, and all instances of \code{@PolyGuardedBy}
  used in a method declaration must have exactly matching
  \code{@GuardedBy} annotations at the method call site.
...
\item[\refqualclass{checker/lock/qual}{EnsuresLockHeldIf}\small{(String[] locks, boolean result)}]
  indicate a method postcondition.  With \code{@EnsuresLockHeld}, the given lock
...
\item[\refqualclass{dataflow/qual}{LockingFree}]
  ...
  such a method when type-checking with the Lock Checker, use the \code{@ReleasesNoLocks}
...
\item[\refqualclass{dataflow/qual}{MayReleaseLocks}]
  ...
  or methods that call other methods annotated with \code{@MayReleaseLocks}.
...
\item[\code{@GuardSatisfied}]
  ...
  \code{@GuardSatisfied}, the method may only be called if its declaration is
  annotated with \code{@LockingFree}, \code{@ReleasesNoLocks}, \code{@SideEffectFree}
  or \code{@Pure} and
  all the locks guarding each the receiver and/or each actual parameter whose
  corresponding formal parameter is annotated with \code{@GuardSatisfied}
  are held at the call site.  An exception is if the actual parameter is itself
  a formal parameter of a method, in which case it may be annotated with \code{@GuardSatisfied}
...
\item[\code{@PolyGuardedBy}]
  ...
  \code{@PolyGuardedBy} may never be dereferenced.
...
\item[Overriding methods annotated with \code{@Holding}]
  ...
  \code{@Holding} require a subset of the locks that a parent
  class method requires in its \code{@Holding} annotation (if any).

\item[Releasing explicit locks]
  By default the Lock Checker treats methods whose bodies it is type checking and
  that are not explicitly annotated with \code{@MayReleaseLocks}, \code{@LockingFree},
  \code{@ReleasesNoLocks}, \code{@SideEffectFree} or \code{@Pure}
  as if they were annotated with \code{@ReleasesNoLocks}.  If an explicit lock is
  released in the body of such an unannotated method, the Lock Checker issues a warning that the
  method must be annotated with \code{@MayReleaseLocks}.

\item[Unannotated library methods]
  When calling unannotated library methods (see \chapterpageref{annotating-libraries}),
  the Lock Checker treats methods not explicitly annotated with \code{@MayReleaseLocks},
  \code{@LockingFree}, \code{@ReleasesNoLocks}, \code{@SideEffectFree} or \code{@Pure}
  as if they were annotated with \code{@MayReleaseLocks}.
...
The polymorphic \code{@GuardSatisfied} type annotation, when used in a method
declaration together with the \code{@LockingFree}, \code{@ReleasesNoLocks},
\code{@SideEffectFree}, or \code{@Pure} method annotation, allows the method
