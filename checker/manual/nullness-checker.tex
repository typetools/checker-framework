...
\refqualclass{checker/nullness/quals}{NonNull} and 
\refqualclass{checker/nullness/quals}{Nullable}.
\refqualclass{checker/nullness/quals}{NonNull} is rarely written, because it is
...
\item
  When an expression of non-\refqualclass{checker/nullness/quals}{NonNull} type
...
\item
  When an expression of \refqualclass{checker/nullness/quals}{NonNull} type
  ...
  As a special case of an of \refqualclass{checker/nullness/quals}{NonNull}
  type becoming null, the checker also warns whenever a field of
  \refqualclass{checker/nullness/quals}{NonNull} type is not initialized in a
...
\item
  ...
  including \refqualclass{checker/nullness/quals}{Nullable} types and primitive
  ...
  that fields of \refqualclass{checker/nullness/quals}{NonNull} type are
...
\item[\refqualclass{checker/nullness/quals}{Nullable}]
...
\item[\refqualclass{checker/nullness/quals}{NonNull}]
...
\item[\refqualclass{checker/nullness/quals}{PolyNull}]
  indicates qualifier polymorphism.  For a description of
  \refqualclass{checker/nullness/quals}{PolyNull}, see
  Section~\ref{qualifier-polymorphism}.

\item[\refqualclass{checker/nullness/quals}{MonotonicNonNull}]
  ...
  \refqualclass{checker/nullness/quals}{Nullable}, but when the variable is
  assigned, its type is treated as
  \refqualclass{checker/nullness/quals}{NonNull}.
...
\item[\refqualclass{checker/nullness/quals}{RequiresNonNull}]
...
\item[\refqualclass{checker/nullness/quals}{EnsuresNonNull}]
\item[\refqualclass{checker/nullness/quals}{EnsuresNonNullIf}]
...
\begin{description}
\item[\refqualclass{checker/initialization/quals}{Initialized}]
\item[\refqualclass{checker/initialization/quals}{UnknownInitialization}]
\item[\refqualclass{checker/initialization/quals}{UnderInitialization}]
\end{description}
...
\begin{description}
\item[\refqualclass{checker/nullness/quals}{Raw}]
\item[\refqualclass{checker/nullness/quals}{NonRaw}]
\item[\refqualclass{checker/nullness/quals}{PolyRaw}]
\end{description}
...
% \begin{itemize}
% \item
%   \refqualclass{checker/nullness/quals}{Nullable}:  Unannotated types are regarded as possibly-null, or
...
%   declaration.
% \item
%   \refqualclass{checker/nullness/quals}{NonNull}:  Unannotated types are treated as non-null.
...
%   \refqualclass{checker/nullness/quals}{NonNull}, \emph{except} that the
%   unannotated raw type of a local variable is treated as
%   \refqualclass{checker/nullness/quals}{Nullable}.  (Any generic arguments to a
%   local variable still default to
%   \refqualclass{checker/nullness/quals}{NonNull}.)  This is the standard
...
\refqualclass{checker/nullness/quals}{EnsuresNonNullIf} method annotations.
...
\refqualclass{checker/nullness/quals}{EnsuresNonNullIf}.
...
\item
  Inference of \refqualclass{checker/nullness/quals}{Nullable}:
  %
  If your code uses the standard CLIMB-to-top default (Section~\ref{climb-to-top}) or
  the \refclass{checker/nullness/quals}{NonNull} default, then use the
...
\item
  Inference of \refqualclass{checker/nullness/quals}{NonNull}:
  %
...
%% \item
%%   Use the \refqualclass{checker/nullness/quals}{AssertParametersNonNull}
...
To try the Nullness Checker on a source file that uses the \refqualclass{checker/nullness/quals}{NonNull} qualifier,
...
%
% You can view the annotated source code, which contains \refqualclass{checker/nullness/quals}{NonNull} annotations, in
% the
% %BEGIN LATEX
% \begin{smaller}
% %END LATEX
...
Your goal is to add \refqualclass{checker/nullness/quals}{Nullable} annotations
...
\item
  ...
  \refqualclass{checker/nullness/quals}{Nullable} or a
  \refqualclass{checker/nullness/quals}{MonotonicNonNull} annotation.  If the field
...
\item
    ...
    called, use \refqualclass{dataflow/quals}{Pure} (see Section~\ref{nullness-method-annotations}).
\item
    ...
    such as \refqualclass{checker/nullness/quals}{RequiresNonNull} (see Section~\ref{nullness-method-annotations}.
\end{itemize}
...
\newcommand{\linktoNonNull}{\refclass{checker/nullness/quals}{NonNull}}
\newcommand{\linktoNullable}{\refclass{checker/nullness/quals}{Nullable}}
...
\noindent
When a field \<f> is declared with a \refqualclass{checker/nullness/quals}{NonNull}
...
\item[\refqualclass{checker/initialization/quals}{UnknownInitialization}]
  ...
  \refqualclass{checker/nullness/quals}{NonNull} may be null because the field
...
\item[\refqualclass{checker/initialization/quals}{UnderInitialization}]
  ...
  \refqualclass{checker/initialization/quals}{UnderInitialization} type until
...
\refqualclass{checker/nullness/quals}{MonotonicNonNull}:  when read, they are
treated as being \refqualclass{checker/nullness/quals}{Nullable}, but when
...
\code{this} starts out with \refqualclass{checker/initialization/quals}{UnderInitialization} type.
As soon as all of the \refqualclass{checker/nullness/quals}{NonNull} fields
...
\refqualclass{checker/nullness/quals}{EnsuresNonNull}\code{(\{"field1", "field2"\})}
...
When you write \refqualclass{checker/initialization/quals}{UnderInitialization}\code{(OtherClass.class) MyClass x;}, that
...
\refqualclass{checker/initialization/quals}{UnderInitialization}\code{(\emph{C})}, rather than
treated as simply 
\refqualclass{checker/initialization/quals}{Initialized}.
...
\begin{itemize}
\item
  Declare the field as \refqualclass{checker/nullness/quals}{Nullable}.  Recall
  that if you did not write an annotation, the field defaults to
  \refqualclass{checker/nullness/quals}{NonNull}.
\item
  Declare the field as \refqualclass{checker/nullness/quals}{MonotonicNonNull}.
  This is appropriate if the field starts out as \<null> but is later set
  to a non-null value.  You may then wish to use the
  \refqualclass{checker/nullness/quals}{EnsuresNonNull} annotation to indicate
  which methods set the field, and the
  \refqualclass{checker/nullness/quals}{RequiresNonNull} annotation to indicate
...
\refqualclass{checker/initialization/quals}{Initialized}.  If
...
\refqualclass{checker/nullness/quals}{EnsuresNonNull} as well; see
...
\refqualclass{checker/nullness/quals}{RequiresNonNull},
\refqualclass{checker/nullness/quals}{EnsuresNonNull}, and
\refqualclass{checker/nullness/quals}{EnsuresNonNullIf} method annotations;
...
\refqualclass{checker/initialization/quals}{Initialized},
\refqualclass{checker/initialization/quals}{UnknownInitialization}, and
\refqualclass{checker/initialization/quals}{UnderInitialization}.
The other uses the two annotations
\refqualclass{checker/nullness/quals}{Raw} (which corresponds to \code{@UnknownInitialization}) and
\refqualclass{checker/nullness/quals}{NonRaw} (which corresponds to \code{@Initialized}).
...
that is, supply the \code{-processor org.checkerframework.checker.nullness.NullnessChecker}
...
field is declared with a \refqualclass{checker/nullness/quals}{NonNull} type.
However, this guarantee does not hold for a partially-initialized object.

The Nullness Checker uses the \refqualclass{checker/nullness/quals}{Raw} annotation to indicate that an object
...
\item[\refqualclass{checker/nullness/quals}{Raw}]
  ...
  \code{this} has \refqualclass{checker/nullness/quals}{Raw} type until all
...
Within the constructor,
\code{this} starts out with \refqualclass{checker/nullness/quals}{Raw} type.
As soon as all of the \refqualclass{checker/nullness/quals}{NonNull} fields
...
% annotated with \refqualclass{checker/nullness/quals}{Raw}.  Similar
...
When you write \refqualclass{checker/nullness/quals}{Raw}\code{ MyClass x;}, that
...
When you write \refqualclass{checker/nullness/quals}{NonRaw}\code{(Foo.class) MyClass
...
\refqualclass{checker/nullness/quals}{NonRaw}\code{(\emph{typeofthis})}, rather than
treated as simply 
\refqualclass{checker/nullness/quals}{NonRaw}.
...
The \refqualclass{checker/nullness/quals}{Raw}
annotation has nothing to do with the raw types of Java Generics.


\section{Map Key Checker\label{map-key-checker}}

The Map Key Checker uses the \refqualclass{checker/nullness/quals}{KeyFor}
...
The \refqualclass{checker/nullness/quals}{KeyFor} annotation states the latter
property.
