...
\subsection{Default annotations in unchecked \<.class>
files \label{null-defaults-classfile}}

If the Nullness Checker was not run during the compiler execution that
created a \<.class> file, then the \<.class> file contains only the
type qualifiers that the programmer wrote explicitly and defaulting is
required.
See Section~\ref{defaults-classfile} for a general discussion.

The Nullness Checker implements conservative defaults for
unchecked \<.class> files:
\begin{itemize}
\item
  For method parameter types and type parameter lower bounds, use the
  bottom qualifier \<@NonNull>.
\item
  For method return types and type parameter upper bounds, use the top
  qualifier \<@Nullable>.
\end{itemize}

The option \<-AunsafeDefaultsForUncheckedBytecode> can be used to
treat unchecked bytecode like checked source code, which is unsound,
but might reduce false positives.
...
\refmethod{checker/nullness}{NullnessUtils}{castNonNull}{-T-} method.
...
\item
The
\href{http://code.google.com/p/plume-lib/}{Plume-lib library}.
...
% LocalWords:  nonnull EnsuresNonNull ReadOnly arg 5cm orElse isPresent
...
%  LocalWords:  UnderInitialization getComponentType 2in AsuppressWarnings
