...
\iffalse
% TODO finish this description, maybe move out of nullness section

\section{Compatibility with Java~7\label{nullness-java7}}

Some of the nullness annotations used by the Checker Framework
(see Section~\ref{nullness-annotations}) are Java~8 type annotations.
Such annotations are allowed in all type use locations, but thereby
create a dependency on Java~8 classes.

To ease the transition from Java~7, there are two possible routes.

1. Using one of the aliased annotations from other projects, see
Section~\ref{nullness-related-work}. These annotations are Java~7
declaration annotations and don't create a dependency on Java~8.

2. Use compatiblity annotations defined by the Nullness Checker:
...
By using the \<Decl> suffix, it is possible to use these declaration
annotations for Java~7 compatibility and at the same time import the
Java~8 type annotations using the annotations-in-comments feature (see
Section~\ref{annotations-in-comments}).

Alternatively, use alternative type annotations defined by
...
TODO: explain the trade-offs more.
