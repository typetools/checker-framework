...
as a type qualifier. (An annotation that is written on an
annotation definition, such as
\sunjavadocanno{java/lang/annotation/Target.html}{Target}, is called a
...
The IGJ Checker adds this qualifier in code in \refclass{checker/igj}{IGJAnnotatedTypeFactory}.
...
method in one of the ways shown below.

Note that \code{createSupportedTypeQualifiers()} returns an immutable set of the
annotation classes supported by a checker.

In total, there are 5 ways to indicate annotations that are supported by a
checker:
\begin{enumerate}
\item Only support annotations located in a checker's \<qual> directory, and
\<PolyAll>:

This is the default behavior. Simply place those annotations within the \<qual>
directory.

\item Support annotations located in a checker's \<qual> directory, but without
\<PolyAll>:

Place those annotations within the \<qual>
directory, and override \<createSupportedTypeQualifiers()> by calling
\<getBundledTypeQualifiersWithoutPolyAll()> with no parameters passed in.
Code example:
...
\item Support annotations located in a checker's \<qual> directory, \<PolyAll>,
and a list of other annotations:

Place those annotations within the \<qual>
directory, and override \<createSupportedTypeQualifiers()> by calling
\<getBundledTypeQualifiersWithPolyAll()> with a varargs parameter list of the
...
\item Support annotations located in a checker's \<qual> directory and a list
of other annotations, but without supporting \<PolyAll>:

Place those annotations within the \<qual>
directory, and override \<createSupportedTypeQualifiers()> by calling
\<getBundledTypeQualifiersWithoutPolyAll()> with a varargs parameter list of the
other annotations. Code example:
...
\item Supporting only annotations that are explicitly listed:

Override \<createSupportedTypeQualifiers()> and return an immutable set of the
supported annotations. Code example:
\begin{Verbatim}
@Override
protected Set<Class<? extends Annotation>> createSupportedTypeQualifiers() {
...
The methods \code{getBundledTypeQualifiersWithoutPolyAll()}
and \code{getBundledTypeQualifiersWithPolyAll} each return an immutable set.
...
\refclass{framework/test}{CheckerFrameworkTest},
