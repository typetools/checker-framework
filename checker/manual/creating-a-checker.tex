...
\section{How checkers build on the Checker Framework\label{tool-relationships}}
...
\subsection{Relevant Java types\label{relevant-java-types}}
...
\subsection{Do not re-use type qualifiers\label{do-not-re-use-type-qualifiers}}
...
\subsection{Java expression annotations\label{expression-annotations}}
The Checker Framework has functionality to support Java expression annotations
used as type qualifiers.  An expression annotation must have one attribute,
\<value>, that is an array of strings.
Section~\ref{java-expressions-as-arguments} explains what expression can be
represented by the strings.

The Checker Framework
verifies that expression strings in annotations are valid expressions.  If
...
The Checker Framework standardize the expression strings in annotations such
that comparisons, such as isSubtype, can be implemented as string comparisons.
For example, a field \<f> can be referred to in an annotation as both ``f'' and
``this.f''.  The Checker Framework will change ``f'' to ``this.f''.
...
To use this built in functionality, override
\refmethod{framework/type}{GenericAnnotatedTypeFactory}{createExpressionAnnoHelper}{--}
and return a new ExpressionAnnotationHelper that has been initialized with the
expression annotations for your checker.

Checkers with expression annotations usually need to create expression
annotations for a given tree or node, and to compare the expression string to a
tree or node.  To do this, first convert the tree or node to a standardized
expression string by create a
...
Node)} then call ``toString'' on that object.  This string can either be used
to create an expression annotation or compared to a string in an expression
annotation.
...
\begin{enumerate}
\item{\ref{dataflow-create-classes}} Create required class and configure its
    use
...
\subsection{Create required class and configure its use\label{dataflow-create-classes}}
...
\section{Annotated JDK\label{writing-a-checker-annotated-jdk}}
...
% LocalWords:  TypeHierarchy GraphQualifierHierarchy Foo qual
% LocalWords:  QualifierHierarchy QualifierRoot createQualifierHierarchy util
% LocalWords:  createTypeHierarchy ImplicitFor treeClasses TypeMirror
...
%%  LocalWords:  MyChecker's subchecker plugins ElementType
%%  LocalWords:  CheckerFrameworkTest
