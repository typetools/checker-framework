...
compile your checker, \<\$CHECKERS/dist/checker.jar> and \<\$CHECKERS/dist/javac.jar>
...
\item{\ref{writing-type-introduction}}
  ...
  other than \<null> has a \<@\refclass{checker/nullness/quals}{NonNull}> type;
...
\item
  ...
  \<@\refclass{checker/nullness/quals}{NonNull}> type may be dereferenced.  You
...
\noindent
Write the \<@\refclass{checker/quals}{TypeQualifier}> meta-annotation on the annotation definition
...
definition, such as \<@\refclass{checker/quals}{TypeQualifier}>, is called a \emph{meta-annotation}.)
...
\item \code{@\refclass{checker/quals}{SubtypeOf}} denotes that a qualifier is a subtype of
  another qualifier or qualifiers, specified as an array of class
  literals.  For example, for any type $T$,
  \code{@\refclass{checker/nullness/quals}{NonNull}} $T$ is a subtype of \code{@\refclass{checker/nullness/quals}{Nullable}} $T$:
  ...
  %
  \<@\refclass{checker/quals}{SubtypeOf}> accepts multiple annotation classes as an argument,
  permitting the type hierarchy to be an arbitrary DAG\@.  For example,
  in the IGJ type system (Section~\ref{igj-annotations}), \<@\refclass{checker/igj/quals}{Mutable}>
  and \<@\refclass{checker/igj/quals}{Immutable}> induce two mutually exclusive subtypes of the
  \<@\refclass{checker/igj/quals}{ReadOnly}> qualifier.
...
%       "root" for ASTs, in particular for CompilationUnitTrees.
  ...
  a supertype of all other qualifiers.  For example, \<@\refclass{checker/nullness/quals}{Nullable}>
...
\item \<@\refclass{checker/quals}{PolymorphicQualifier}> denotes that a qualifier is a
  ...
  no \<@\refclass{checker/quals}{SubtypeOf}> meta-annotation and need not be
  mentioned in any other \<@\refclass{checker/quals}{SubtypeOf}>
  meta-annotation.
...
be used together.  In particular, when using the \<@\refclass{checker/quals}{SubtypeOf}>
...
using the \code{@\refclass{checker/quals}{DefaultQualifierInHierarchy}}
meta-annotation.
...
% \<@\refclass{checker/interning/quals}{Interned}>.
...
the Nullness type system has both \<@\refclass{checker/nullness/quals}{Nullable}> and
\<@\refclass{checker/nullness/quals}{NonNull}>.  Because it has no built-in meaning
...
literal (other than \<null>) has a \<@\refclass{checker/nullness/quals}{NonNull}> type.
...
The \<@\refclass{checker/quals}{ImplicitFor}> meta-annotation indicates implicit annotations.
...
For example, consider the definitions of the \<@\refclass{checker/nullness/quals}{NonNull}> and \<@\refclass{checker/nullness/quals}{Nullable}>
type qualifiers:
...
\code{@\refclass{checker/quals}{TypeQualifiers}}, which lists the annotations
...
\code{@\refclass{checker/quals}{TypeQualifier}} meta-annotation (or is
returned by the
