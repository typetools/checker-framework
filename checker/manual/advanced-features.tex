...
not all types.  In particular, unless otherwise stated, type systems use the CLIMB-to-top rule.  This
...
\<@DefaultFor(\{ DefaultLocation.EXCEPTION\_PARAMETER \})> on \<@NonNull> overrides 
the CLIMB-to-top rule.

A type system designer can specify defualts that differ from the CLIMB-to-top rule.  In
...
\<-AuseDefaultsForUncheckedCode=-byte> command-line option.)
