...
defaulting rules apply if you supply the command-line option
\<-AsafeDefaultsForUncheckedBytecode>.
...
if you supply the command-line option \<-AsafeDefaultsForUncheckedBytecode>,
...
For example, an unnannotated method in a classfile
...
Typically the bottom qualifier should be used when writing
to a field and the top qualifier when reading from the field.
Currently, for fields the same defaulting rules that apply to
source code that is read by the compiler apply when the compiler
reads a \<.class> file.  In the rare circumstance that there is a
mutable public field in a library, this can result in unsound
handling of reads and writes of this field by client code.
The Checker Framework developers are working to correct this.
...
%%  LocalWords:  AssumeAssertion AsafeDefaultsForUncheckedBytecode
