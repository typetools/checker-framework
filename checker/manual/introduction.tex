...
TODO: Organize the following, pssobly moving it into section 24.1.

You can also learn more about how to run the Checker Framework from the
command line (Section~\ref{running-command-line}), 
...
\item
  ...
  when you are not doing pluggable type-checking, has no negative
  consequences.
  ...
  (The directions use a \code{CHECKERFRAMEWORK} environment variable to the
  \code{.../checker-framework} directory.  You can set such an environment
...
\item \<checker/bin/javac> A \<javac> replacement script to use the
Checker Framework.
...
\item \<checker/dist/> Contains jar files for use by advanced users:
  ...
  \item \<jdk7.jar> and \<jdk8.jar> The annotated JDK classes for
  Java~7 and Java~8.
