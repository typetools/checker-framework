...
%% Not needed any more.
%   Then, set the
%   \code{CHECKERFRAMEWORK} environment variable to the
%   \code{checker-framework} directory.  For instance, if you use
...
\item
  ...
  First, set a \code{CHECKERFRAMEWORK} environment variable to the
  \code{.../checker-framework} directory.  Alternately, you can
  use an absolute path where the below instructions use the \code{CHECKERFRAMEWORK}
  ...
  \item
    Add directory
    \code{\$CHECKERFRAMEWORK/checker/bin} to your path, \emph{before} any other
...
%     \$CHECKERFRAMEWORK/checker/bin/javac.jar and providing jsr308-all.jar on the bootclasspath.
...
%     you can substitute \$CHECKERFRAMEWORK/checker/dist/checker.jar in order to use
