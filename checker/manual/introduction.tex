...
\end{Verbatim}
  \item
    ...
    can instead run \code{\$CHECKERS/bin/javac}.
...
\begin{Verbatim}
  alias javacheck='$CHECKERS/bin/javac'
\end{Verbatim}
...
\begin{Verbatim}
  java -jar $CHECKERS/dist/checker.jar ...
\end{Verbatim}

   More generally, anywhere that you use \<javac.jar>, you can substitute
   \<\$CHECKERS/dist/checker.jar>; the result is to use the Checker
   Framework compiler instead of the regular \<javac>.

%     Note: Previous versions of these instructions required the use of
%     \$CHECKERS/bin/javac.jar and providing jsr308-all.jar on the bootclasspath.
%     This is no longer the case.  The provided javac script and checker.jar
...
%     you can substitute \$CHECKERS/dist/checker.jar in order to use
...
\begin{Verbatim}
  # Unix
  alias javacheck='java -jar $CHECKERS/dist/checker.jar'

  # Windows
  doskey javacheck=java -jar %CHECKERS%\dist\checker.jar $*
\end{Verbatim}
\end{itemize}
...
\item The programmer writes annotations, such as \code{@\refclass{checker/nullness/qual}{NonNull}} and
  \code{@\refclass{checker/interning/qual}{Interned}}, that specify additional information about Java types.
...
\begin{Verbatim}
  ...
  java -jar $CHECKERS/dist/checker.jar -processor org.checkerframework.checker.nullness.NullnessChecker MyFile.java
\end{Verbatim}
%BEGIN LATEX
\end{smaller}
%END LATEX
...
\item \<-J> Supply an argument to the JVM that is running javac; example:
  \<-J-Djsr308\_imports=org.checkerframework.checker.nullness.qual.*:org.checkerframework.dataflow.qual.*>; see Section~\ref{implicit-import-statements}
\end{itemize}
...
guarantees that every expression whose type is a \code{@\refclass{checker/nullness/qual}{NonNull}} type never
evaluates to null.  The Interning Checker (Chapter~\ref{interning-checker})
guarantees that every expression whose type is an \code{@\refclass{checker/interning/qual}{Interned}} type
...
\code{@\refclass{checker/nullness/qual}{Nullable}} (see Section~\ref{nullness-annotations})
...
\code{@\refclass{checker/nullness/qual}{NonNull}}, indicating
...
variable is annotated as \code{@\refclass{checker/nullness/qual}{Nullable}} but it
actually never contains \<null> at run time, then change the annotation to 
\code{@\refclass{checker/nullness/qual}{NonNull}}.  The weakness might be in the
