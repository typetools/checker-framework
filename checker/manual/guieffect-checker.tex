...
\subsection{Effect annotations\label{guieffects-method-annotations}}
...
\item \code{@UIEffect} is a method annotation marking code that may access
  the UI directly.
...
\code{@PolyUIType}.
To use the effect variable in the class body, annotate a method with \code{@PolyUIEffect}.
...
\begin{itemize}
\item \code{@Safe} instantiates the type's effect to \code{@SafeEffect}.
\item \code{@UI} instantiates the type's effect to \code{@UIEffect}.  \emph{Additionally}, it changes the
default method effect for the class to \code{@UIEffect}.
\item \code{@PolyUI} instantiates the type's effect to \code{@PolyUIEffect} for the same instantiation as
the current (containing) class.  For example, this is the qualifier of the receiver \code{this}
inside a method of a \code{@PolyUIType} class, which is how one method of an effect-polymorphic
