...
\begin{itemize}
\item
  effect annotations on methods (Section~\ref{guieffects-annotations}),
\item
 class or package annotations controlling the default effect (Section~\ref{guieffects-defaults}), and
\item
...
\begin{itemize}
\item
  \refqualclass{checker/guieffects/qual}{SafeEffect}
  is a method annotation marking code that must not
  access UI objects.
\item
  \refqualclass{checker/guieffects/qual}{UIEffect}
...
\begin{itemize}
\item 
  \refqualclass{checker/guieffects/qual}{UIType}
  is a class annotation that makes the effect for unannotated methods in that
class default to
\code{@UIEffect}.  (See also \code{@UI} in Section \ref{guieffects-polymorphism-using}.)
\item
  \refqualclass{checker/guieffects/qual}{UIPackage}
...
\item
  \refqualclass{checker/guieffects/qual}{SafeType} is a class annotation that makes the effect for unannotated methods in that
...
\subsection{Defining an effect-polymorphic type\label{guieffects-polymorphism-defining}}

To declare that a class is effect-polymorphic, annotate its definition with
\refqualclass{checker/guieffects/qual}{PolyUIType}.
To use the effect variable in the class body, annotate a method with \refqualclass{checker/guieffects/qual}{PolyUIEffect}.
...
\subsection{Using an effect-polymorphic type\label{guieffects-polymorphism-using}}

To instantiate an effect-polymorphic type, write one of these three type
qualifiers before a use of the type:
\begin{itemize}
\item
  \refqualclass{checker/guieffects/qual}{Safe}
  instantiates the type's effect to \code{@SafeEffect}.
\item
  \refqualclass{checker/guieffects/qual}{UI}
...
\item
  \refqualclass{checker/guieffects/qual}{PolyUI} instantiates the type's
...
The ECOOP 2013 paper ``JavaUI: Effects for Controlling UI Object Access''
  ...
  \myurl{http://homes.cs.washington.edu/~mernst/pubs/gui-thread-ecoop2013-abstract.html}
...
%  LocalWords:  PolyUIType PolyUIEffect PolyUI UIRunnable SafeRunnable
%  LocalWords:  JavaUI JPanel myDates
