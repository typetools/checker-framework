...
\code{-Aquals} must be accessible to the compiler during compilation in the
classpath. In other words, they must already be compiled (and, typically, be on
the javac bootclasspath) before you run the Fenum Checker with \code{javac}. It
...
that contain the qualifiers using the \code{-AqualDirs} option, for example:
