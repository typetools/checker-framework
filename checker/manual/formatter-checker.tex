...
The \code{@\refclass{checker/formatter/qual}{Format}} qualifier on a string type
...
The \code{@\refclass{checker/formatter/qual}{Format}} qualifier is parameterized with
...
\code{@\refclass{checker/formatter/qual}{InvalidFormat}} indicates an invalid format
string --- that is, a string that cannot be used as a format string.  For
example, the type of \code{"\%y"} is \<@InvalidFormat String>.
\code{@\refclass{checker/formatter/quals}{FormatBottom}} is the type of the
\code{null} literal.
\code{@\refclass{framework/qual}{Unqualified}} is the default that is
