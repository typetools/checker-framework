...
JVM\@.  Sections~\ref{java5-class-files} and~\ref{java7-classfiles} discuss this situation.
...
classpath.  
% The reason is that a Java compiler issues an error if an
% imported package is not on the classpath.
The disadvantage of having the annotation definitions on the classpath is
that it makes it impossible to compile to Java 7 classfiles.
...
\subsection{Class-file compatibility with Java~7\label{java7-classfiles}}
...
To ensure that your program can run on a Java~7 JVM, you can use
annotations that are Java~7 declaration annotations.  These can only be
written at code locations that are legal for both declaration and type
annotations, such as for fields, method returns, and method parameters.

If you wish to write type annotations at other locations such as generic
type parameters and \<extends>
clauses, then you should write them in comments so that a standard Java 7
compiler will ignore them, and be sure to distributed binaries that were
created by a standard Java 7 compiler (not the Checker Framework compiler,
which reads annotations in comments and writes them to classfiles).

For example, if you wish to run the Nullness Checker using with Java~7
...
``\<*Type>'' compatibility annotations in comments.  For example, the Nullness Checker
...
\item
Use ``\<*Decl>'' compatibility annotations.  For example, the Nullness Checker
declares these declaration annotations:
...
annotations without a name conflict.

Eventually, when backward compatibility with Java~7 is not important, the
codebase should be refactored to use only the regular Checker Framework
annotations, not in comments.
