...
// its type is therefore the erasure of its declared type.  javac agrees; for the last call below
// it warns "unchecked call to put(Desc<String>) as a member of the raw type Super".
//
// TypesUtils.isRawCall only treats the call as raw when the member's enclosing element is the
// receiver's own class, so a member inherited from a raw type's supertype is not erased.
...
public class RawInheritedMember {
  ...
  // Control: the member is declared in the receiver's own class, which TypesUtils.isRawCall does
  // handle, so its signature is erased and the call is legal.
  ...
  void inheritedFromRawSupertype(Sub raw, Desc<String> d) {
    // TODO: This is a false positive.  javac erases the inherited member's signature.
    // :: error: [argument]
    raw.put(d);
    Desc<String> x = raw.get();
  }
