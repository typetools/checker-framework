...
public class Manifest implements Cloneable {
    ...
    static class FastInputStream extends FilterInputStream {
        ...
        FastInputStream(InputStream in, @NonNegative int size) {
            super(in);
            buf = new byte[size];
        }
        ...
        public @NonNegative int read(byte[] b, @NonNegative int off, @NonNegative int len) throws IOException {
            int avail = count - pos;
            if (avail <= 0) {
                if (len >= buf.length) {
         ...
         */
        public @NonNegative int readLine(byte[] b, @NonNegative int off, @NonNegative int len) throws IOException {
            byte[] tbuf = this.buf;
            int total = 0;
            while (total < len) {
                int avail = count - pos;
        ...
        public @NonNegative int readLine(byte[] b) throws IOException {
