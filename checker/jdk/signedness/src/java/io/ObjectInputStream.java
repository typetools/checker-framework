    ...
    extends InputStream implements ObjectInput, ObjectStreamConstants
{
     ...
     */
    public int read(@PolySigned byte[] buf, int off, int len) throws IOException {
        if (buf == null) {
     ...
     * @throws  IOException If other I/O error has occurred.
     */
    public void readFully(@PolySigned byte[] buf) throws IOException {
     ...
     * @throws  IOException If other I/O error has occurred.
     */
    public void readFully(@PolySigned byte[] buf, int off, int len) throws IOException {
        int endoff = off + len;
        if (off < 0 || len < 0 || endoff > buf.length || endoff < 0) {
            throw new IndexOutOfBoundsException();
        }
    ...
    private static class PeekInputStream extends InputStream {
        ...
        void readFully(@PolySigned byte[] b, int off, int len) throws IOException {
            int n = 0;
            while (n < len) {
        ...
        extends InputStream implements DataInput
    {
         ...
         * b).
         */
        int read(@PolySigned byte[] b, int off, int len, boolean copy) throws IOException {
            if (len == 0) {
                return 0;
            } else if (blkmode) {
                if (pos == end) {
        ...
        public void readFully(@PolySigned byte[] b) throws IOException {
            readFully(b, 0, b.length, false);
        }

        public void readFully(@PolySigned byte[] b, int off, int len) throws IOException {
            readFully(b, off, len, false);
        }

        public void readFully(@PolySigned byte[] b, int off, int len, boolean copy)
            throws IOException
        {
            while (len > 0) {
        ...
        void readChars(@PolySigned char[] v, int off, int len) throws IOException {
            int stop, endoff = off + len;
            while (off < endoff) {
                if (!blkmode) {
        ...
        void readShorts(@PolySigned short[] v, int off, int len) throws IOException {
            int stop, endoff = off + len;
            while (off < endoff) {
                if (!blkmode) {
                    int span = Math.min(endoff - off, MAX_BLOCK_SIZE >> 1);
        ...
        void readInts(@PolySigned int[] v, int off, int len) throws IOException {
            int stop, endoff = off + len;
            while (off < endoff) {
                if (!blkmode) {
        ...
        void readLongs(@PolySigned long[] v, int off, int len) throws IOException {
            int stop, endoff = off + len;
            while (off < endoff) {
                if (!blkmode) {
