...
then it may be impossible for users to run both tools.  The
Checker Framework necessarily uses the Dataflow Framework.
Because of this, you should distribute a version of the Dataflow Framework
with where the packages name been renamed.  This can be done using the Maven Shade Plugin,
\url{https://maven.apache.org/plugins/maven-shade-plugin/}, or the Gradle Shadow Plugin,
\url{https://imperceptiblethoughts.com/shadow/}.
If your tool cannot shadow the Checker Framework and Dataflow Framework classes, then
you can create a pull request so that one is created for you during the Checker Framework release.
