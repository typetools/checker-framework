...
\autoref{tab:nodesWithException} shows all node types whish can possibly throw an exception and type to be thrown.
Java class name of nodes are simplified as with \autoref{tab:nodes}.
All of specific Exceptions type in table are in package \code{java.lang}.
    ...
    \begin{longtable}{lp{0.6\linewidth}l}
        ...
        \code{ArrayAccess} & NullPointerException, ArrayIndexOutOfBoundsException \\
        \code{FieldAccess} & NullPointerException \\
        \code{MethodAccess} & NullPointerException \\
        \code{MethodInvocation} & Throwable, Types in throws clause of the signature \\
        \code{IntegerDivision} & ArithmeticException \\
        \code{IntegerRemainder} & ArithmeticException \\
        \code{ObjectCreation} & Throwable, Types in throws clause of the signature \\
        \code{TypeCast} & ClassCastException \\
        \code{Throw} & Type of \code{e} when \code{throw e} \\
        \code{AssertionError} & AssertionError \\
        \midrule
...
\begin{workinprogress}
There is a \code{CaseNode}. How is the default case of a switch
handled?
\end{workinprogress}
