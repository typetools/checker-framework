...
//      new Entry(0x0130, new char[]{0x0069}, new char[]{0x0130}, "az", 0), // # LATIN CAPITAL LETTER I WITH DOT ABOVE
    ...
    private static @Nullable char[] lookUpTable(String src, int index, Locale locale, boolean bLowerCasing) {
        HashSet set = (HashSet)entryTable.get(new Integer(src.codePointAt(index)));

        if (set != null) {
            Iterator iter = set.iterator();
            @Nullable String currentLang = locale.getLanguage();
            while (iter.hasNext()) {
                Entry entry = (Entry)iter.next();
                @Nullable String conditionLang= entry.getLanguage();
