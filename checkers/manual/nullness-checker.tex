...
The Checker Framework supplies several ways to suppress warnings (see
Section~\ref{suppressing-warnings}).  The Nullness checker provides two
additional ways to suppress warnings.  These are appropriate when the
Nullness Checker issues a warning, but the programmer knows for sure that
the value cannot be null and the warning is a false positive.

\begin{enumerate}
\item
  Use an assertion.

  TODO:  discuss string that must be in message
...
\item
  Use the NullnessUtils.swNonNull method.
