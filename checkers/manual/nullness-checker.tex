...
\begin{description}
\item[\code{@\refclass{initialization/quals}{Initialized}}]
\item[\code{@\refclass{initialization/quals}{UnknownInitialization}}]
\item[\code{@\refclass{initialization/quals}{UnderInitialization}}]
\item[\code{@\refclass{initialization/quals}{PolyInitialized}}]
\end{description}
...
\code{@\refclass{nullness/quals}{KeyFor}}, indicates that
...
Section~\ref{defaults}.  Its effect is to default all types to
...
\item
  ...
  %
  If your code uses the standard CLIMB-to-top (non-null-except-locals) default or
  the \refclass{nullness/quals}{NonNull} default, then use the
  \ahref{\url{http://groups.csail.mit.edu/pag/daikon/download/doc/daikon.html#AnnotateNullable}}{AnnotateNullable}
  tool of the \ahref{\url{http://pag.csail.mit.edu/daikon/}}{Daikon invariant
    detector}.
...
\item
The
\ahref{\url{http://groups.csail.mit.edu/pag/daikon/}}{Daikon invariant detector}.
Run the command \code{make check-nullness}.
...
\noindent
When a field \<f> is declared with a \code{@\refclass{nullness/quals}{NonNull}}
...
\item[\code{@\refclass{initialization/quals}{UnderInitialization}}]
  ...
  \code{@\refclass{initialization/quals}{UnderInitialization}} type until
  all the \code{@NonNull} fields have been assigned.

\item[\code{@\refclass{initialization/quals}{PolyInitialized}}]
  indicates qualifier polymorphism over initialization (see
  Section~\ref{qualifier-polymorphism}).
...
\code{this} starts out with \code{@\refclass{initialization/quals}{UnderInitialization}} type.
...
When you write \code{@\refclass{initialization/quals}{UnderInitialization}(OtherClass.class) MyClass x;}, that
...
\code{@\refclass{initialization/quals}{UnderInitialization}(\emph{C})}, rather than
treated as simply 
\code{@\refclass{initialization/quals}{Initialized}}.
...
\begin{itemize}
\item
  Declare the field as \code{@\refclass{nullness/quals}{Nullable}}.  Recall
  that if you did not write an annotation, the field defaults to
  \code{@\refclass{nullness/quals}{NonNull}}.
\item
  Declare the field as \code{@\refclass{nullness/quals}{MonotonicNonNull}}.
  This is appropriate if the field starts out as \<null> but is later set
  to a non-null value.  You may then wish to use the
  \code{@\refclass{nullness/quals}{EnsuresNonNull}} annotation to indicate
  which methods set the field, and the
  \code{@\refclass{nullness/quals}{RequiresNonNull}} annotation to indicate
...
\code{@\refclass{initialization/quals}{Initialized}}.  If
...
\code{@\refclass{nullness/quals}{EnsuresNonNull}} as well; see
...
\code{@\refclass{initialization/quals}{Initialized}},
\code{@\refclass{initialization/quals}{UnknownInitialization}}, and
\code{@\refclass{initialization/quals}{UnderInitialization}}.
The other uses the two annotations
\code{@\refclass{nullness/quals}{Raw}} (which corresponds to \code{@UnknownInitialization}) and
\code{@\refclass{nullness/quals}{NonRaw}} (which corresponds to \code{@Initialized}).
...
The Map Key Checker uses the \code{@\refclass{nullness/quals}{KeyFor}}
annotation to declare that a is valke a key in a given map.
...
%  LocalWords:  UnderInitialization PolyInitialized getComponentType 2in
