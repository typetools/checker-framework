...
all been assigned.  For details, see
Section~\ref{raw-partially-initialized}.


\begin{figure}
\begin{center}
\resizebox{!}{2.5cm}{\includegraphics{figures/nullness-and-raw}}
\end{center}
%BEGIN LATEX
...
\item
  ...
  default via the \code{@DefaultQualifier(value="NonNull",
...
\subsection{Conditional nullness}

The Nullness Checker supports a form of conditional nullness types, via the
\code{@\refclass{nullness/quals}{AssertNonNullIfTrue}} method annotation.
...
\item
  ...
  This check is performed only if the \code{nulltest} lint option is
...
\<@NonNull> type argument, as in \code{List<@Nullable Object>}.  For a more
...
In general, code can depend on \<d> not being null, but not for a
partially-initialized object.  A partially-initialized object (\code{this}
in a constructor) may be passed to a 
...
Suppose that class C extends class B.  Within the C
constructor, until the B superclass constructor is called, \code{this} has
type \code{@Raw C} and also \code{@Raw B}.  After the
superclass constructor has been called, then \code{this} has type
\code{@Raw C} and also \code{@NonRaw B}.
By the time that the constructor exits, \<this> has type \<@NonRaw C> and
also \<@NonRaw B>.

When you write \<@Raw>, the annotation applies only to the given class, not
to any superclass.  For instance, the checker interprets \<@Raw C> as
``\code{@Raw C} and also \code{{\bfseries @NonRaw} B}'', rather than
``\code{@Raw C} and also \code{@Raw B}'', which would be less useful.  The
...
% LocalWords:  Nullability typeargs nulltest AssertNonNullIfTrue listFiles
% LocalWords:  isDirectory AssertionError intraprocedurally SuppressWarnings
% LocalWords:  FindBugs's getObject
