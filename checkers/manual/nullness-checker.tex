...
In addition to the standard set of lint options (Section~\ref{lint-options}),
the Nullness checker recognizes one more extra check:

\code{nulltest} (default: off), an option to warn against null checks when
the reference is guaranteed to be non-null, as in \code{(``m'' == null)}.
...
\subsubsection{Conditional nullness}
