...
\subsection{\code{@Raw} annotation for partially-initialized objects\label{raw-partially-initialized}}
...
is different from every other tool for nullness analysis, and it inevitably
surprises programmers, so it is detrimental rather than useful as documentation.
In
practice, your best bet is to not rely on FindBugs for nullness analysis,
...
analysis is also very imprecise.  It is best to stay away from the FindBugs
nullness annotations, and use a more capable tool like the Checker
Framework for type-related analyses, even if you use FindBugs for other
purposes.)
...
% LocalWords:  isDirectory AssertionError
