...
\item
  ...
  As a special case, the checker also warns whenever a field of
  \code{@\refclass{nullness/quals}{NonNull}} type is not initialized in a
  constructor.  To disable this warning for primitive types, you can supply
  the \code{-Alint=-uninitialized} command-line option, but it is better
  style to supply an explicit initializer, even if the default value such
  as \code{0} or \code{false}.
...
finishes.  This is relevant to the Nullness Checker because within the
constructor --- that is, before initialization completes --- a \<@NonNull>
field may be observed to be non-null, until that field is set.  In
particular, the Nullness Checker issues a warning for code like this:

\begin{Verbatim}
  public class MyClass {
    // f has a non-null type, but accessing it can still cause a NullPointerException
...
\noindent
In general, code can depend on field \<f> not being null, but not in a
partially-initialized object.

The Nullness Checker uses the \<@Raw> annotation to indicate that an object
is not yet fully initialized --- that is, not all its fields have been
assigned.  Rawness is mostly relevant within the constructor, or for
references to \code{this} that escape the constructor.  Thus, use of
rawness annotations in quite rare in most code.
...
type annotation?  Here are some examples, using the following declarations:
...
\begin{tabular}{l|l}
Expression & Its nullness type or error \\ \hline
\<a> & \<@NonNull> \\
\<a.f> & \<@NonNull> \\
\<a.g> & \<@Nullable> \\ \hline
\<b> & \<@NonNull> \\
...
\<c.g> & error: deref of nullable \\ \hline
\<d> & \<@Nullable> \\
...
Currently, the type-checker requires that all fields either have a
...
\item
    ...
    such as \code{@\refclass{nullness/quals}{NonNullOnEntry}} (see Section~\ref{nullness-non-qualifiers}.
\end{itemize}
...
% LocalWords:  AssertParametersNonNull nonnull AssertNonNullAfter ReadOnly arg
% LocalWords:  keySet getField keyfor param TypeName containsValue indexOf
% LocalWords:  lastIndexOf
