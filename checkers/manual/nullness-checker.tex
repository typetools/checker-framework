...
The argument to \code{@AssertNonNullIfTrue} and
\code{@AssertNonNullIfFalse} is a Java expression, including method calls
...
\code{@\refclass{nullness/quals}{AssertNonNullIfTrue}} and
\code{@\refclass{nullness/quals}{AssertNonNullIfFalse}}.
...
The checker can perform additional checks if desired.  See
Section~\ref{suppressing-warnings} for more details about the \code{-Alint}
command-line option.
...
in a field or passed to a method before initialization is complete).  Thus, use of
...
\begin{figure}
\begin{tabular}{l|l|l}
Declarations & Expression & Expression's nullness type or error \\ \hline
...
\code{@\refclass{nullness/quals}{AssertNonNullAfter}(\{"field1", "field2"\})}
for all the fields that the helper method assigns.
It's a bit odd, but you use that same annotation, \code{@AssertNonNullAfter},
...
\paragraph{Checking assignment of all fields, not just \code{@NonNull} ones}
...
% LocalWords:  lastIndexOf deref getProperty getProperties
