...
\noindent
then you likely are using the wrong bootclasspath.  (Field
\<NATIVE\_HEADER\_OUTPUT> was added in JDK 8, but the compiler is executing
on a JDK that lacks the field.)  If you are using Ant, then one possibility
...
(On Unix variants, you can \emph{sometimes} determine a value for
\<JAVA\_HOME> by running <tt>which java</tt> and then dropping
<tt>/bin/java</tt> from the end.)
...
% LocalWords:  MyFile buildfiles ClassName JRE
