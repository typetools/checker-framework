...
\multicolumn{2}{p{3\bw}|}{\centering \ahref{\url{http://types.cs.washington.edu/annotation-file-utilities/}}{Annotation File Utilities} \par (\code{.java} $\leftrightarrow$ \code{.class} files)} 
\\ \hline
\multicolumn{8}{|p{8.5\bw}|}{\centering
  \ahref{\url{http://types.cs.washington.edu/jsr308/}}{Type Annotations} syntax
...
\item{\ref{writing-type-introduction}}
  ...
  other than \<null> has a \<@\refclass{checkers/nullness/quals}{NonNull}> type;
...
\item
  ...
  \<@\refclass{checkers/nullness/quals}{NonNull}> type may be dereferenced.  You
...
\noindent
Write the \<@\refclass{checkers/quals}{TypeQualifier}> meta-annotation on the annotation definition
...
definition, such as \<@\refclass{checkers/quals}{TypeQualifier}>, is called a \emph{meta-annotation}.)
...
subclassing \refclass{checkers/types}{QualifierHierarchy} or
\refclass{checkers/types}{TypeHierarchy} (Section~\ref{procedural-hierarchy}).
...
\item \code{@\refclass{checkers/quals}{SubtypeOf}} denotes that a qualifier is a subtype of
  another qualifier or qualifiers, specified as an array of class
  literals.  For example, for any type $T$,
  \code{@\refclass{checkers/nullness/quals}{NonNull}} $T$ is a subtype of \code{@\refclass{checkers/nullness/quals}{Nullable}} $T$:
  ...
  % (The actual definition of \refclass{checkers/nullness/quals}{NonNull} is slightly more complex.)
  ...
  %
  \<@\refclass{checkers/quals}{SubtypeOf}> accepts multiple annotation classes as an argument,
  permitting the type hierarchy to be an arbitrary DAG\@.  For example,
  in the IGJ type system (Section~\ref{igj-annotations}), \<@\refclass{checkers/igj/quals}{Mutable}>
  and \<@\refclass{checkers/igj/quals}{Immutable}> induce two mutually exclusive subtypes of the
  \<@\refclass{checkers/igj/quals}{ReadOnly}> qualifier.
...
%       "root" for ASTs, in particular for CompilationUnitTrees.
  ...
  properly annotated with \refclass{checkers/quals}{SubtypeOf}.

  The top qualifier is annotated with
  \<@SubtypeOf( \{ \} )>.  The top qualifier is the qualifier that is
  a supertype of all other qualifiers.  For example, \<@\refclass{checkers/nullness/quals}{Nullable}>
...
\item \<@\refclass{checkers/quals}{PolymorphicQualifier}> denotes that a qualifier is a
  ...
  no \<@\refclass{checkers/quals}{SubtypeOf}> meta-annotation and need not be
  mentioned in any other \<@\refclass{checkers/quals}{SubtypeOf}>
  meta-annotation.

\end{itemize}

\urldef{\isSubtypeURL}\url{api/checkers/util/GraphQualifierHierarchy.html#isSubtype(java.util.Collection,%20java.util.Collection)}

The declarative and procedural mechanisms for specifying the hierarchy can
be used together.  In particular, when using the \<@\refclass{checkers/quals}{SubtypeOf}>
...
\urldef{\createQualifierHierarchyURL}\url{api/checkers/basetype/BaseTypeVisitor.html#createQualifierHierarchy()}
\urldef{\createTypeHierarchyURL}\url{api/checkers/basetype/BaseTypeVisitor.html#createTypeHierarchy()}

While the declarative syntax suffices for many cases, more complex
type hierarchies can be expressed by overriding, in \refclass{checkers/basetype}{BaseTypeVisitor},
...
\refclass{checkers/types}{QualifierHierarchy} and \refclass{checkers/types}{TypeHierarchy}.

The \refclass{checkers/types}{QualifierHierarchy} class represents the qualifier hierarchy (not the
type hierarchy), e.g., \refclass{checkers/igj/quals}{Mutable}
is a subtype of \refclass{checkers/igj/quals}{ReadOnly}.  A type-system designer may subclass
\refclass{checkers/types}{QualifierHierarchy} to express customized qualifier
relationships (e.g., relationships based on annotation
arguments).

The \refclass{checkers/types}{TypeHierarchy} class represents the type hierarchy ---
that is, relationships between
annotated types, rather than merely type qualifiers, e.g., \<@Mutable
Date> is a subtype of \<@ReadOnly Date>.  The default \refclass{checkers/types}{TypeHierarchy} uses
\refclass{checkers/types}{QualifierHierarchy} to determine all subtyping relationships.
The default \refclass{checkers/types}{TypeHierarchy} handles
...
\urldef{\addAbsoluteDefaultURL}\url{api/checkers/util/QualifierDefaults.html#addAbsoluteDefault(javax.lang.model.element.AnnotationMirror,%20checkers.quals.DefaultLocation)}
...
using the \code{@\refclass{checkers/quals}{DefaultQualifierInHierarchy}}
meta-annotation.
...
% \<@\refclass{checkers/interning/quals}{Interned}>.
...
the Nullness type system has both \<@\refclass{checkers/nullness/quals}{Nullable}> and
\<@\refclass{checkers/nullness/quals}{NonNull}>.  Because it has no built-in meaning
...
literal (other than \<null>) has a \<@\refclass{checkers/nullness/quals}{NonNull}> type.
...
The \<@\refclass{checkers/quals}{ImplicitFor}> meta-annotation indicates implicit annotations.
When written on a qualifier, \refclass{checkers/quals}{ImplicitFor}
...
specified via any combination of five fields in \refclass{checkers/quals}{ImplicitFor}:
  ...
  \item \code{typeClasses}: an array of class literals for classes
    ...
    a subclass of \refclass{checkers/types}{AnnotatedTypeMirror}.
...
For example, consider the definitions of the \<@\refclass{checkers/nullness/quals}{NonNull}> and \<@\refclass{checkers/nullness/quals}{Nullable}>
type qualifiers:
...
For more details, see the Javadoc for the \refclass{checkers/quals}{ImplicitFor}
...
\refclass{checkers/types}{AnnotatedTypeMirror}, that extends the standard \<TypeMirror>
...
\emph{base type factory} class, \refclass{checkers/types}{AnnotatedTypeFactory},
...
\refclass{checkers/types}{AnnotatedTypeFactory} inserts the qualifiers that the programmer
...
\refclass{checkers/types}{AnnotatedTypeFactory} and override
...
Class
\refclass{checkers/basetype}{BaseAnnotatedTypeFactory}
...
\refclass{checkers/basetype}{BaseTypeVisitor}, that performs type-checking at each node of a
...
override only a few methods in \refclass{checkers/basetype}{BaseTypeVisitor}.  For example, the
...
By default, \refclass{checkers/basetype}{BaseTypeVisitor} performs subtyping checks that are
similar to Java subtype rules, but taking the type qualifiers into account.
\refclass{checkers/basetype}{BaseTypeVisitor} issues these errors:
...
A checker's entry point is a subclass of \refclass{checkers/basetype}{BaseTypeChecker}.  This entry
...
\urldef{\getSupportedTypeQualifiersURL}{\url}{http://types.cs.washington.edu/checker-framework/current/api/checkers/basetype/BaseTypeChecker.html#getSupportedTypeQualifiers()}

The checker class must be annotated by
\code{@\refclass{checkers/quals}{TypeQualifiers}}, which lists the annotations
...
\code{@\refclass{checkers/quals}{TypeQualifier}} meta-annotation (or is
returned by the
\ahref{\getSupportedTypeQualifiersURL}{\<BaseTypeChecker\-.getSupportedTypeQualifiers>}
method).

\urldef{\reportURL}{\url}{http://types.cs.washington.edu/checker-framework/current/api/checkers/source/SourceChecker.html#report(checkers.source.Result,%20java.lang.Object)}
...
\refclass{checkers/basetype}{BaseTypeChecker} uses the convention to
...
multiple checkers.  Extend \refclass{checkers/source}{AggregateChecker} and override
...
to the full message text when reporting errors or warnings.  This is useful
for tools that wish to parse the Checker Framework output.
...
pseudo-assignment check (as performed by \refclass{checkers/basetype}{BaseTypeVisitor}; see Section
...
should be revsed, or parts eliminated, in light of that document.)
...
\refclass{checkers/types}{AnnotatedTypeMirror}, instead.  \code{AnnotatedTypeMirror}
...
(\refclass{checkers/basetype}{BaseTypeVisitor} extends
\refTreeclass{util}{TreePathScanner}) and for type
checking (\refclass{checkers/types}{TreeAnnotator} implements
...
\code{Utils} suffix instead (e.g., \refclass{javacutils}{TypesUtils},
\refclass{javacutils}{TreeUtils}, \refclass{javacutils}{ElementUtils}), with one
notable exception: \refclass{checkers/util}{AnnotatedTypes}.
...
\refclass{javacutils}{AbstractTypeProcessor}; these get run on each class
source file after the compiler confirms that the class is valid Java code.

The most important methods of \refclass{javacutils}{AbstractTypeProcessor}
...
\ahref{\url{http://gcc.gnu.org/java/}}{gcj}.  This also limits the tightness of
the integration of the Checker Framework into other IDEs such as \ahref{\url{http://www.jetbrains.com/idea/}}{IntelliJ IDEA}\@.
...
% LocalWords:  typeProcess getSupportedSourceVersion fenum classpath
