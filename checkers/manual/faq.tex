...
locations or only type annotations.  (It's OK for it to list both
\<ElementType.TYPE\_PARAMETER> and \<ElementType.TYPE\_USE>, but no other
declaration location along with \<ElementType.TYPE\_USE>.)
...
The goal property is that versions of widgets don't get assigned to
...
annotations.  In particular, the method annotation should be a type
annotation on the \emph{receiver} of the method.  The property being
checked is that the annotation on the type of the actual receiver is
compatible with the annotation on the type of the formal parameter
receiver.  No annotation belongs on the method itself.


Another example is JCIP's \<@GuardedBy> annotation~\cite{Goetz2006}.  As
discussed in Section~\ref{jcip-annotations}, it means two unrelated things
when applied to a field or a method.  To reduce confusion and increase
expressiveness, the Lock Checker (see Section~\ref{lock-checker}) uses the
...
% LocalWords:  castNonNull createWidget backporting JCIP's GuardedBy Awarns
