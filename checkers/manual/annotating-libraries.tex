...
libraries, such as the JDK\@.  It does this in two different ways.  The
Nullness, Javari, and IGJ Checkers use an annotated JDK
(Section~\ref{skeleton}), and all other checkers use stub classes
(Section~\ref{stub}).  These annotated libraries are automatically used, so
usually you do not have to think about them.
...
distribution and used automatically by the Interning Checker;
see file \code{checkers/src/checkers/interning/jdk.astub}, which
...
\subsection{Distributing stub files with the Checker Framework\label{stub-distribution}}

A stub file that is distributed with the Checker Framework doesn't need to
be explicitly specified by the
...
whatever other arguments you usually use, including \code{-classpath}.
...
The annotated JDK should \emph{not} be in your classpath at run time.
...
% LocalWords:  IGJ's jdk Astubs skipClasses astub AskipClasses toArray IGJ
