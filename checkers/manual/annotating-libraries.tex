...
this problem is to add annotations to the library (though you can instead
suppress all warnings related to an unannotated library by use of the 
\code{-AskipClasses} command-line option).  If you have source code for the
...
\code{.jar}) form.  It is also useful if you do not wish to edit the
...
\item You can annotate the compiled
  ...
  (\myurl{http://groups.csail.mit.edu/pag/jsr308/annotation-file-utilities/}).
...
\subsection{Using stub classes\label{stub}\label{stub-creating-and-using}}
...
\subsubsection{Using a stub file\label{stub-using}}
...
The stub file format differs from Java source code in the following
ways:
\begin{description}
...
% As of 9/27/2008.  Update as these bugs are fixed.
...
% \subsection{Installing the skeleton class generator\label{skeleton-installing}}
%
...
% LocalWords:  IGJ's jdk Astubs skipClasses astub
