...
that you used \code{<String>asList()} instead of just \code{aslist()}, or
\code{List<?\ extends String>} instead of just \code{List<String>}, because
...
\refclass{regex}{RegexUtil.CheckedPatternSyntaxException}. This exception is
functionally the same as a
\sunjavadoc{java/util/regex/PatternSyntaxException.html}{Pattern\-Syntax\-Exception}
except it is checked to guarantee that the error will be handled up the call stack.
...
% LocalWords:  regexException PatternSyntaxException 9cm Matcher
