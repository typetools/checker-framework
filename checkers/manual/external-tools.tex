...
If you use \code{javac} compiler from the command line, then you can use the
Java 7 \code{javac} that is bundled with the Checker Framework.  The bundled
\code{javac} recognizes type annotations, and annotations in
comments (see Section~\ref{annotations-in-comments}).
(Eventually, you will be able to use any Java 7 compiler, such as the
OpenJDK compiler, but Oracle has
been slow to incorporate all the patches, so the bundled \code{javac} is
