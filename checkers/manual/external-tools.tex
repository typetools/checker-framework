...
If you use the \ahref{http://maven.apache.org/}{Maven} tool,
...
\begin{Verbatim}
    <dependencies>
        ...
        <!-- annotations for the standard checkers: nullness, interning, locking, ... -->
        <dependency>
...
To run just compilation and checking you can run:
mvn compile checkers:check

%We can relax this and force the Checker Framework to either make or omit the destination directory
%which is currently causing javac to fail if we don't run the compilation phase first
If class files have already been generated, you can run:
mvn checkers:check
...
\end{Verbatim}
By default, the Checker Framework Maven plugin will only check source files and will not produced
...
a build from failing and still produce error and warning output, set \code{failOError} to false.
...
by default.  That is, if you are using the Checker Framework Maven plugin version 1.6.3 then it will run the
Checker Framework version 1.6.3 when checking source code.  However, you can specify an earlier (as early as 1.5.0)
or later version of the Checker Framework using the \code{checkersVersion} element.

\begin{Verbatim}
    <checkersVersion>1.6.2</checkersVersion>
\end{Verbatim}
