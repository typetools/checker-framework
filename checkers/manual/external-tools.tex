...
instead use the Type annotations compiler that is bundled with the Checker
...
(Note: the classes in \<checkers.jar> are
actually  read from \<jsr308-all.jar> due to the \<-Xbootclasspath> flag.
Eventually, it may be possible to remove one of these flags.)
        ...
        <plugins>
            <plugin>
                ...
                <configuration>
                    ...
                    <!-- versions of checkers to use; defaults to the current newest version: 1.4.0 -->
...
which is run by specifying command-line arguments.  See the
...
You can also combine these methods as follows:
