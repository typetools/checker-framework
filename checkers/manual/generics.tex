...
\code{A<X>} is a subtype of \code{B<Y>} only if $\<X> \equiv \<Y>$.  For
...
type to be \emph{covariant} in the type parameter.  This means that a
subtype's type parameter can be a subtype of the supertype's type
parameter.  Use the \<@\refclass{nullness/quals}{Covariant}> annotation to indicate
this.
...
Another example would be a hypothetical class \<ImmutableList>.

The \<@Covariant> annotation is trusted but not checked.
If you write \<@Covariant> in a position that can be written (say, by a
\<set> operation), then you have created an unsoundness in the type system.
For example, it would be incorrect to annotate the type parameter of
\<ListIterator>, which supports a \<set> operation.
...
\subsection{\label{qualifier-polymorhism-vs-subtyping}Relationship to subtyping and generics}
...
type qualifiers only (not the base Java types).
...
\subsection{\code{@PolyAll} qualifier applies to every type system}
...
\begin{Verbatim}
  ...
  public static int indexOfEq(/*@PolyAll*/ Object[] a, Object elt) {
...
// Omit this:
//    *  The arguments must be non-null and have equal length.

\begin{Verbatim}
  /** Returns true if the arrays are elementwise equal,
   *  testing for equality using == (not the equals method). */
  public static int eltwiseEqualUsingEq(/*@PolyAll*/ Object[] a, /*@PolyAll*/ Object[] b) {
    for (int i=0; i<a.length; i++)
      if (elt != a[i])
        return false;
    return true;
...
\begin{Verbatim}
  public static int eltwiseEqualUsingEq(/*@PolyAll(1)*/ Object[] a, /*@PolyAll(2)*/ Object[] b)
\end{Verbatim}
...
\<@PolyAll @NonNull String s> is polymorphic over every type system
...
%  LocalWords:  MyList1 MyList2 MyList4 MyList3 MyClass2 toUpperCase
