...
This mechanism permits annotations to be written in comments, but it does
...
\item
There is no mechanism for the compiler to read import statements written in
comments, as it does for annotations in comments (see
Section~\ref{annotations-in-comments}).
% The annotations in comments feature is fragile and its implementation is
% complex.  It can already pick up unintended comments, and it cannot be
% turned off.  Using implicit import statements seems less risky and
% equally effective.  If you want to implement import statements in
% comments, we would consider accepting such a patch.
...
\item
  Set the environment variable for a single comamnd, via a \code{javac}
...
makes this difficult), you can instead use the javac command-line argument:
\code{-jsr308_imports ...} or \code{-Djsr308.imports=...}.  The same
syntax, and warnings about quoting from the command line, apply as for the
...
% LocalWords:  AskipDefs mypackage
