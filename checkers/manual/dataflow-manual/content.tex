...
This document describes a \emph{dataflow analysis framework} for the
...
Framework is divided into three projects: \code{javacutils},
\code{dataflow}, and \code{checkers}, which can be built into distinct
jar files.

\code{javacutils} provides convenient interfaces to routines in
...
Element.  The \code{javacutils.trees} package provides a class to
...
The \code{dataflow} project depends on \code{javacutils}, while the
...
As is standard, a control-flow graph in our framework is a set of
...
\begin{definition}[Nodes]
    ...
    is considered at that point in the basic block. In our example of
...
produced by our translation.  An example of a field assignment
...
left out).  To relieve the user of our dataflow framework from
...
Our solution is offer the user of the dataflow framework, and
ultimately the user of the checker, the option of stating that
...
When assertions are assumed to be enabled, CFG Nodes are built
...
\code{javacutils} to make the process easier.  The new trees are
...
Symbol.VarSymbol called \code{javacutils.tree.DetachedVarSymbol} for
...
graph is performed and what the user of our framework has to implement
to define a particular analysis.


\subsection{Overview}

Roughly, a dataflow analysis in our framework works as follows. Given
...
To support both kinds of queries, our framework builds a map from AST
...
In the concurrent mode, the dataflow analysis cannot not infer any
...
use our own representation of field accesses (that is different from
