...
% TODO:  Move most of this to the section on defining the type system, with
% a cross-reference from here saying that sometimes you are not permitted
...
qualifier.  Another way of saying this is that the default is \<@Unqualified>.
In such a case, it is not sensible for a user to specify a default for
unannotated types via \<@DefaultQualifier>.
...
It is usually best for a type system designer to make the
type hierarchy ``complete'', in the sense that there is a qualifier for
every location in the hierarchy.
The reason is that this permits users to specify defaults.
