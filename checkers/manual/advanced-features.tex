...
\item
  ...
  % Type
  % qualifier refinement is implemented by the \refclass{checkers/flow}{Flow} class.
...
\item
  Use \code{@\refclass{checkers/quals}{Unqualified}}, which the framework
...
% \refclass{checkers/util}{QualifierDefaults} class.)


The end-user programmer specifies a default qualifier by writing the \code{@\refclass{checkers/quals}{DefaultQualifier}}
annotation on a package, class, method, or variable declaration.  The
argument to \<@\refclass{checkers/quals}{DefaultQualifier}> is the \code{String}
...
the default in all locations.  See the Javadoc of \refclass{checkers/quals}{DefaultQualifier} for details.
...
\<@\refclass{checkers/quals}{DefaultQualifier}> annotations at a single location,
use
\<@\refclass{checkers/quals}{DefaultQualifiers}> instead.  For example:
...
\code{@\refclass{checkers/nullness/quals}{NonNull}}.  That means that when a user
...
\<\code{@\refclass{checkers/nullness/quals}{Nullable}}> is the top qualifier in the
hierarchy, see Figure~\ref{fig-nullness-hierarchy}).

%% Users don't care about this detail.
% The CLIMB-to-top rule is an improvement over a previous defaulting rule,
% called NNEL for ``non-null except locals''~\cite{PapiACPE2008}.
...
\code{@\refclass{checkers/quals}{DefaultQualifier}} annotation; see
...
The checker treats a variable or expression as \code{@\refclass{checkers/nullness/quals}{NonNull}}
\begin{itemize}
\item
...
treated as a \code{@\refclass{checkers/nullness/quals}{NonNull}} one.  The inference never treats a variable as
a supertype of its declared type (e.g., an expression of \code{@\refclass{checkers/nullness/quals}{NonNull}}
...
The \code{@\refclass{dataflow/quals}{TerminatesExecution}} annotation
...
Type systems that do not support a run-time test, but could do so with some
...
\code{@\refclass{checkers/nullness/quals}{EnsuresNonNull}}.
...
Usually, you only need to use \code{@\refclass{dataflow/quals}{SideEffectFree}}.


\begin{description}

\item[\code{@\refclass{dataflow/quals}{SideEffectFree}}]
  indicates that the method has no (visible) side effects.

\item[\code{@\refclass{dataflow/quals}{Deterministic}}]
  indicates that if the method is called multiple times with the same
  arguments, then it returns the same result.

\item[\code{@\refclass{dataflow/quals}{Pure}}]
...
\noindent
(where \code{@\refclass{dataflow/quals}{Pure}} means both
\code{@\refclass{dataflow/quals}{SideEffectFree}} and
\code{@\refclass{dataflow/quals}{Deterministic}}).  Every overriding
...
\item \code{@\refclass{checkers/quals}{RequiresQualifier}}
\item \code{@\refclass{checkers/quals}{EnsuresQualifier}}
\item \code{@\refclass{checkers/quals}{EnsuresQualifierIf}}
...
%  \code{@\refclass{checkers/nullness/quals}{KeyFor}}.  Write the variable name.  For
%  example: \<myLocalVar>.
\item
...
the \code{@\refclass{checkers/quals}{Unused}} annotation
...
% \code{@\refclass{checkers/quals}{Dependent}} annotation.
...
% hierarchies.
% 
% When the \code{@\refclass{checkers/quals}{Unused}} annotation is sufficient, you
% should use it instead of \code{@Dependent}.
% 
% Here is a restatement of the example of Section~\ref{unused-fields}, using
% \code{@\refclass{checkers/quals}{Dependent}}:
% 
% \begin{Verbatim}
% @interface Mammal { }
% @interface Marsupial { }
...
%  LocalWords:  AenablePurity AassumeSideEffectFree iMplicit instanceof m2
...
%  LocalWords:  EnsuresQualifierIf AsuppressWarnings AinvariantArrays
