...
\code{@\refclass{nullness/quals}{NonNull}}.  That means that when a user
...
\<@\code{@\refclass{nullness/quals}{Nullable}}> is the top qualifier in the
...
instanceof should be defaulted to top becuse they are the locations to
...
and the declaration is interpreted as \code{class C<T extends @Nullable
  ...
  @NonNull Object> \ttlcb\ ... \ttrcb}.  The latter would forbit
...
an assignment.  The assignment or test may occur within a method call, in
which case it is signaled by use of a postcondition annotation such as
\code{@\refclass{nullness/quals}{EnsuresNonNull}}.
...
% Need to periodically check/update this list.
\begin{itemize}
...
\item a method invocation on any expression.
  The method must be side-effect-free and have no formal parameters.  For example:
...
representation, and it is much simpler and more usable than alternative
approaches.
% such as defining specification fields or ghost fields
...
% \ahref{\myurl{http://types.cs.washington.edu/jsr308/specification/java-annotation-design.html#receiver-type-parameter-annotations}}{extending}
...
%  LocalWords:  getPouchSize TerminatesExecution myvar myField getField
%  LocalWords:  computeValue ReadOnly IGJ AsuggestPureMethods
%  LocalWords:  AenablePurity AassumeSideEffectFree
%  LocalWords:  AassumeAssertionsAreEnabled
