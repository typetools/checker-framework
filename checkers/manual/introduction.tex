...
Installation consists of one required and one optional step, which we
overview here.
\begin{enumerate}
\item Unpack the distribution.
\item (Optional) Add a new version of \<javac> to your path, so the new
  compiler is used when you type \code{javac}.  If you plan to use the
  compiler primarily via Ant or Maven, you can skip the optional step.

  If you skip the optional step, you can still use the compiler from the
  command line, but you will have to type a slightly longer command:  you
  can either use the full pathname to the new version of \<javac>, or you
  can type \<java -jar jsr308-all.jar> instead of \<javac>.

  (In the unlikely event that you use \<-J> arguments to \<javac>, the
  \<java -jar jsr308-all.jar> alternative also requires you to remove \<-J>
  and pass them before \<-jar>.  For example, \<javac -J-Xmx512m Test.java>
  would become \<java -Xmx512m -jar jsr308-all.jar Test.java>.)

  % Mahmood will try to fix this.
  (The \<java -jar> approach does not work if you are using the default JDK
  for Mac OS.  It does work with
  \ahref{http://landonf.bikemonkey.org/static/soylatte/}{SoyLatte},
  however.)
\end{enumerate}
...
\begin{alltt}
  # If you ran the optional installation step from Section \ref{installation}.
  javac -processor \textit{ProcessorName} Foo.java Bar.java

  # If you did not run the optional installation step from Section \ref{installation}.
  # Give the full pathname to the jsr308-all.jar file.
  java -jar jsr308-all.jar -processor \textit{ProcessorName} Foo.java Bar.java
\end{alltt}
...
\item
  Run the nullness checker on the class.  Either run one of these commands:

\begin{Verbatim}
  # Use the first line if you ran the optional installation step; use the second line otherwise.
  javac -processor checkers.nullness.NullnessChecker GetStarted.java
  java -jar ...jsr308-all.jar -processor checkers.nullness.NullnessChecker GetStarted.java
\end{Verbatim}
...
\subsection{Do not annotate local variables\label{tips-local-inference}}
...
% LocalWords:  callees Xmx unconfuse
