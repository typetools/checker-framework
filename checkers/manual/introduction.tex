...
\item The programmer writes annotations, such as \code{@\refclass{checkers//checkers/nullness/quals}{NonNull}} and
  \code{@\refclass{checkers//checkers/interning/quals}{Interned}}, that specify additional information about Java types.
...
\item \<-J> Supply an argument to the JVM that is running javac; example:
  \<-J-Djsr308\_imports=checkers.nullness.quals.*:dataflow.quals.*>; see Section~\ref{implicit-import-statements}
\end{itemize}
...
guarantees that every expression whose type is a \code{@\refclass{checkers//checkers/nullness/quals}{NonNull}} type never
evaluates to null.  The Interning Checker (Chapter~\ref{interning-checker})
guarantees that every expression whose type is an \code{@\refclass{checkers//checkers/interning/quals}{Interned}} type
...
\code{@\refclass{checkers//checkers/nullness/quals}{Nullable}} (see Section~\ref{nullness-annotations})
...
\code{@\refclass{checkers//checkers/nullness/quals}{NonNull}}, indicating
...
variable is annotated as \code{@\refclass{checkers//checkers/nullness/quals}{Nullable}} but it
actually never contains \<null> at run time, then change the annotation to 
\code{@\refclass{checkers//checkers/nullness/quals}{NonNull}}.  The weakness might be in the
...
%  LocalWords:  AresourceStats noannotationsincomments jls r78 JDKs
%  LocalWords:  AinvariantArrays AcheckCastElementType
