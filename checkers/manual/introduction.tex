...
The installation process is simple!  Just download the Checker
Framework distribution
(\ahrefurl{http://types.cs.washington.edu/checker-framework/current/checkers.zip})
and unzip it to create a \code{checkers} folder.

(If you wish to have a set of commands that you can copy and paste into
your command shell, see Section~\ref{javac-installation}.)

% You can put it anywhere convenient and memorable.

To ensure that it was installed properly, run the following command:

\begin{Verbatim}
  java -jar /path/to/../checker-framework/checkers/binary/jsr308-all.jar -version  # in Linux
  java -jar c:\Path\To\...\checker-framework\checkers\binary\jsr308-all.jar -version # in Windows
\end{Verbatim}

The output should be:

\begin{Verbatim}
  javac 1.7.0-jsr308-1.1.0b
\end{Verbatim}
...
\code{javac}, add the directory \code{.../checker-framework/checkers/binary} to the
...
\begin{alltt}
  # Use this version if you ran the optional installation step from Section \ref{installation}.
  javac -processor \textit{ProcessorName} Foo.java Bar.java

  # Use this version if you did not run the optional installation step from Section \ref{installation}.
...
issuing the same worning multiple times.  For example, in this code:
