...
A pluggable type-checker enables you to certain detect bugs in your code,
...
Additional topics are covered later in the manual:
\begin{itemize}
\item Section~\ref{advanced-type-system-features}: Advanced type system features
\item Section~\ref{warnings-and-legacy}: Handling warnings and legacy code
\item Section~\ref{annotating-libraries}: Annotating libraries
\item Section~\ref{writing-a-checker}: How to create a new checker
\end{itemize}
...
For your convenience, inside the the Checker Framework distribution \code{.zip} file
is a jar file,
...
Section~\ref{external-tools} for details about
...
\subsubsection{Summary of command-line options\label{checker-options}}
...
Checker to check; see Section~\ref{basic-checker}.
...
\item \<-sourcepath> Indicates where to find the skeleton classes;
  see Section~\ref{skeleton-using}
\item \<-implicit:class> Enables processing for implicitly compiled files
...
\subsubsection{Checker auto-discovery\label{checker-auto-discovery}}
...
% LocalWords:  html JCIP MultiSet Astubs Afilenames Anomsgtext Ashowchecks
% LocalWords:  Aquals processorpath
