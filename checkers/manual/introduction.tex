...
In some sense, \code{null} is a legal argument with a well-defined
semantics:  throw an exception.  Another reason \code{@Nullable} might seem
like a reasonable annotation is that it describes a possible program
execution:  it might be possible for \code{null} to flow there, if your
program has a bug.
...
the intended use of the method.  The \code{@NonNull} annotation will enable
...
annotation would be \code{@NonNull}.  Given an arbitrary Multiset
\code{ms}, the only way to be sure not to throw an exception is to pass
only non-\code{null} elements to its \code{add} method.  A particular class
...
reference can call \code{add(null)} with confidence that no
...
% LocalWords:  subpackages bak tIDE Multiset NullPointerException
