...
\item
  ...
  You can do this in four ways.  You can use any one of them.  However, if
  you are using the Windows command shell, you must use the last one.  The
  instructions assume an \code{CHECKERS} environment variable that is set
  to the \code{.../checker-framework/checkers} directory, but you can just
...
% Unconfuse Emacs LaTeX mode:  $
 ...
 \item
   Introduce an alias that 
   uses any version of javac, but passes an extra \code{-Xbootclasspath/p:} and
   \code{-jar} argument.  For example:
