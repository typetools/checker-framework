...
\item
  ...
  performed on variables of correct units of measurement
  (see \chapterpageref{units-checker})
\item
  \ahrefloc{external-checkers}{External checkers} provided by third
  parties (see \chapterpageref{external-checkers})
...
\item
  ...
  you run \code{javac}, you will use the updated compiler.
  Alternately, whenever this document tells you to run \code{javac}, you
  can instead run \code{.../checker-framework/checkers/binary/javac}.
...
\item
  Use any version of javac, but pass an extra \code{-Xbootclasspath/p} and
  \code{-jar} argument.  For
  example (don't forget to fill in the ``...''!):

\begin{Verbatim}
  # Unix
  java -Xbootclasspath/p:.../checker-framework/checkers/binary/jsr308-all.jar \
       -jar .../checker-framework/checkers/binary/jsr308-all.jar -version

  # Windows
  java -Xbootclasspath/p:C:\...\checker-framework\checkers\binary\jsr308-all.jar
       -jar C:\...\checker-framework\checkers\binary\jsr308-all.jar -version
\end{Verbatim}
...
\item
  ...
  \item
    Code compiled with the \code{-AskipClasses} property (see
...
Ordinarily, the javac compiler halts processing as soon as it encounters a
file with compilation errors.  To convert pluggable-type-checking errors
into warnings so that you can see them all, use \code{-Awarns -Xmaxwarns
  10000} (see Section~\ref{running}).
