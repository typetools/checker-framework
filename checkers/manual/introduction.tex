...
tIDE, an opensource Java IDE, supports the Checker Framework.  See its
...
value or type.
%  --- the type is dynamically, not statically, deterimined.
...
A more informative declaration (assuming you have defined the qualifier
\code{@Married}) would be
...
Then, if a person is known to be \code{@Married}, the \code{spouse} field
is known to be non-\code{null}:
...
declaration of the \code{spouse} variable, the Nullness checker would have
complained that \code{toString} was being invoked on a possibly-\code{null}
value.

\subsection{Fields access control\label{unused-fields}}

It is desired sometimes to restrict use of a field that get
initialized only in certain contexts.

The Checker Framework supports a form of access control, via the
\code{@\refclass{quals}{Unused}} annotation.  This annotation
declares that when the receiver of a particular qualified type, the
field may not be accessed.

\subsubsection{Example}

Consider the example of dependent types
(Section~\ref{dependent-types-example}).  Suppose we have a class
\code{Person} and a field \code{spouse} that is only initialized if
the person is married.  Section~\ref{dependent-types-example} offers a
possible implementation using dependent types.

Another more informative declaration using field access control
(assuming you have defined the qualifier \code{@Single}) would be
