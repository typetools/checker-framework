...
/**
 * A node for the narrowing primitive conversion operation.  See JLS
 * 5.1.3 for the definition of narrowing primitive conversion.
 *
 * A {@link NarrowingConversionNode} does not correspond to any tree node
 * in the parsed AST.  It is introduced when a value of some primitive
 * type appears in a context that requires a different primitive with
...
public class NarrowingConversionNode extends Node {
	...
	public NarrowingConversionNode(Node operand, TypeMirror type) {
		assert TypesUtils.isPrimitive(type) :
			"non-primitive type in narrowing conversion";
