...
import static checkers.nonnull.NonNullFlow.hasVar;
import static checkers.nonnull.NonNullFlow.isNull;
import static checkers.nonnull.NonNullFlow.var;
 ...
 * 
 * FLOWTODO: extract any common behavior to a FlowConditions superclass. FLOWTODO: can
 * we benefit from using FlowState?
 */
//Note: this code is originally based on NullnessFlowConditions
public class NonNullFlowConditions extends SimpleTreeVisitor<Void, Void> {
	...
	protected final NonNullAnnotatedTypeFactory typefactory;
	protected final NonNullFlow nullnessFlow;

	protected final PrintStream debug;

	public NonNullFlowConditions(NonNullAnnotatedTypeFactory tf,
			NonNullFlow nf, PrintStream debug) {
		this.typefactory = tf;
	 ...
	 */
	public @ReadOnly
	Set<VariableElement> getNonnullElements() {
	 ...
	 */
	public @ReadOnly
	Set<VariableElement> getNullableElements() {
	 ...
	 */
	private @ReadOnly
	Set<VariableElement> getElements(boolean isNN) {
	...
	public @ReadOnly
	List<String> getNonnullExpressions() {
		return nonnullExpressions;
	}

	public @ReadOnly
	List<String> getNullableExpressions() {
		return nullableExpressions;
	}

	public @ReadOnly
	List<VariableElement> getExplicitNonnullElements() {
		return nonnullElements;
	}

	public @ReadOnly
	List<VariableElement> getExplicitNullableElements() {
	...
	public @ReadOnly
	Set<Element> getExcludes() {
		return excludes;
	}

	public @ReadOnly
	Map<Tree, AnnotationMirror> getTreeResults() {
		return treeResults;
	}
	...
	public Void visitUnary(final UnaryTree node, final Void p) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::visitUnary: " + node);
		}
	...
	public Void visitInstanceOf(InstanceOfTree node, Void p) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::visitInstanceOf: " + node);
		}
	...
	private void splitAndMerge(Tree left, Tree right, final boolean mergeAnd) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::splitAndMerge; left: " + left
					+ " right: " + right + " mergeAnd: " + mergeAnd);
		}
	...
	public Void visitConditionalExpression(ConditionalExpressionTree node,
			Void p) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::visitConditionalExpression: "
					+ node);
		}
	...
	private void mark(Element var, boolean isNonnull) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::mark; var: " + var
					+ " isNonnull: " + isNonnull);
		}
	...
	public Void visitBinary(final BinaryTree node, final Void p) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::visitBinary: " + node);
		}
		...
		if (oper == Tree.Kind.CONDITIONAL_AND) {
			...
			if (var != null) {
				...
				if (leftType.hasEffectiveAnnotation(typefactory.NONNULL)
						&& !rightType
								.hasEffectiveAnnotation(typefactory.NONNULL))
					mark(var(right), true);
			...
			// FLOWTODO: why is there no handling of Poly and NONNULL here??
			...
			if (isNull(right) && isUseOfStaticVariableElement(left))
				this.nonnullElements.add((VariableElement) TreeUtils
						.elementFromUse(left));
			else if (isNull(left) && isUseOfStaticVariableElement(right))
				this.nonnullElements.add((VariableElement) TreeUtils
						.elementFromUse(right));

		} /*
		 * else { System.out.println("Also looking at: " + left + " " + oper +
		 * " " + right); visit(left, p); visit(right, p); }
		 */
	...
	public Void visitIdentifier(final IdentifierTree node, final Void p) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::visitIdentifier: " + node);
		}
	...
	public Void visitMemberSelect(final MemberSelectTree node, final Void p) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::visitMemberSelect: " + node);
		}
	...
	public Void visitAssignment(final AssignmentTree node, final Void p) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::visitAssignment: " + node);
		}
	...
	public Void visitMethodInvocation(MethodInvocationTree node, Void p) {
		if (debug != null) {
			debug.println("NonNullFlowConditions::visitMethodInvocation: "
					+ node);
		}
