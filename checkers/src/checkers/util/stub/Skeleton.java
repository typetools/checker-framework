...
public class Skeleton implements ClassVisitor {
    ...
    private void appendGenerics(final int access, final String signature) {
        ...
        r.accept(sv);
        buf.append(toClassName(sv.getDeclaration())).append(" ");
    }

    private void appendGenericsField(final int access, final String signature) {
        if (signature == null) return;
        TraceSignatureVisitor sv = new TraceSignatureVisitor(access);
        SignatureReader r = new SignatureReader(signature);
        r.accept(sv);
        // Hack!
        String str = toClassName(sv.getDeclaration());
        if (str.contains("<"))
    ...
    private void appendFieldType(final int access, final String desc) {
        ...
        r.acceptType(sv);
        buf.append(adjustType(sv.getDeclaration()));
    }
    ...
    private void appendReturnType(final int access, final String signature,
        ...
        buf.append(adjustType(ret)).append(" ");
    }
    ...
    private String formalAndTypeParams(final int access, final String
        ...
        if ((access & Opcodes.ACC_VARARGS) != 0) {
            // need to replace last argument from an array to ...
    ...
    private void appendParamType(final int access, final String signature,
        ...
        if (allparams.length() == 0)
            ...
            String params = adjustType(allparams.substring(paren));
            buf.append(params);
        }
    }
    ...
    public void visit(final int version, final int access, final String name,
        ...
        // If it's an inner class make static
        if (name.contains("$"))
            buf.append("static ");
        appendType(access);
        ...
        if (signature != null) {
            ...
            if (superName != null && !toClassName(superName).equals("java.lang.Object")
                    && ((access & Opcodes.ACC_ENUM) == 0))
                buf.append("extends " + toClassName(superName) + " ");
            if (interfaces != null && interfaces.length != 0) {
                buf.append((access & Opcodes.ACC_INTERFACE) == 0 ? "implements " : "extends ");
                buf.append(toClassName(interfaces[0]));
                for (int i = 1; i < interfaces.length; ++i)
                    buf.append(", " + toClassName(interfaces[i]));
                buf.append(" ");
            }
        }
        buf.append("{");
