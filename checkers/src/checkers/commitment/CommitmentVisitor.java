...
public class CommitmentVisitor<Checker extends CommitmentChecker> extends
	...
	protected void commonAssignmentCheck(Tree varTree, ExpressionTree valueExp,
		...
		if (TreeUtils.isFieldAccess(varTree)) {
			...
			AnnotatedTypeMirror xType = atypeFactory.getReceiver(lhs);
	...
	public Void visitMethodInvocation(MethodInvocationTree node, Void p) {
		// TODO: this is a hack, it seems like this could be done at the TypeFactory level.
	...
	public Void visitMethod(MethodTree node, Void p) {
		if (TreeUtils.isConstructor(node)) {
			...
			NonNullFlow nnflow = ((CommitmentAnnotatedTypeFactory<CommitmentChecker>) atypeFactory)
					.getFlow();
			Set<VariableElement> violatingFields = nnflow
					.fieldsViolatingInvariantAfterConstructor(node);
	...
	public Void visitReturn(ReturnTree node, Void p) {
		if (TreeUtils.isConstructor(TreeUtils.enclosingMethod(atypeFactory
			...
			NonNullFlow nnflow = ((CommitmentAnnotatedTypeFactory<CommitmentChecker>) atypeFactory)
					.getFlow();
			Set<VariableElement> violatingFields = nnflow
					.fieldsViolatingInvariantAtReturn(node);
			if (!violatingFields.isEmpty()) {
				System.err
