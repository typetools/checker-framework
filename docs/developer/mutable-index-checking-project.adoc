...
* A `@CannotRemoveFrom` reference to the collection cannot be used to remove elements,
...
If a list is `@UncheckedShrinkable`, then `@IndexFor(list)` is equivalent to `@NonNegative`
and does not guarantee that the value is a valid index for the collection
(in other words, the checker behaves as if all indices are valid for this collection).
