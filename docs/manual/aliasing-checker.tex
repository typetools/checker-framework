...
constructor's body does not create an alias to the constructed object.
...
identifies a formal parameter that is neither leaked nor
returned by the method body.
For example, the formal parameter of the String copy constructor,
\code{String(String s)}, is \code{@NonLeaked} because the body of the constructor
...
each time it is evaluated. In practice, these are only method/constructor calls
with a \code{@Unique} return type. A variable/field is not fresh because it can
produce the same value when evaluated twice.
\item
...
void foo() {
    ...
    // Both sb and sb2 have aliases and therefore have type @MaybeAliased.
}
