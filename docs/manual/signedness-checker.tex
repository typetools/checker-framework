...
surprising for the Signedness type system.  Ordinarily, an unsigned value
may not participate in a division, as shown in
Sections~\ref{signedness-checker-prohibited-operations}
and~\ref{signedness-checker-rationale}.  However, if an \<@Unsigned>
variable is assigned to a constant, then the variable can be treated as
either signed or unsigned as needed.  For example, it can participate in
division.  Note that the result of the division is signed, so you cannot
accidentally assign the division result to an \<@Unsigned> variable.
