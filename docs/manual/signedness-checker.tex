...
\<short>, \<int>, or \<long> expression.  To verify these annotations, also
run the Index Checker.  A \<char> is always non-negative, so these
annotations do not affect a \<char> expression; a \<char> expression has
type \<@SignedPositive> only when range analysis determines that its value is
between 0 and 127.
