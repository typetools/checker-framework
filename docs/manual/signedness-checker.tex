...
or \refqualclass{checker/index/qual}{Positive} annotation, even on an
\<@Unsigned> value:  such a value has type
\refqualclass{checker/signedness/qual}{SignedPositive}, so it may be used
wherever a signed or an unsigned value is expected.  To verify these
annotations, also run the Index Checker.
