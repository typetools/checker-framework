...
This chapter describes what to do if the methodology of
...
or unannotated portions of your own code, because of application
...
\item
  the \code{-Alint} command-line option, which enables/disables optional checks (Section~\ref{alint}),
\item
...
It is lowercase by default, though a checker can choose a different casing.
For
...
safe.  This example uses the Tainting Checker
(Chapter~\ref{tainting-checker}).  Assume that \<expr> has compile-time
...
A non-standard use for assertions is to document facts that a programmer
...
command-line option; see Section~\ref{askipdefs}).
...
refined in the future.  Often, \code{-AskipUses} is more useful.
...
A common scenario for using these options is when you are starting out by
...
A common scenario for using these options is when you want to exclude certain
...
comma-delimited list of check names.  If a check name is preceded by a
...
% LocalWords:  AskipDefs mypackage Makefile PLXCOMP expr AskipFiles upvote
% LocalWords:  TODO AsuppressWarnings AssumeAssertion AonlyUses AonlyDefs
% LocalWords:  ing warningkey redundantNullComparison qual proc Decl astub
...
% LocalWords:  refactored AassumeAssertionsAreEnabled assertNotNull Astubs
...
% LocalWords:  verifyNotNull subcheckers subchecker AonlyFiles NullAway's
% LocalWords:  askipfiles
