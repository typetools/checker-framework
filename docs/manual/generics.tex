...
generic collection.  A qualifier on a use of \<StringBuffer> is treated as
...
\begin{enumerate}
  \item If the receiver type has a qualifier parameter and is annotated with a polymorphic qualifier,
  ...
  \item If the receiver type does not have a qualifier parameter or if it is not annotated with a
...
\subsectionAndLabel{Implicit qualifier parameters by default}{default-has-qualifier-parameter}
...
Writing this on a class indicates it has no implicit qualifier parameter and
