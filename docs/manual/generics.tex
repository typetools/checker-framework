...
\sectionAndLabel{Qualifier polymorphism for methods}{method-qualifier-polymorphism}
...
\sectionAndLabel{Classes with implicit qualifier parameters}{class-qualifier-polymorphism}
...
For example, in Java, a collection is typically written generically, as in
\begin{Verbatim}
class MyCollection<T> { ... }
\end{Verbatim}

This has attractive properties, such as:
\begin{itemize}
  \item argument types depend on the element type, as in
\begin{Verbatim}
class MyCollection<E> {
void add(E arg) { ... }
}
\end{Verbatim}
\item two collections with different element types are not related in the subtype hierarchy, as in
\begin{Verbatim}
MyCollection<@Tainted Character> tBuffer = ...
MyCollection<@Untainted Character> uBuffer = ...
\end{Verbatim}
\end{itemize}

Sometimes, a collection is specialized to a particular Java type, as in a StringBuffer whose elements can
be thought of as characters or strings. Because the element type is hard-coded, the qualifier on that
element type is hard-coded, too. For example,
...
The qualifier on the parameter type of \<append> must be the same as that on the return type of \<toString>.
If we use \<@Tainted>, then the type of all \<StringBuffer#toString> calls will be \<@Tainted> even
if only Objects with type \<@Untainted> are appended. If we use \<@Untainted>, no objects with type
\<@Tainted> may be appended.  It might seem that annotating all the parameter, receiver, and return
types of both \<append> and \<toString> with \<@PolyTainted> would be correct, but the bodies of the
methods would not type check and these annotations lead to the following unsoundness.

\begin{Verbatim}
  @Untainted StringBuffer usb = ...;
  @Tainted Object tainted = ...;
  @Tainted StringBuffer tsb = usb;
  tsb.append(tainted);  // puts tainted value in (alias to) untainted list
\end{Verbatim}

We can fix the problems by providing the StringBuffer class with the two features that generic
collections have.  It analogizes StringBuffer to a generic collection.  The qualifier on the
StringBuffer itself is treated as appearing both on the StringBuffer and on its conceptual type
argument.  That is:\\
\begin{tt}
...
To do this, we add a qualifier parameter to a class by annotating its declaration with \refqualclass{framework/qual}{HasQualifierParameter}.
...
subtype of \<@Tainted StringBuffer>; an attepmt to cast between them (in
...
The default of qualifier for uses of types with a qualifier parameter is the polymorphic qualifier
within the class that declares the type.

\subsectionAndLabel{Using class qualifier parameters in method signatures}{class-qual-param-method}

A polymorphic qualifier in a method signature has two different meanings.
1.) If the receiver type has a qualifier parameter and is annotated with a polymorphic qualifier,
...
2.) If the receiver type does not have a qualifier parameter or if it is not annotated with a
polymorphic annotation, then the polymorphic qualifiers are instantiated as explained in
Section~\ref{method-qualifier-polymorphism}.

In a class with a class qualifier parameter, a formal parameter type should
...
the receiver expression at call sites to \<append>.

\subsectionAndLabel{Using class qualifier parameters in the type of a field}{class-qual-param-field}

In a class with a class qualifier parameter, sometimes the type of a field
