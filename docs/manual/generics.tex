...
arguments that replace \code{T} in \code{MyRegexes<T>} must be subtypes of
\code{@Regex(1) String} and supertypes of \code{@Regex(5) void}.
Since \<void> cannot be used to instantiate a generic class, \<MyRegexes> may
...
\subsectionAndLabel{Overriding a generic method}{generics-override}

When a method overrides a generic method, the type parameter bounds of the
overriding method must contain the type parameter bounds of the overridden
method.  That is, the upper bound of the overriding method must be a supertype
of the upper bound of the overridden method, and the lower bound of the
overriding method must be a subtype of the lower bound of the overridden
method.  The reason is that a client may call the overridden method with any
type argument that is within the bounds of the overridden method, and the
overriding method is what runs, so the overriding method must permit every such
type argument.
...
\begin{Verbatim}
  ...
  class Sub extends Super {
    @Override
    // error: the bounds do not contain those of the overridden method
    <@Nullable T extends @Nullable Object> T pick(T p) { ... }
  }
\end{Verbatim}

The declaration in \code{Sub} is illegal because a client may call
\code{pick} through a reference of type \code{Super}, with the type argument
\code{@NonNull String} that \code{Sub.pick} does not permit:

\begin{Verbatim}
  void client(Super s) {
    @NonNull String result = s.<@NonNull String>pick("x");
  }
\end{Verbatim}

It is legal for the overriding method to state weaker bounds than the
overridden method, because that permits more type arguments rather than fewer.
...
\href{https://docs.oracle.com/javase/specs/jls/se25/html/jls-4.html#jls-4.5.1}{JLS
  \S 4.5.1}.
...
\<MyList2> and \<MyList2a> must be instantiated with a non-null type.
...
\begin{Verbatim}
  T t;
  @Nullable T nble;      // Section "Type annotations on a use of a generic type variable", above,
...
Java's generics.  You use them for similar reasons, such as:
\begin{itemize}
\item
...
Similarly, it is unusual, but permitted, to write just one
...
There is no point in writing just one polymorphic qualifier in a method
...
\end{Verbatim}
Method \<m2b()> can be called with (for example) \code{List<@Nullable
...
without a Java base type) to any class (generic or not).
...
and supply the class of the top qualifier as its argument.
...
A class qualifier parameter changes the rules for instantiating polymorphic
...
\end{Verbatim}
The above \<append> call is illegal because the \<@PolyTainted> qualifier is instantiated to \<@Untainted> and
...
is the same as that on \<myBuffer>.
...
Section~\ref{climb-to-top}). Type refinement determines whether a variable can be
...
Writing this on a class indicates that it has no class qualifier parameter, and
...
has a qualifier parameter. If they were allowed as type arguments for any type parameter, then
