...
\item[\refqualclass{checker/signature/qual}{FieldDescriptor}]
  ...
  for example to refer to other types.  It abbreviates primitives and
        arrays.  It uses internal form (\emph{ClassGetName}s, but with \</> instead of
  \<.>; see
...
\item[\refqualclass{checker/signature/qual}{InternalForm}]
  ...
  the same as the ClassGetName, but with periods (\<.>) replaced by slashes
