...
Casting to \<@BinaryName> from one of the other types adds clutter if it is
not necessary.  Suppose that a method returns a \<@ClassGetName> and
the value will be passed to another method that requires a
\<@ClassGetName>.  Then there is no point in casting to \<@BinaryName> in
between, even if you know the type being represented is not a primitive or
an array.
