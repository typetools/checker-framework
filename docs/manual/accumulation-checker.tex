...
\<value> that defaults to an empty array.  Note that the Checker Framework's
...
``it is not true that \<x> was definitely accumulated'' or, equivalently, ``there is some path on which \<x> was not accumulated''.
...
annotations.
To enforce properties in the code being type-checked, write type rules
