...
\item[\<-AstubWarnIfNotFound>]
  ...
  % written for library version B but an earlier version of the library is on
  % the classpath; a warning is issued for every method in the stub file but
  % not in the earlier version of the library.
...
\item[\<-AstubWarnIfRedundantWithBytecode>]
  ...
  so that entry in the stub file is not necessary.
