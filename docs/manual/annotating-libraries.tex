...
superclass, even though the subtype does not define the method and just
...
can change its annotations.

This feature currently only works for return types.  In the future, it will
be extended to formal parameters and fields.

As an example, consider a type system that tracks whether a value is
cryptographically secure.  Its stub file would contain:
...
\noindent
\<SecureRandom> does not override \<nextInt> --- it only overrides the source of randomness.
