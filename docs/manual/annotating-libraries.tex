...
Ajava files are valid Java source files whose annotations can be read and used
by the Checker Framework. They are similar to stub files, but annotations in
anonymous and local classes are also read and used.
...
If an ajava file has an associated Java file, in order for the Checker Framework
to match the two the ajava file must be located in a subdirectory of one of the
arguments to \code{-Aajava} matching its package name, and it must be named
...
the ajava file's annotations belong to. For example, an ajava file with
...
\begin{itemize}
  ...
  \item The ajava file may have explicit receivers where the source file
    doesn't. If a source file has a method declaration \code{void myMethod()},
    the ajava file might contain \code{void myMethod(MyClass this)}.
