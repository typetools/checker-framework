...
If you are distributing stub files with a checker implementation, the stub files
should appear in the same directory as the checker class, which is a subtype of
\<BaseTypeChecker>.  A \refqualclass{framework/qual}{StubFiles} annotation on
the checker class lists stub files that are always used.  You must also
configure your build system to copy the stub files into the distributed jar for
your checker.  If you are using Gradle, the following build configuration is
sufficient:
\begin{Verbatim}
  sourceSets {
    main {
...
\end{Verbatim}
(If you would prefer to keep resource files separate from your source files, you
can place the stub files under the \<src/main/resources> directory, in a
sub-directory matching the package of your checker class.  In this case, the
default Gradle build configuration is sufficient.) For stub files whose use is
optional (for example, because the behavior is unsound, or unsound except in
certain circumstances), users must supply the \<-Astubs=...> command-line
option.
