...
\begin{enumerate}
\item
Get Typetools version of Java 11 source code
\begin{enumerate}
\item Fork and then clone \href{https://github.com/typetools/jdk}{typetools/jdk}
\item Add annotations to the required classes of JDK.
\item
Don't forget to add \refqualclass{checker/framework/qual}{AnnotatedFor} to the classes
you have annotated with the TypeSystem eg. Use \<@AnnotatedFor("nullness") if a class is annotated with nullness annotations.
\end{enumerate}

\item
Set up the JDK build system (replace \textit{mychecker} with the name of your checker)
\begin{enumerate}
...
\item
Build the annotated JDK\@. (make sure you are in JDK directory before proceeding)
\begin{itemize}
\item
  Configure your environment by running: \<bash configure --disable-warnings-as-errors>
  If any error occurs due to absence of any libraries please install and repeat until this command is executed successfully.
\item
  run \<make images>, then
  Set \<jdkShaHash> to \<`local'> in \<checker/build.gradle>
\end{itemize}
