...
The
ajava file must contain the same source code as the sourec file with the following exceptions:
\begin{itemize}
...
The \code{InsertAjavaAnnotations.java} program takes a directory containing
ajava files and a directory containing Java files and inserts all annotations
from the ajava files into their corresponding Java files. This can be used to
insert the annotations inferred by whole program inference into the source code
of a program, similarly to the
\code{insert-annotations-to-source} script from the Annotation File Utilities
project.

To use the insertion program, run the Java file with the \code{checker.jar} file
in the classpath:
\begin{Verbatim}
  java -cp $CHECKERFRAMEWORK/checker/dist/checker.jar \
