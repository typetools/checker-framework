...
the checker to apply defaults normally for classes with a relevant \<@AnnotatedFor>
...
\noindent
for an appropriate value of \<@Top> and \<@Bottom>.  These choices are
conservative.  They are likely to cause many false-positive type-checking
errors in client code, which will help you to know which library methods
need annotations.  You can then write those library annotations.
...
systems.
You may write \<@AnnotatedFor> on a class that contains no annotations, if
you examined the source code and verified that all appropriate annotations
are present --- that is, if the default is appropriate for every type in
the class.
...
Expression type systems.  It has no effect when running other type systems
