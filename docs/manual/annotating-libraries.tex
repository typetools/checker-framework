...
\item
  It is only a bit more effort to annotate an entire class than to annotate one
  method or field.  In either case it is usually necessary to understand the entire
...
%  client of the library.
  ...
  % This is a strong recommendation, but not a requirement.  @AnnotatedFor
...
See Section~\ref{library-tips-verify}.)
...
argument \<-AuseConservativeDefaultsForUncheckedCode=source,bytecode>.
This has the following effect.
For classes with \<@AnnotatedFor> for the current checker,
the checker applies defaults normally.
For classes without \<@AnnotatedFor> for the current checker,
the checker uses conservative defaults
...
\noindent
in a class \emph{with} a relevant \<@AnnotatedFor> is defaulted normally.

In a class \emph{without} a relevant \<@AnnotatedFor>, it is interpreted as
...
false-positive type-checking errors in client code, which will help you to
know which library methods need annotations.  You can then write those
library annotations.
...
systems.
You may write \<@AnnotatedFor> on a class that contains no explicit
annotations from the given type system, if the defaulting rules already
give appropriate qualifiers for every type in the class.

For example, \<@AnnotatedFor(\ttlcb"nullness", "regex"\ttrcb)> means that
the programmer has written annotations for the Nullness and Regular
Expression type systems.
For any type system other than Nullness or Regex, that \<@AnnotatedFor>
annotation has no effect --- the checker treats the class as
unannotated for that type system.
...
creating stub classes and creating an annotated library.
...
\item
  An unannotated type variable in a stub file is used instead of
...
You may write the name of a formal parameter type in fully-qualified,
partially-qualified, or simple form, and you may omit its type arguments.
...
\item{\textbf{Method declarations:}}
  ...
  its annotations from the library's \<.class> files.  If you are using a stub class, then
...
\item{\textbf{Import statements:}}
  ...
  An import applies to the whole file.
...
\item
  ...
  directory in a subdirectory matching the package of your checker class,
...
\noindent
by removing the method \<enableForegroundNdefPush(...)> from
...
Ajava files are primarily read and written by tools, such as whole-program type
inference (see Section~\ref{whole-program-inference}).  (However,
programmers are permitted to write \<.ajava> files.)
This section about
...
If there's an annotation in both a source file and the corresponding ajava
file, the Checker Framework uses the greatest lower bound, as with
...
files is specified ambiguously if one of their fully-qualified names
...
would be considered ambiguous and an \<ambiguous.ajava> warning would be issued.
Files found under a directory are always unambiguous, because their file structure
must match their package name; the solution to an \<ambiguous.ajava> warning is to
...
\begin{description}
\item \<-verbose>
  Outputs information about compile phases --- when the compiler
...
% LocalWords: enableForegroundNdefPush BCEL getopt ajava html drawImage
