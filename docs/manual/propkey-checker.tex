...
An access without a valid key does not fail in an obvious way:
\code{ResourceBundle.getString} throws a \code{MissingResourceException}, and
\code{Properties.getProperty} returns \code{null} or the default value supplied
by the caller.  This can lead to a \code{NullPointerException} or
hard-to-trace behavior.
The Property File Checker (Section \refwithpage{genpropkey-checker}) ensures
that the keys used are found in the corresponding property file or resource
bundle.
...
It is a subtype of \refqualclass{checker/i18n/qual}{UnknownLocalized},
...
This annotation is a specialization of the \code{@PropertyKey} annotation, which
gets checked by the general Property Key Checker.
It is a subtype of \refqualclass{checker/i18n/qual}{UnknownLocalizableKey},
which is the default and which programmers do not need to write.
\end{description}
...
These keys are replaced by localized strings for user-visible error messages.
...
for example, class \code{org.checkerframework.framework.source.DiagMessage}
uses \code{@CompilerMessageKey} in its constructor and in method \code{error}.
