...
Such a default takes precedence over rule 6, that is, over a user-written
\<@DefaultQualifier>, and over a default that is registered for an enclosing element.
A default for \<OTHERWISE> applies only if no default for a more concrete location applies,
whichever element each default is written or registered on.
