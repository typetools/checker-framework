...
The \refqualclass{dataflow/qual}{SideEffectsOnly} annotation
specifies a superset of the expressions that the method might side effect.
Expressions that are not specified as annotation values of
\refqualclass{dataflow/qual}{SideEffectsOnly} have their dataflow facts
unaffected.
If a method has neither \refqualclass{dataflow/qual}{SideEffectsOnly}
nor \refqualclass{dataflow/qual}{SideEffectFree} annotation, then the Checker Framework
assumes that the method side effects all expressions (fields and arguments).
...
A method annotated with \<@SideEffectsOnly(\{\})> (i.e without any annotation value)
is equivalent to annotating it with \<@SideEffectFree>.
