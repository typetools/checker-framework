...
(\<@SideEffectsOnly("\#1")> permits modifying \<\#1.f.g>), or if the method body assigns
it to or from a listed expression.  Assigning to a local variable is never a side effect,
because it is not visible outside the method.
...
the object being constructed; list \<this> to permit assigning to the new object's own
fields.  At a \<new> expression, the expressions that are reached through \<this> are
