...
\item
  ...
  as exactly as if the programmer had written them explicitly.
...
The examples in this section use the usual type qualifier hierarchy \code{@A :> @B :> @C>}.
...
This means that \<@B MyClass> is an invalid type.

If no annotation is present on a type declaration, then the bound is top. This can be changed by
overriding a method in AnnotatedTypeFactory.

There are two exceptions.
\begin{itemize}
\item
  An expression can have a super-qualifier of the class annotation; that is, some expression could
...
\item
  ...
  (for most type systems) being passed to polymorphic routines such as println or System.identityHashCode.
\end{itemize}

These operations might type-refine it.  If a user wishes to annotate a method that does type refinement,
its formal parameter must be of illegal type \<@A MyClass>, which requires a warning suppression.

If we forbid expressions and local variables from having types forbidden by the class annotation,
...
\item
  The qualifier with a meta-annotation \<@DefaultFor(types = MyClass.class)>.
\item
...
\subsectionAndLabel{Annotations on type declarations}{annotations-on-type-declarations}

% TODO: use a more realistic example.

Writing an annotation on a type (class or interface) declaration makes that annotation implicit
for all uses of the type (see Section~\ref{effective-qualifier}).
...
%%  LocalWords:  Signedness MonotonicNonNull
