...
type.
As a result, you usually do not need to write type qualifiers on local variables.
...
\begin{itemize}
\item
starting at the time that it is
assigned a value, a run-time check is performed (e.g., via an assertion or
\code{if} statement), or a method establishes a postcondition (e.g., as
expressed by \refqualclass{checker/nullness/qual}{EnsuresNonNull}).
\item
...
since you do not need to annotate local variables.
...
By default, all checkers automatically incorporate type refinement.
...
(Type refinement does not change them, because doing so does not produce a
...
\subsubsectionAndLabel{Fields and flow-sensitive analysis}{type-refinement-fields}
...
%  LocalWords:  computeValue AsuggestPureMethods Instanceof arg myInt
