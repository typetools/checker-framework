...
that a field's type in the class on which this annotation is written is a
subtype of its declared type. The field must be declared in a superclass and
...
@FieldInvariant(qualifier = NonNull.class, field = "field")
class Sub {
...
that \<fieldA> and \<fieldB> are both non-null. A field invariant annotation
can also apply different qualifiers to fields. For example,
...
This annotation is inherited so, if a superclass is annotated with
...
Whether the field actually has the type specified in the invariant is not checked, but rather trusted.
