...
A type system designer can also register a default for a particular element
and for the elements that it encloses, by calling
\<QualifierDefaults.addElementDefault> (Section~\ref{creating-typesystem-defaults}).
At the same location, such a default takes precedence over rule 6, that is, over a user-written
\<@DefaultQualifier>, and over a default that is registered for an enclosing element.
A default for a specific location, such as \<RETURN>, takes precedence over a default for
\<OTHERWISE> or \<ALL>, whichever element each default is written or registered on.
