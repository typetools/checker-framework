...
checked by a checker.  This paragraph and
Section~\ref{defaults-classfile} are about reading an already-compiled
class file (say, \<A.class>).
%
If the checker ran during the compilation that created \<A.class>,
then \<A.class> contains the explicit or defaulted
annotations from the source code and no defaulting is necessary.
%
If the checker did not  run during the compilation that created \<A.class>,
then \<A.class> file contains only explicit annotations and
...
Java types to be treated as if they had a default annotation.
(Defaulting never applies to uses of type varables, even if they do not
...
\item
  Use \refqualclass{common/subtyping/qual}{Unqualified}, which the framework
