...
Defaulting does not apply to type declarations; see
Section~\ref{nnotations-on-type-declarations} for an explanation of writting no
qualifier in this location means.
...
Class declaration locations are not defaulted.  If no annotation is written on
a class declaration, then the upper bound for all uses of the type is top.  In
other words, it is legal to annotate the type with any annotation.  However,
writing no annotation is not equivalent to writing the top type qualifier,
because if no annotation is written, the top qualifier is not implicitly added
to all uses of the type.
% A type system designer can choose a different qualifier than top.
% The implementation gives the class declaration a type qualifier, because every type must have a type qualifier.  But elsewhere, the implementation checks whether the type was defaulted or explicitly written.
