...
string \<"(foo"> is given the type \<@PartialRegex String> because it is not
...
In particular, programmers rarely write qualifiers in method bodies (except on
...
rules 6 and 7 apply. If rule 7 applies, it makes the type of local variables top so they can be refined.
...
for uses of types, using the \<types> element, or type kinds, using the \<typeKinds> element.
...
parameters to some other type T (instead of the top type); then the Checker
...
user passes a secret integer and the JDK throws an ArithmeticException
...
\noindent
As mentioned above, exception parameters are always non-null, so
...
be automatically inherited by subclasses that override it.

The alternate design would reduce annotation effort.

However, the alternate design would reduce program comprehensibility.
...
Note that this copying of annotations for a wildcard's bounds applies only to
...
Note that the upper bound of the wildcard \code{?~extends Object} is defaulted to
...
the compiler.  When the compiler reads a \<.class> file, different
defaulting rules apply.  The rules are the same regardless of whether the
\<.class> file is from your own codebase or from a library, including the
JDK\@.

If the \<.class> file was compiled \emph{using} the checker (that is,
if the checker was run during the compiler execution that created
the \<.class> file),
...
%% True, but it distracts from the point of this section.
% When you are performing pluggable type-checking,
% it is best if the compiler only reads such \<.class> files.
...
The rest of this section discusses the situation when the \<.class> file
was \emph{not} compiled using the checker (that is, the checker was not run
during the compiler execution that created the \<.class> file).
The \<.class> file contains only the type qualifiers
that the programmer wrote explicitly --- possibly none.
(Furthermore, there is no guarantee
that these qualifiers are correct, since they have not been checked.)
Each checker decides what qualifier to use
where the programmer did not write an annotation.

% In the future, we would like to change the default behavior from
%  -AuseConservativeDefaultsForUncheckedCode=-bytecode (as it is currently)
%  to -AuseConservativeDefaultsForUncheckedCode=bytecode.

\begin{itemize}
\item With \<-AuseConservativeDefaultsForUncheckedCode=-bytecode>:
(This is also the behavior if
\<-AuseConservativeDefaultsForUncheckedCode> is not supplied or is supplied
without ``bytecode'' in its argument.)
...
\noindent
for an appropriate value of \<@Default>.  (The default might be different for
return types and parameter types, depending on how the checker is written.)

These defaulting rules are unsafe:  the default might be different than
what the library author intended, so the Checker Framework may not warn
about some misuses of the library.  (This includes both misuse by passing
in a bad value and misuse by using a returned value incorrectly.)  However,
this unsafe default can be useful because it permits you to focus on errors
within the codebase before you have annotated the libraries it uses.
...
\noindent
where \<@MyTopQualifier> is the top qualifier in the hierarchy and
\<@MyBottomQualifier> is the bottom qualifier in the hierarchy.
...
There is one case in which these choices are not conservative.  Fields
default to the top qualifier, so any assignment into a field is permitted.
If an unannotated library has a mutable public field (which would be poor
code style), the Checker Framework won't warn about incorrect assignments
to the field.
...
than --- that is, a subtype of --- its declared or defaulted type (Section~\ref{defaults}).
...
is never treated as possibly-null, and such an assignment is always illegal.
...
subtype, as explained in Section~\ref{covariant-type-parameters} and
...
\item
  ...
  \sectionpageref{initialization-checker})
\item
...
After a call to a \<@SideEffectsOnly> method, a checker discards what it knows about an
expression \<e> if \<e> contains a listed expression:  modifying \<x> can change \<x.f>,
but not the other way around.  If \<e> is a call to a \<@Pure> method, then a checker also
discards what it knows when a listed expression is reached through the call's receiver or
one of its arguments.  A \<@Pure> method's result depends on the state that the method
reads, which no annotation declares, so a checker approximates that state by what is
reachable from the call's receiver and arguments.  For example, a call to a
\<@SideEffectsOnly("x.f")> method discards what is known about \<x.getF()>, but not what is
known about \<y.getF()>.

Each listed expression is viewpoint-adapted to the call site.  If the adaptation yields an
expression that the checker cannot represent --- as when an argument is a conditional
expression or a cast --- then the checker does not know what the call modifies, so it
discards every refinement, exactly as for a method with no side-effect annotation.
...
indicates that a given method never returns.  This can enable
...
with an empty list of expressions) is illegal; write \<@SideEffectFree> instead.
...
Like the other purity annotations, \<@SideEffectsOnly> is inherited by overriding
methods, so an overriding method that is not annotated is checked against the
overridden method's \<@SideEffectsOnly> annotation.  In the annotation's arguments,
\<this> and \<\#1>-style parameter references are interpreted in the overriding
method.  An overriding method may list fewer expressions than the overridden method
does, or an expression that is reached through one that the overridden method lists
(an override of a \<@SideEffectsOnly("\#1")> method may be annotated
\<@SideEffectsOnly("\#1.f")>).

An overriding method may be annotated \<@SideEffectFree>, which is a
stronger guarantee than any \<@SideEffectsOnly> annotation; that is not a conflict.
A method reference is checked in the same way, against the \<@SideEffectsOnly> annotation
of the functional interface method that it implements.  The two methods' parameters do not
necessarily correspond positionally:  for an unbound reference such as \<Collection::clear>,
the interface method's first parameter is the referenced method's receiver, and the
interface method's parameter \<\#(i+1)> is the referenced method's parameter \<\#i>.  For a
bound reference such as \<myList::clear>, no expression at the interface method's
declaration denotes the bound receiver, so the reference does not satisfy any
\<@SideEffectsOnly> annotation if the referenced method modifies \<this>.


\subsubsectionAndLabel{Checking \<@SideEffectsOnly>}{side-effects-only-checking}

Under \<-AcheckPurityAnnotations>, the Checker Framework verifies that the body of a
\<@SideEffectsOnly> method modifies at most the listed expressions.  An expression is
covered by the annotation if it is listed, if it is reached through a listed expression
(\<@SideEffectsOnly("\#1")> permits modifying \<\#1.f.g> and, if \<\#1> is an array,
\<\#1[i]>), or if the method body assigns
it to or from a listed expression.  Assigning to a local variable is never a side effect,
because it is not visible outside the method.  Because \<super> and \<this> denote the same
object, \<@SideEffectsOnly("this")> also covers \<super.f> and a call to \<super.m()>.

The result of a method call is reached through the call's receiver only if the method is
\<@SideEffectFree>.  \<@SideEffectsOnly("\#1")> covers \<\#1.subList(0, 1)>, because
\<List.subList> is \<@SideEffectFree> and is therefore assumed to return a part of its
receiver; it does not cover \<\#1.m()> for a method \<m> that is not \<@SideEffectFree>,
because such a method may return an object that its receiver does not reach.

Each expression in the annotation must denote the same location every time it is
evaluated:  it must be a variable, a field access, an array access, a literal, a class
name, or a call to a \refqualclass{dataflow/qual}{Pure} method, recursively.  A
\<@Pure> method returns the same value every time it is called with the same arguments, so a
call to one denotes the same location every time it is evaluated, so long as its receiver
and its arguments do.  An expression such as \<\#1.getList()>, where \<getList> is not
\<@Pure>, is rejected with \<purity.unstable.sideeffectsonly>, because two evaluations of
it may yield unrelated values, so no method body could satisfy it --- not even one whose
only side effect is written identically.  So is \<\#1.get(\#1.index())>, where \<get> is
\<@Pure> but \<index> is not.

Modifying an object that the method itself created is not a side effect that is visible to
the caller, because the object did not exist before the call.  The checker recognizes this
for a local variable that is assigned only \<new> expressions.  It permits modifying that
variable's value and assigning to the object's own fields and array elements, but not
assigning to a field of a field, such as \<fresh.f.g>, because \<fresh.f> may be an object
that existed before the call.  If the method stores the fresh object into pre-existing
state, that store is itself a side effect, which is reported unless the annotation covers
it.

The body of a lambda, and the body of a method of a class that is declared within the
method being checked, are not side effects of that method:  they run when the lambda or
the method is invoked, and each such invocation is checked where it appears.

A lambda's body is instead checked against the \<@SideEffectsOnly> annotation of the
functional interface method that the lambda implements.  (A method of a class declared
within a method needs no such special treatment:  it is a method declaration, and it
inherits the annotation of the method it overrides.)  In the interface method's annotation,
\<\#1>-style parameter references denote the lambda's own parameters, and \<this> is
ignored:  it denotes the object that evaluating the lambda expression creates, which has no
state that the body can modify.  A lambda whose interface method is \<@SideEffectFree> or
\<@Pure> is not checked.  An expression in the interface method's annotation that does not
denote the same location every time it is evaluated is rejected at the lambda, with
\<purity.unstable.sideeffectsonly>, just as it is at a method declaration; otherwise the
lambda's every modification would be reported.  An expression that cannot be parsed is
likewise reported at the lambda, with \<flowexpr.parse.error>, because the annotation then
does not say what the lambda's body may modify.  Both are reported at the lambda as well as
at the interface method's declaration, because that declaration may be in an annotation
file or in another compilation unit, where no error would be issued.

An argument of a call whose type is a functional interface is a possible callback:  the
callee might invoke it before returning, so whatever the invocation modifies is a side
effect of the calling method.  When the functional interface method has a side-effect
annotation, the callee's own annotation accounts for the invocation, just as it does for
every other call the callee makes.  Otherwise --- as for \<Predicate.test>, which has no
side-effect annotation --- a lambda argument's body is checked as part of the calling
method, and any other argument, such as a variable that holds a lambda, reports
\<purity.unknown.sideeffectsonly>, because its body is not at hand to be checked.

Every method that a \<@SideEffectsOnly> method calls must itself be annotated
\<@Pure>, \<@SideEffectFree>, or \<@SideEffectsOnly>.  A call to a method with none of
these reports \<purity.unknown.sideeffectsonly>, because such a method may modify
arbitrary state, including static state and other state that is not reachable from its
receiver and arguments.  When the callee is annotated \<@SideEffectsOnly>, it is treated
as modifying exactly the expressions in its own annotation, viewpoint-adapted to the call
site --- not its receiver and arguments.

The same holds for a constructor invocation --- a \<new> expression, or a \<this(...)> or
\<super(...)> call.  Because most constructors, including most of those in the JDK, have no
side-effect annotation, a \<@SideEffectsOnly> method that creates an object often reports
\<purity.unknown.sideeffectsonly>.

Because \<AutoCloseable.close()> has no side-effect
annotation --- closing a resource can modify arbitrary state --- a try-with-resources
statement whose resource is of a JDK type reports \<purity.unknown.sideeffectsonly>.

On a constructor, \<@SideEffectsOnly> constrains what the constructor modifies besides
the object being constructed.  Assigning to the new object's own fields is always
permitted and need not be listed, because the object did not exist before the call;
writing \<this> in the annotation is legal but has no additional effect.
At a \<new> expression, \<this> in the constructor's annotation is ignored, because the
object being constructed did not exist before the call and therefore no expression at the
call site refers to it.  A larger expression that contains \<this>, such as \<this.f>, is
not ignored:  its value may be an object that existed before the call, as it does for a
constructor whose body contains \<this.f = p;> where \<p> is a formal parameter.  No
expression at the call site denotes such a value either, so the \<new> expression reports
\<purity.unknown.sideeffectsonly>.  A constructor's \<@SideEffectsOnly>
annotation does not yet affect type refinement at \<new> expressions.

Calling a constructor runs more than its body.  Unless the constructor delegates to
another constructor of the same class with \<this(...)>, the class's instance field
initializers and instance initializer blocks run first; and unless the constructor
contains an explicit constructor call of either kind, the superclass's no-argument
constructor runs before those.  All of that code is checked against the constructor's
\<@SideEffectsOnly> annotation.  In particular, if the superclass's no-argument
constructor has no side-effect annotation, then the constructor reports
\<purity.unknown.sideeffectsonly>, just as an explicit \<super()> call would.

Checking of \<@SideEffectsOnly> is unsound in the presence of aliasing.  The checker
reasons syntactically, and treats two expressions as possibly aliased only when an
assignment of a reference relating them appears in the method body.  Assigning a literal,
such as \<null>, relates nothing, because a literal is not a reference to an object that
other code can modify.  Two evaluations of an expression that computes a new value, such
as a call to a method that is not \<@Pure> or an array creation, are treated as unrelated,
even when the two expressions are written identically.  Conversely, two evaluations of a
call to a \<@Pure> method with stable arguments are treated as the same location; this is
unsound if the method body changes state that the callee reads, because \<@Pure> promises
the same return value only in the same environment.  Treating the result of a
\<@SideEffectFree> call as reached through the call's receiver is unsound for a method that
returns state the receiver does not reach, such as a static field; no annotation expresses
the difference between that and a view such as \<List.subList>.  The method body is scanned once,
in source order, so an alias is visible only after the assignment that creates it; and
alias sets grow but are never split, even when a later assignment makes the aliasing
impossible.  A method that modifies a listed expression through an alias is therefore
accepted:
...
\noindent
Because the scan is in source order, moving the modification before the assignment that
creates the alias causes it to be reported:

\begin{Verbatim}
  @SideEffectsOnly("#1")
  void m(List<String> a, List<String> b) {
    List<String> t = b;
    t.add("x");   // reported:  t is not yet known to be aliased to a
    t = a;
  }
\end{Verbatim}
...
\subsectionAndLabel{The \<var> keyword}{type-refinement-var}
...
annotation, and those fields' annotations must be subtypes of (or equal to) the
...
\<Kangaroo>) might have a field \<pouchSize> indicating the size of the animal's
...
% LocalWords:  actuals toArray CollectionToArrayHeuristics nn PartialRegex
...
% LocalWords:  fenum i18n RequiresQualifier EnsuresQualifier ClassName sql
...
% LocalWords:  EnsuresKeyFor EnsuresKeyForIf ArithmeticException
