...
expresses the calling obligations of the elements of an array. Rarely written, most common use case is to specify the obligations of an \<@OwningArray> method parameter. See Section~\ref{resource-leak-arrays-mcoe-cmoe}.
...
on a method in your code that fulfills a must-call-on-elements obligation of one of its array parameters or of an \<@OwningArray> field.
...
\item[\refqualclass{checker/mustcallonelements/qual}{OwningArray}]
  expresses ownership over an array of resources. Everything not explicitly annotated \<@OwningArray> is not owning and its elements may not be assigned to objects with calling obligations.
  For more details, see Section~\ref{resource-leak-arrays}.
...
To understand how the checker handles arrays, let us first remind ourselves what happens if we assign resources with non-empty must-call type to an array:
...
The assignment throws an error, because the scope of the constructed Socket ends, without its must-call obligations having been fulfilled.
...
We need a way for the array to take on this obligation for the resource. This functionality is jointly provided by the \<MustCallOnElements> and \<CalledMethodsOnElements> subcheckers.
...
\noindent However, the following is allowed:

\begin{verbatim}
@OwningArray Socket[] s = new Socket[n]; // initial assignment
s = new Socket[n]; // no open obligations for s, hence this is allowed
\end{verbatim}

\subsectionAndLabel{Aliasing of \<@OwningArray> references}{resource-leak-arrays-aliasing}
We've seen in the previous Section~\ref{resource-leak-arrays-declaration} that an \<@OwningArray> reference may not alias, i.e. be assigned to, another \<@OwningArray> reference. However, it is allowed for a non-\<@OwningArray> reference to be assigned to, and alias, an \<@OwningArray> reference:

\begin{verbatim}
@OwningArray Socket[] s_owning = new Socket[n];
Socket[] s_alias = s_owning; // this assignment is allowed
\end{verbatim}

Such aliases are read-only. They may not assign their elements, since that would make it impossible to fulfill the obligations of overwritten elements.

\subsectionAndLabel{Must-call and called-methods for arrays}{resource-leak-arrays-mcoe-cmoe}
Must-call-on-elements and called-methods-on-elements provide similar semantics for arrays as must-call and called-methods do for objects.
...
\subsectionAndLabel{Directly fulfilling calling obligations on \<@OwningArray> arrays}{resource-leak-array-fulfilling-obligations}
...
\subsectionAndLabel{\<@OwningArray> method parameters }{resource-leak-arrays-parameters}
When \<@OwningArray> is written on a method parameter, the default of the parameter is \<@MustCallOnElements(\textit{obligations})>, where \textit{obligations} is the set of \<@MustCall> obligations of the component type. It is allowed to put any manual \<@MustCallOnElements> annotation to override this default however.
...
To call a method with an \<@OwningArray> parameter, the argument must also be \<@OwningArray>. The called method only borrows the ownership of the \<@OwningArray> parameter, since the \<@OwningArray> parameter cannot leave the method scope and its obligations have to be fulfilled by the time of method exit. Thus, not only can the caller keep the ownership for the passed argument, but due to the calling guarantees, the call also resets the \<@MustCallOnElements> type to an empty value and add the methods guaranteed to have been called to the \<CalledMethodsOnElements> type:
...
After calling \texttt{m1}, \textit{arr} behaves like a freshly allocated array and its elements may be reassigned.

\subsectionAndLabel{\<@OwningArray> fields}{resource-leak-arrays-fields}
...
\subsectionAndLabel{\<@OwningArray> constructor parameters}{resource-leak-arrays-constructor}
