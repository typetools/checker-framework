...
\sectionAndLabel{Errors about unknown must call obligations of generic types}{resource-leak-generic-unknown}
...
new \<@MustCall> annotation. Another common, similar situation is a local
variable whose type is a type variable, and whose initializer also has a generic type:
local variables are owning by default, so it might be necessary to write \<@MustCall({})>
on the initializer. For example, in the Checker Framework itself this annotation is necessary
to avoid a false positive error at a call site:
\begin{verbatim}
