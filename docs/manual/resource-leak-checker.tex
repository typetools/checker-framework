...
The Resource Leak Checker issues a warning about every assignment to a
\textbf{static field}; such an assignment does not satisfy a must-call
obligation.  As an exception, the \<-ApermitStaticOwning> suppresses
warnings related to static owning fields.  This can help in checking legacy
code.  It permits only a small number of resource leaks, related to the
number of such fields and assignments to them.  When those fields are used
throughout execution, until the program exits, there is no good place to
dispose of them.
