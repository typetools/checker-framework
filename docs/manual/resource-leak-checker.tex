...
The Resource Leak Checker guarantees that every resource is closed rather
than leaked.  Examples of resources are a channel, executor,
\<ExecutionControl>, file, \<FileLock>, \<Formatter>, reader,
\<Scanner>, socket, stream, writer, etc.

More precisely, the Resource Leak Checker guarantees that the program
fulfills every object's must-call obligations before the object is
de-allocated.
...
the Called Methods Checker, plus five others:
...
\item[\<-AenableReturnsReceiverForRlc>]
  Enables the Returns Receiver Checker (Chapter~\ref{returns-receiver-checker})
  ...
  because it has a significant type-checking cost but provides little benefit in the common case,
...
(and this fact must be expressed via an \<@EnsuresCalledMethods> annotation on \<d()>),
...
obligations satisfied on all paths in (only) the destructor, and will issue a \<contracts.exceptional.postcondition>
...
checking legacy code.  It permits only a small number of resources to be retained
...
\item
  Creates a fresh obligation to check, as if \emph{expr} were assigned to a newly-allocated
...
present before the call. Intuitively, calling a \<@CreatesMustCallFor> method
``resets'' the obligations of the target expression, so whether they were satisfied before
the call or not is irrelevant.

If a \<@CreatesMustCallFor>
method \emph{n} is invoked within a method \emph{m} that has a \<@CreatesMustCallFor> annotation,
...
treats calls to a \<@CreatesMustCallFor> method like \<reconnect()> as if the call might
...
exceptions, use either of these, which have the same effect:
...
Both of these ignore \<java.lang.Error> (and all its subclasses) as well as
...
\myurl{https://homes.cs.washington.edu/~mernst/pubs/resource-leak-esecfse2021.pdf})
