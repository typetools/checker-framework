...
Consider the following example of a safe use of a \<Socket>, in which the comments indicate the
inferrred Must Call and Called Methods types for \<s> at the program points where they appear:
\begin{verbatim}
Socket s = null;
...
At point 1, <s>'s types are the types of \<null>:
...
do have a must-call obligation (\<@MustCall("close")>)
and can have methods called on them (\<@CalledMethods(\{\})>).
...
At point 5, its type becomes the least upper bound of the types at points 3
and 4.

At point 6, the Resource Leak Checker determines that \<s> goes out of scope
and therefore compares the inferred Must Call and Called Methods types.
