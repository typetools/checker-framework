...
Without the \<@MustCall("close")> annotation, the checker is unable to
determine \<value>'s must call obligation, and so conservatively issues an
error. If you encounter such an error, you can almost always resolve it by
adding a new
\<@MustCall> annotation.
...
where the type argument itself isn't \<@MustCall(\{\})>. A programmer who
