...
when whole program inference (WPI) is enabled, as described in the paper
``Inference of Resource Management Specifications''~\cite{ShadabGTEKLLS2023}
(OOPSLA 2023, \myurl{https://homes.cs.washington.edu/~mernst/pubs/resource-inference-oopsla2023-abstract.html}).
The algorithm is enabled automatically when WPI is run as described in
Section \ref{whole-program-inference}; no special settings are required.
