...
type system, follow these steps.
...
The resulting \<.class> files should be on the same path (the classpath or
...
\begin{alltt}
  javac -processorpath \textit{myqualpath} -processor org.checkerframework.common.subtyping.SubtypingChecker \ttbs
...
which is easy with an annotation but disruptive when changing the Java type.
...
particular case of units of measurement, such as kilometers versus miles.
...
achieve the same effect using the Subtyping Checker).  The Fake Enum Checker forbids
...
For example, if \<@NotMyType> is the default, this code does not type-check:
