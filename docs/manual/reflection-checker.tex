...
If the estimate of invoked methods is large, the resulting types may be no better than the
...
\item
  ...
  and
\item
  the reflectively invoked methods/constructors are on the classpath at
  compile time.
\end{enumerate}
...
Value Checker (Chapter~\ref{constant-value-checker}) in that their
...
\[\forall \textrm{ indexes } i \; \exists \textrm{ an index } j:  CA[i] = CB[j], MA[i] = MB[j], \textrm{ and } PA[i] = PB[j]\]
