...
// TODO: clarify "up to equality":  explain whether it is with respect to == or .equals.
...
TODO: The below seems incorrect.  Is it necessary on \<@PolyDet int
identity(@PolyDet int)>?  Do you only mean if \<@PolyDet> is applied to a
collection somewhere in the signature?

Since the type \<@OrderNonDet} is invalid for non-collections, any return
type that isn't a collection should be annotated as \<@PolyDet("up")> or
\<@PolyDet("down")>.
...
is \<@PolyDet>.  This permits all possible arguments to the method.  As
an exception, the return type of a static method without paramaters is
\<@Det>.
