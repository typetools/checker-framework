...
\item \refqualclass{framework/qual}{PolymorphicQualifier} denotes that a qualifier is a
  ...
  Section~\ref{method-qualifier-polymorphism}.  A polymorphic qualifier needs
...
To generate a String graph, remove \<output/> and \<-pdf> but add \<-string>.
For example (with JDK 8):
...
For more information, run \refclass{dataflow/cfg}{CFGVisualizeLauncher} with
