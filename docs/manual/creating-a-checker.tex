...
\newcommand{\TreeAPIBase}{https://docs.oracle.com/en/java/javase/25/docs/api/jdk.compiler/com/sun/source}
\newcommand{\refTreeclass}[2]{\href{\TreeAPIBase{}/#1/#2.html?is-external=true}{\<#2>}}
\newcommand{\ModelAPIBase}{https://docs.oracle.com/en/java/javase/25/docs/api/java.compiler/javax/lang/model}
...
\item
  ...
  A problem with this strategy is that you can get tangled up if you don't fully understand
  ...
  (If you are going to copy a checker despite the dangers, one good choice to copy and modify
...
\label{creating-tips-write-manual}
  ...
  If your checker is in a clone of the Checker Framework, don't forget to
  add a \LaTeX\ \<\textbackslash{}include> directive to \<manual.tex>.
...
\end{Verbatim}
  ...
  If you are working in a clone of the Checker Framework, put the above
  test case in file \<checker/tests/\emph{mycheckername}/TestHierarchy.java>.

  Type-check your test files using the Subtyping Checker
  (\chapterpageref{subtyping-checker}), using a command like

\begin{small}
\begin{Verbatim}
javacheck \
  -processor org.checkerframework.common.subtyping.SubtypingChecker \
  -Aquals=org.checkerframework.checker.mycheckername.qual.MyTopQual,org.checkerframework.checker.mycheckername.qual.MyBottomQual \
  TestHierarchy.java
\end{Verbatim}
\end{small}
...
\end{Verbatim}
  ...
  If the default qualifier is not the top qualifier, you may see \<warning:
  (inconsistent.constructor.type)>.  If the checker should never issue such
  an error (because a newly-constructed object can be correctly viewed as
  any type at all), then copy \<TaintingVisitor.java> into your checker,
  changing ``Tainting''.
...
\label{creating-tips-test-infrastructure}
  ...
  If there are errors in the ``all-systems'' tests, don't worry about them
  until after you have annotated the JDK\@.  However, specifying
  \<@RelevantJavaTypes> (Section~\ref{creating-relevant-java-types}) may
  eliminate some of those errors, especially if the default qualifier is
  not the top qualifier.

\item
  Annotate parts of the JDK, if relevant
  (Section~\ref{creating-a-checker-annotated-jdk}).
  You will clone the \href{https://github.com/typetools/jdk}{Checker
  Framework JDK}, naming it \<jdk> in a sibling directory of
  \<checker-framework> (that is, they have the same parent).
  Most changes will consist of adding an \<import> statement, plus uses of
  qualifiers, in Java files under \<src/java.base/share/classes/>.
  Also follow the instructions in section ``Qualifier definitions'' of file
  \<README.md>.
  ...
  If there are errors in the Checker Framework's ``all-systems'' tests, you can
  suppress them if they are expected. See \<framework/tests/all-systems/README>
...
\item
  ...
  Some type systems need JDK annotations but don't have any type introduction rules.
...
\item
  ...
  Dataflow refinement rules are only possible for a type system that
  supports a run-time test (Section~\ref{type-refinement-runtime-tests}).
...
have the value \<ElementType.TYPE\_USE>, and optionally the additional value
...
As mentioned in Section~\ref{creating-parts-of-a-checker}, the files for that \<qual> package can be
...
In source code, declaration annotations are written on their own line, and
type qualifiers are written on the same line as the type they apply to.
Section~\ref{faq-type-annotation-formatting} explains how to configure
tools to format type annotations properly.
...
relationship but taking qualifiers into consideration.  Some type
...
declaratively and procedurally, as well.  To do so declaratively, after the
declaration of the element in the qualifier file, append the string
\<default \emph{value}>, where \emph{value} is the value you want to be the
default.  For instance, \code{int value() default 0;} would make
\code{value} default to zero. Alternatively, the procedural method
described above can be used.

The default qualifier applies to most, but not all, unannotated types.
Section~\ref{climb-to-top} describes
other defaulting rules that are automatically added to every checker.
Also, Section~\ref{defaults}
...
written on those types, their subtypes, and their supertypes.
...
If a user writes an annotation on an irrelevant type, the Checker Framework
issues an \<anno.on.irrelevant> error.

Irrelevant types are ordinarily defaulted to the top annotation.
A checker designer may change the default, and it may differ from type to type.  For
...
\subsubsectionAndLabel{Relevant types and supertypes}{creating-relevant-java-types-supertypes}

If a type is relevant, so are all its supertypes including \<Object>.
This prevents qualifiers from being lost via an assignment to a supertype.
...
bounds.  Also, the definition of the bottom qualifier should be
meta-annotated with:
...
must have one element, \<value>, that is an
...
and field declarations at uses of those methods, constructors, and fields.  For example, given the
following class
...
named \<FooChecker>, \<FooVisitor>, and \<FooAnnotatedTypeFactory>.
...
The \<messages.properties> file only needs to mention the new messages that
the checker defines.
...
By default, a checker supports all type annotations in a
subdirectory named \<qual> that is in the same directory as the
...
subdirectory, or annotations that have other \<ElementType> values,
...
checker that has its own qualifiers), and if you are using the Buck build
tool, whose class loader cannot find the qualifier directory.
...
(see \sectionpageref{i18n-checker}), which consists of
...
\refmethodterse{framework/source}{SourceChecker}{getOptions}{()} method in the
...
\refmethodterse{framework/source}{SourceChecker}{getOptions}{()} as follows:
...
\href{https://docs.oracle.com/en/java/javase/25/docs/api/jdk.compiler/module-summary.html}{jdk.compiler
API},
and it issues an error or warning (by calling
...
\item invalid overriding type parameter bound
  (override.type.parameter.upper, override.type.parameter.lower)
  when a type parameter bound in a method declaration does not contain that
  bound in the overridden method's declaration
...
The annotated type of expressions and types is defined via type introduction rules in the
...
on the type system, so the framework provides a way to specify what qualifiers should apply to these expressions.
...
\item
  ...
  visitor has access to only the type (including base type and
  qualifiers).  In your subclass of
...
\subsectionAndLabel{Determine which expressions to refine}{creating-dataflow-determine-expressions}
...
graph) represents source code as \refclass{dataflow/cfg/node}{Node}s.  Some
\refclass{dataflow/cfg/node}{Node}s correspond to a node in the abstract syntax
tree of the program being checked, while others are synthesized during CFG
construction and have no corresponding AST node (see
...
\refclass{dataflow/cfg/node}{LeftShiftNode}, and
...
\code{RegexUtil.asRegex} invocation, it updates the
\refclass{dataflow/analysis}{TransferResult}'s result type. This changes the
type of the \code{RegexUtil.asRegex} invocation when its
...
then branch.  Not all kinds of expressions can be refined; currently
...
the result of \code{RegexUtil.asRegex} invocations:
...
For example, the refined type of \code{RegexUtil.asRegex} is dependent on the
...
\code{RegexUtil.asRegex} method call, the checker will refine the return
...
\item
  ...
  \url{https://github.com/typetools/jdk}, then the checker and the JDK must
  use the same fork name (GitHub organization) and branch name; this is
  necessary so that the CI jobs use the right annotated JDK.
  ...
  \item
    ...
    Section~\ref{library-tips-fully-annotate}.
  \item
  ...
  \item
    ...
    If you forget to import an annotation, or you misspell it in the
    \<import> statement or at some use, it will be \emph{silently ignored},
...
\item \code{-Aonelinemsg}: print each error message on a single line, with
logical lines separated by ``/''.  This is useful when using a tool that
only shows the first line of the error.
...
\item \code{-AdumpOnErrors}: output a stack trace when reporting errors or warnings.

\item \code{-AexceptionLineSeparator}: control what line separator is used when printing exceptions.
This is useful when tools, such as Maven, only display the first line of exception messages. For
...
This option can also help to keep a CI job alive (for example, Travis CI
terminates any job that does not produce output for 10 minutes).
This does not work if you are using Maven, because Maven does not print
...
\begin{itemize}
  \item From a different shell than the Checker Framework is running in
    (running just \<jcmd> gives a list of process IDs):
  \begin{itemize}
...
\item \code{-AoutputArgsToFile}:
  This saves to a file the final command-line parameters as passed to the compiler.
...
\begin{Verbatim}
javacheck -processor mypackage.MyProcessor \
  -Acfgviz=org.checkerframework.dataflow.cfg.visualize.DOTCFGVisualizer,outdir=.,verbose \
  MyClass.java
for dotfile in *.dot; do dot -Tpdf -o "$dotfile.pdf" "$dotfile"; done
\end{Verbatim}
...
The \<dot> program is part of \href{https://www.graphviz.org}{Graphviz}.
...
remove \<--pdf> but add \<--string>. For example:
...
\item[Section: Belly Rub annotations]
  ...
  internally by the implementation) last within each variety of annotation.
...
Since this section of the manual was written, the useful ``Hitchhiker's
...
for a method invocation, and \code{EnhancedForLoopTree} for an enhanced \<for>
statement (also known as a foreach loop).

You should limit your use of trees. A checker uses \<Tree>s mainly to
...
use a \<com.sun.source.util.Trees> instance to obtain a
\<com.sun.source.util.SourcePositions> via \<getSourcePositions()>, then call
\<getStartPosition(...)> (or \<getEndPosition(...)>) on it.
\<com.sun.source.tree.CompilationUnitTree> has a \<LineMap> that lets you convert
positions to line numbers.
Alternatively, you can cast the \<Tree> to the javac-internal type
\<com.sun.tools.javac.tree.JCTree> and call its \<pos()> method, which returns a
\<JCDiagnostic.DiagnosticPosition>; doing so requires your code to be compiled
and run with \<-{}-add-exports jdk.compiler/com.sun.tools.javac.tree=ALL-UNNAMED>
and \<-{}-add-exports jdk.compiler/com.sun.tools.javac.util=ALL-UNNAMED>\@.
...
both return true if \<type> is a \<com.sun.tools.javac.code.Type.UnionClassType> object.
...
method.  For example, if \<type> is a \<com.sun.tools.javac.code.Type.UnionClassType> object, then
\<type.getKind() == TypeKind.DECLARED> is false and \<type.getKind() == TypeKind.UNION> is true.
...
\refclass{javacutil}{AnnotationMirrorMap} and
\refclass{javacutil}{AnnotationMirrorSet} can be used.
...
method is where you would insert any sort of method call to walk the AST\@,
and the latter just returns a constant indicating the source version that is
supported. Implementing these two methods should be enough
...
The javac compiler interfaces can be daunting to a
newcomer, and their documentation is a bit sparse. The Checker Framework
...
% LocalWords:  createSupportedTypeQualifiers FooChecker nullness KeyFor md
...
%%  LocalWords:  RegexUtil LockTransfer FormatterTransfer CFValue argfile
...
% LocalWords:  pids jstack ctrl mycheckername TaintingVisitor FBCBottom
% LocalWords:  getSupportedCheckers SignednessChecker ValueChecker isRegex
% LocalWords:  getOptions OptionalChecker treeannotators visitBinary
% LocalWords:  visitCompoundAssignment UpperBoundAnnotatedTypeFactory
% LocalWords:  RegexAnnotatedTypeFactory UnitsAnnotatedTypeFactory
% LocalWords:  sideEffectsUnrefineAliases postInit AexceptionLineSeparator
% LocalWords:  DOTCFGVisualizer StringCFGVisualizer
