...
and \<Short>.  Every string literal in the program is guaranteed to be interned
(\href{https://docs.oracle.com/javase/specs/jls/se25/html/jls-3.html#jls-3.10.5}{JLS
  \S3.10.5}), and the
...
(\href{https://docs.oracle.com/javase/specs/jls/se25/html/jls-5.html#jls-5.1.7}{JLS \S5.1.7}).
...
\item
  ...
  \item
    ...
    a string contains any upper-case characters.
  \end{itemize}
\end{enumerate}
...
\item[\refqualclass{checker/interning/qual}{InternedDistinct}]
  indicates that the value is not \<equals()> to any other
...
An \<@Interned> annotation on a class declaration indicates that all objects of
that type are interned \textit{except for newly created objects}. That means that
...
body of the constructor are \<@UnknownInterned> by default. Although \<@UnknownInterned InternedClass>
...
@Interned class InternedClass {
  ...
  @SuppressWarnings("interning") // Only creation of an InternedClass object.
...
If the result of every invocation of a constructor is guaranteed to be interned, then the
...
@Interned class AnotherInternedClass {
  // manually verified that all constructor invocations are used such that all
...
The \<@InternedDistinct> annotation is trusted (not verified), because it would be too
complex to analyze the \<equals()> method to ensure that no other value is
\<equals()> to an \<@InternedDistinct> value.  You will need to manually
...
\item
  ...
  As a special case, the operation is permitted if either operand is of
...
value, depending on the run-time argument (\href{https://docs.oracle.com/javase/specs/jls/se25/html/jls-5.html#jls-5.1.7}{JLS
...
(\myurl{https://plse.cs.washington.edu/daikon/}) is also annotated with
