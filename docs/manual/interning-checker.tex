...
\item[\refqualclass{checker/interning/qual}{InternMethod}]
  is a method annotation that indicates that this method returns an interned object and maybe be invoked
...
body of the constructor are \<@UnknownInterned> by default, but no ``type.invalid''
error is issued at the declaration. (It would be issued because
\<@UnknownInterned InternedClass> is not a legal type, given the
qualifier \<@Interned> on \<InternedClass>.)  Instead, an ``interned.object.creation''
...
@Interned class InternedClass {
  ...
  @SuppressWarnings("intern") // ONE is interned for some reason.
...
For example, String literals and the \<null> literal are always considered interned.
