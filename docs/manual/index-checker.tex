...
\refqualclasswithparams{checker/index/qual}{IndexOfIndexFor}{value="this", offset="\#1.length()-1")}.
...
\item[\refqualclasswithparams{checker/index/qual}{IndexOfIndexFor}{String[] value, String[] offset}]
  ...
  the annotated integer is either -1, or it is non-negative and
  for each corresponding sequence \<receiver> and offset \<offset>
  specified in the annotation, the annotated integer
  is less than or equal to \<receiver.length - offset>.
...
\subsection{The need for the \<@IndexOfIndexFor> annotation\label{index-indexof-justification}}
...
That is useful for type-checking code that adds the length of the substring
to the found index, in order to obtain the rest of the string.
