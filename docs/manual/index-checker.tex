...
\section{Typing the result of \<String.indexOf(String)>\label{index-substringindex}}
...
% Without the \<@SubstringIndexFor> annotation, the Upper Bound Checker would not be able to verify that
...
The \<@SubstringIndexFor>
annotation is implemented in a separate Substring Index Checker which runs together with the Index Checker
and has its own type hierarchy (Figure~\ref{fig-index-substringindex}) with three type qualifiers:
\begin{description}
