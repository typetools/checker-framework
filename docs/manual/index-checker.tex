...
Improved feature to support mutable-length data structures like {\tt List}, addressing mutation
...
\item[\refqualclass{checker/index/qual}{Shrinkable}]
  Allows calls to element-removing methods such as remove() and clear() on collections.
...
\item[\refqualclass{checker/index/qual}{UncheckedShrinkable}]
  Allows shrinking but \emph{doesn't guarantee} index safety.
  ...
  index operations (making it the programmer’s responsibility to ensure safety).
\end{description}
