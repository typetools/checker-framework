...
a new annotation such as \<@IndexOfSubstring\allowbreak(value="substring", in="receiver")> is necessary.
...
Because of the value -1, the return type of these methods cannot be
annotated by a \<@LTLengthOf> with a positive offset.
The \<@IndexOfIndexFor> annotation solves it by having the same
meaning as \<@LTLengthOf> with the same parameters, except additionally
allowing the -1 value. The Upper Bound Checker can then infer
...
The \<@IndexOfSubstring> annotation has its own type hierarchy (Figure~\ref{fig-index-indexof}) with three type qualifiers:
\begin{description}
\item[\refqualclasswithparams{checker/index/qual}{IndexOfSubstring}{String[] value, String[] in}]
  ...
  specifed in the annotation, it is less than or equal to \<l.length-s.length>.
