...
\item
  whether an integer came from calling the JDK's \<String.indexOf> or
  \<String.lastIndexOf> method (Section~\ref{index-indexof})
\end{itemize}
...
\section{IndexOf indices\label{index-indexof}}

The sole purpose of the IndexOf Checker is to allow expressing the
properties of the return value of the JDK's
\sunjavadoc{java/lang/String.html\#indexOf-java.lang.String-}{String.indexOf} and
\sunjavadoc{java/lang/String.html\#lastIndexOf-java.lang.String-}{String.lastIndexOf} methods.
These methods return an index of a substring, or -1 if the substring is not found.
...
The IndexOf Checker's type hierarchy (Figure~\ref{fig-index-indexof}) has three type qualifiers:
\begin{description}
\item[\refqualclasswithparams{checker/index/qual}{IndexOfIndexFor}{String[] value, String[] offset}]
  ...
  the annotated integer is either -1, or for each array \<a> and offset \<o>
  specifed in the annotation, it is less than \<a.length-o>.
