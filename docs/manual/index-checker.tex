...
Section~\ref{index-mutable-length} discusses checking of
mutable-length sequences that do not shrink.
...
\sectionAndLabel{Support for mutable-length sequences}{index-mutable-length}

This section discusses sequences whose length can change, such as \<List>.
If an index is valid and then a sequence is made longer, the index remains
valid.  You must indicate which sequences only grow in size, using the type
qualifiers listed below.  The Index Checker verifies the correctness of
your annotations (that is, the Index Checker verifies that any
\refqualclass{checker/index/qual}{GrowOnly}-annotated sequence never
shrinks), and then the Index Checker takes advantage of that fact to
validate indices into the sequence.

The type qualifiers of Figure~\ref{index-mutability-lattice} indicate
whether a sequence might shrink.

\begin{description}
\item[\refqualclass{checker/index/qual}{UnShrinkableRef}]
  The sequence cannot be shrunk through this reference, but
  the sequence might shrink due to operations via other aliases.
  This is the default qualifier for mutable-length structures.

\item[\refqualclass{checker/index/qual}{GrowOnly}]
  The annotated expression can be used to grow the sequence (e.g., via
  \<Collection.add()>), but cannot be shrunk.  Furthermore, every alias is
  also \<@GrowOnly>, so no other alias can shrink the sequence.

\item[\refqualclass{checker/index/qual}{Shrinkable}]
  The length of the sequence may be increased \emph{or} decreased, via
  methods such as \<add()>, \<remove()>, and \<clear()>.  The sequence may
  also change size due to operations through an alias.  Using indexes into
  a \<@Shrinkable> sequence can lead to many warnings.

\item[\refqualclass{checker/index/qual}{UncheckedShrinkable}]
  Like \<@Shrinkable>, but suppresses Index Checker warnings.  That is, the
  Index Checker never warns about the index to a \<get(int)> operation on
  the collection.  Using this type is like suppressing warnings
  (Chapter~\ref{suppressing-warnings}), with the same benefits and
  drawbacks.
\end{description}
...
\end{center}
\caption{Qualifier hierarchy for indicating how a sequence's length may change.}
