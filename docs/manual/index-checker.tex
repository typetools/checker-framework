...
which ranges from \code{from} to \code{to}. So, for example:
...
and that valid indexes to \code{this} range from \code{start} to \code{end}. The length of \code{this}
...
in the example above, then \code{start + x} is \code{@IndexFor("array")}. If \code{y} is \code{@IndexFor("array")},
then \code{y - start} is \code{@IndexFor("this")}. Finally, \code{end - start} is \code{@IndexOrHigh("this")}.
