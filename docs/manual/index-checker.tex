...
annotation on the formal parameter's type.
...
\begin{itemize}
\item
  the minimum length of a sequence, such as ``\<myArray> contains at least 3
  elements'' (Section~\ref{index-minlen})
\item
  whether two sequences have the same length (Section~\ref{index-samelen})
\end{itemize}

The Index Checker checks all these properties at once, but
...
\item[\refqualclass{checker/index/qual}{LowerBoundBottom}]
    There are no values of this type. This is the bottom type, which
    should never need to be written by the programmer.
\end{description}
...
\item[\refqualclasswithparams{checker/index/qual}{LTOMLengthOf}{String[] names}]
  ...
  It may always be used as an index for a sequence listed in \<names>, if it is
  non-negative.
...
that the value of \code{shorter} equals the subsequence that the annotation describes.
\end{description}
...
\item[\refqualclasswithparams{common/value/qual}{MinLen}{int value}]
  ...
  A \code{@MinLen} annotation is treated internally as an
...
\item[\refqualclasswithparams{common/value/qual}{ArrayLen}{int[] value}]
  ...
  The argument can contain at most 10 integers; larger collections of
...
\item[\refqualclasswithparams{checker/index/qual}{NegativeIndexFor}{String[] names}]
  ...
  a \<@SearchIndexFor> and negative.  Applying the bitwise complement
...
\item[\refqualclasswithparams{checker/index/qual}{SubstringIndexFor}{String[] value, String[] offset}]
  ...
  than \<receiver.length() - offset> (where the sequence
...
\item[\refqualclass{checker/index/qual}{LessThanUnknown}]
  There is no information about the value of an expression of this type relative to other expressions.
...
annotations.
This allows the Index Checker to type-check the data structure's
implementation and to type-check uses of the class.
...
The Index Checker treats a call to a method annotated with \code{@LengthOf("this")}
like \code{arr.length} for arrays and \code{str.length()} for strings.

With these annotations, client code like the following type-checks with no
warnings:
\begin{Verbatim}
...
\item[\refqualclass{checker/index/qual}{CanShrink}]
  The length of the sequence may be increased or decreased, via methods such as
  \<add()>, \<remove()>, and \<clear()>.

\item[\refqualclass{checker/index/qual}{UncheckedCanShrink}]
  For the purposes of the Grow-Only Checker, this is treated exactly like
  \<@CanShrink>. It indicates that the collection can shrink. (Its special
...
\item[\refqualclass{checker/index/qual}{PolyGrowShrink}]
  is the polymorphic annotation for the Grow-Only type system.
...
%%  LocalWords:  UpperBoundLiteral UnshrinkableRef UncheckedCanShrink
