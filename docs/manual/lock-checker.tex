...
%  \emph{E}; see \href{https://docs.oracle.com/javase/specs/jls/se25/html/jls-17.html#jls-17.1}{JLS \S17.1}.)
...
re-acquire a lock when correctness requires holding it throughout a
computation.
%
...
\item[\refqualclass{checker/lock/qual}{GuardSatisfied}{\small(\emph{index})}]
  ...
  A return type can only be annotated with \<@GuardSatisfied(\emph{index})>,
  not \<@GuardSatisfied>.
...
\begin{sloppypar}
\begin{description}
\item[\refqualclass{checker/lock/qual}{Holding}\small{(String[] value)}]
  All the given lock expressions
  are held at the method call site.

\item[\refqualclass{checker/lock/qual}{EnsuresLockHeld}\small{(String[] value)}]
  The given lock
  expressions are
...
\item[\refqualclass{checker/lock/qual}{EnsuresLockHeldIf}\small{(String[] expression, boolean result)}]
...
\item[\code{@GuardSatisfied}]
  ...
  \item
    ...
    an identical \<@GuardSatisfied(\emph{index})> type.
  \end{itemize}
...
\item[Overriding methods annotated with \code{@Holding}]
  ...
  $B$'s \<@Holding>
  annotation must be a subset of those in $A$'s \<@Holding>
  annotation.
...
% were not stated explicitly.
  ...
  is the same as that prior to the method call.
...
dereferenced only when the current thread holds all the locks named in the
...
allows a guarded value to be dereferenced even when the variable to which
the value was originally assigned falls out of scope.
...
The following example demonstrates the reason the Lock Checker enforces this
rule:  if \emph{exprSet1} $\neq$ \emph{exprSet2}, then
...
be held.  However, the declaration does not know what those locks are, and they
...
locking and the Lock Checker work.
...
\item A method call, such as
  ...
  \sunjavadoc{java.base/java/util/concurrent/locks/Lock.html\#unlock()}{Lock.unlock()}.
  We use the term ``explicit lock'' for a lock acquired in this
  way.
\end{itemize}
...
\paragraphAndLabel{Synchronization protocol}{lock-checker-holding-synchronization-protocol}
  ...
  the benefits of the annotations and of checking them.
...
\paragraphAndLabel{Method summary that simplifies reasoning}{lock-checker-holding-method-summary}
  ...
  this case, the \code{@Holding} annotation doesn't necessarily introduce a new
  ...
  execution reaches this point, the given locks are known to be already held.  This
...
It is better to write a \code{@Holding} annotation than to write the
extra synchronized block.  Here are the reasons:
...
\item
  ...
  consumes time, even when it succeeds.
\end{itemize}
...
annotations express and enforce a locking discipline over values, just as
...
\href{https://clang.llvm.org/docs/ThreadSafetyAnalysis.html}{Clang's
