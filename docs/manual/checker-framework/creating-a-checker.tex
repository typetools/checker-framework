...
It might seem that the hierarchy shown in Figure~\ref{fig-initialization-hierarchy} lacks
a bottom qualifier.  The actual implementation does contain a bottom
qualifier, \refqualclass{checker/initialization/qual}{FBCBottom}, that
users rarely write.


\paragraphAndLabel{Top qualifier}{creating-top-qualifier}
Your qualifier hierarchy must have a single top qualifier
--- a qualifier that is a (direct or indirect) supertype of every other
qualifier.
There are many reasons this is essential.
One reason is that the default type for local variables is the top
qualifier, and there must be an unambiguous choice for the default type.
...
\<docs/manual/checker-framework/Makefile>.  Keep the lists in
