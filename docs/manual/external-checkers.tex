...
\sectionAndLabel{Determinism checker}{determinism-checker}
...
deterministic program is easier to test and to debug (for example, by
comparing executions).

The Determinism Checker focuses on sequential programs.  It detects
nondeterminism due to the iteration order of a hash table (or on any other
...
help you find whether your program uses any weak or unsupported crypto
algorithms or unsupported algorithm providers. If the
Crypto Checker issues no warnings for a given program, then you have a
guarantee that your program at run time will never have these issues.  This
...
Web Services' Key Management Service only request 256-bit (or longer) data
...
supports class-level and object-level immutability, based on the Checker
Framework.
...
\href{https://uwspace.uwaterloo.ca/bitstream/handle/10012/13181/zhuo_chen.pdf}{``Pluggable
  ...
  Inference''}~\cite{Chen2018} describe the Read Checker.
...
\ahreforurl{https://glacier.coblenz.us/}{Glacier}
...
\item
  Immutability: state in an object is not changeable through any reference to
  the object.
\end{itemize}
...
\href{https://reports-archive.adm.cs.cmu.edu/anon/2020/CMU-CS-20-127.pdf}{``User-Centered
  Design of Principled Programming Languages''}~\cite{Coblenz2020}.


\sectionAndLabel{SQL checker that supports multiple dialects}{sql-checker}

\href{https://www.jooq.org/}{jOOQ} is a database API that lets you build
typesafe SQL queries.  jOOQ version 3.9.0 and later ships with a SQL
...
They were removed from the main distribution on 2016-06-01 because the
...
Types'' (2014) describe SFlow and SFlowInfer.
...
\ahreforurl{https://checklt.github.io/}{CheckLT} uses taint tracking to
...
\ahreforurl{https://sampa.cs.washington.edu/research/approximation/enerj.html}{EnerJ}
...
% Source-Code Repository: https://code.google.com/p/scj-jsr302/
...
% https://portal.acm.org/citation.cfm?doid=1850771.1850792
...
% Developer resources are available at the project page
% \url{https://java.net/projects/loci/}.
For more details, see the paper
\href{https://janvitek.org/pubs/ecoop09.pdf}{``Loci: Simple thread-locality
  for Java''}~\cite{WrigstadPMZV2009} (ECOOP 2009) and the thesis
\href{https://uu.diva-portal.org/smash/get/diva2:428159/FULLTEXT01.pdf}{``The
  Design, Implementation and Evaluation of a Pluggable Type Checker for
  Thread-Locality in Java''}~\cite{Sherwany2011}.

% A paper was published in ECOOP 2009, release 0.1 was made in March 2011,
...
(see Chapter~\ref{units-checker}), this checker includes dynamic checks and
