...
\href{https://github.com/d367wang/value-inference/tree/artifact}{Constant
  Value Inference} project is a whole-program inference approach for
integral range (interval) analysis.

The thesis ``Interval Type Inference: Improvements and
Evaluations''~\cite{Wang2021} (U.~of Waterloo,
\url{https://uwspace.uwaterloo.ca/bitstream/handle/10012/17788/Wang_Di.pdf})
describes Constant Value Inference.


\sectionAndLabel{Crypto Checker}{crypto-checker}

The \href{https://github.com/vehiloco/crypto-checker}{Crypto Checker} can
...
The paper ``Ensuring correct cryptographic algorithm and provider usage at
compile time''~\cite{XingCD2021} (FTFjP 2021, \myurl{https://vehiloco.github.io/crypto-checker/2021-06-04-Crypto-Checker-FTfJP-2021-preprint.pdf})
and the thesis ``Light-weight verification of cryptographic API usage''~\cite{Xing2020}
(U.~of Waterloo, \myurl{https://uwspace.uwaterloo.ca/bitstream/handle/10012/16555/Xing_Weitian.pdf})
describe the Crypto Checker.
...
The \href{https://github.com/opprop/units-inference}{PUnits system} can
check the correctness of a program or annotate a program with units of
measurement.
It is described in the paper ``Precise inference of expressive units of measurement types'' (OOPSLA 2020)~\cite{XiangLD2020}.
...
\ahreforurl{https://github.com/uber/NullAway}{NullAway}~\cite{BanerjeeCS2019} is
...
The theses ``Context Sensitive Typechecking And
Inference: Ownership And Immutability''~\cite{Ta2018} (U.~Waterloo,
\myurl{https://uwspace.uwaterloo.ca/bitstream/handle/10012/13185/Ta_Mier.pdf})
and
``An Immutability Type System for Classes and Objects: Improvements,
Experiments, and Comparisons''~\cite{Sun2021}
(U.~Waterloo,
\myurl{https://uwspace.uwaterloo.ca/bitstream/handle/10012/16882/Lian_Sun.pdf})
describe PICO.

The implementation is available at
\href{https://github.com/opprop/immutability}, and
a docker image is available at
\url{https://hub.docker.com/repository/docker/lnsun/pico}.


\sectionAndLabel{ReadChecker for ensuring that EOF is recognized}{readchecker-checker}
...
and the \href{https://github.com/opprop/ReadChecker}{ReadChecker} and/or
\href{https://github.com/opprop/cast-checker}{Cast Checker} enforces it.

The paper ``"Don't miss the end: Preventing unsafe end-of-file
comparisons''~\cite{ChenD2018} and
the thesis ``Pluggable Properties for Program Understanding: Ontic Type
Checking and Inference''~\cite{Chen2018} (U.~Waterloo,
\myurl{https://uwspace.uwaterloo.ca/bitstream/handle/10012/13181/zhuo_chen.pdf}
describe the ReadChecker.


\sectionAndLabel{Ontology type system}{ontology-checker}

The \href{https://github.com/opprop/ontology}{Ontology type system} groups
related types into coarser concepts.  For example, the SEQUENCE concept
includes Java's \<Array>, \<List>, and their subtypes.  Other examples are
VELOCITY and FORCE.

The thesis ``Pluggable Properties for Program Understanding: Ontic Type
Checking and Inference''~\cite{Chen2018} (U.~Waterloo,
\myurl{https://uwspace.uwaterloo.ca/bitstream/handle/10012/13181/zhuo_chen.pdf}
describes the Ontic type system.
...
Glacier is described by the paper ``Glacier: Transitive class immutability
for Java'' (ICSE 2017)~\cite{CoblenzNAMS2017} and the PhD thesis
``User-Centered Design of Principled Programming Languages''
(2020)~\cite{Coblenz2020}.
...
JCrypt is a static program analysis which transforms a Java program into an
equivalent one, so that it performs computation over encrypted data and
preserves data confidentiality.  It is available at
\url{https://github.com/proganalysis/type-inference}.  It is described in
the paper ``JCrypt: Towards computation over encrypted data''~\cite{DongMD2016:JCrypt}.


%%% 2015

DroidInfer determines the Android library sources (e.g. location access,
phone state) and sinks (e.g., Internet access) used by a program, and
determines whether there is impermissible information flow between them.
It is described in the paper ``Scalable and Precise Taint Analysis for Android'' (ISSTA 2015)~\cite{HuangDMD2015}.

DroidInfer is available at \href{https://github.com/proganalysis/type-inference}.
...
\ahref{https://github.com/proganalysis/type-inference}{SFlow} is a
...
The paper ``Type-based taint analysis for Java web applications'' (FASE
2014, \myurl{https://www.cs.rpi.edu/~milanova/docs/FASE14.pdf}) and the
thesis ``An Inference And Checking Framework For Context-Sensitive
Pluggable Types'' (PhD thesis, 2014)
describe SFlow and SFlowInfer.
...
The ``SFlow $\times$ ReIm'' system combines information flow and reference
immutability.
It is described in the paper ``Composing polymorphic information flow systems with reference immutability''~\cite{MilanovaH2013}.
It is available at \url{https://github.com/proganalysis/type-inference}.
...
\myurl{http://janvitek.org/pubs/ecoop09.pdf}) and the thesis ``The Design,
Implementation and Evaluation of a Pluggable Type Checker for
