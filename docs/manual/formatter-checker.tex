...
as supplying \<null> to each format specifier.  For example,
...
Only three additional conversion categories are needed to represent all possible
...
\begin{enumerate}
    \item The format method's first argument (or second if a \sunjavadoc{java.base/java/util/Locale.html}{Locale} is provided) is a valid
...
\noindent Following are examples of every guarantee:
