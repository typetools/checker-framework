...
Consider the example in Figrue~\ref{fig-value-ignore-overflow}.
Inside the \code{if} block, \code{i}'s type is \code{@IntRange(from = 5, to = Integer.MAX\_VALUE)}.
When the line \code{i = i + 1} is executed, if overflow is taken into account then
\code{i} could have any integer value (since \code{Integer.MAX\_VALUE + 1} is out of
the possible integer range). If the checker is set to ignore overflow, \code{i}'s type
is instead the more precise \code{@IntRange(from = 6, to = Integer.MAX\_VALUE)}.
