...
concatenation.  The \code{-AnonNullStringsConcatenation} command-line option
makes it unsound.
...
Checker assumes that when such a value is used in string
concatenation, it expands to \<"a">, \<"b"> or \<"null">. Every variable
that is not a primitive, String literal or compile-time constant is
considered nullable.

The \code{-AnonNullStringsConcatenation} command-line option makes
the checker assume that all variables are non-null. It ignores Nullness Checker
