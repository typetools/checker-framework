...
The definition of \<@StringVal> says that an expression of type
\<\refqualclass{common/value/qual}{StringVal}("a", "b")> evaluates to one of
the values \<"a">, \<"b">, or \<null>. However, Java converts a String that
is null to ``null'' when that String is used in a String concatenation
expression. Typically, the Value Checker does not consider this fact.
The \code{-AnullStringsConcatenation} command-line option makes the checker
more sound by assuming that all objects can be null. Only String literals,
