...
\item
Alternatively, you can run just the Constant Value Checker by
...
could have more than 10 different values, the Constant Value
...
\<@StringVal> annotation is a subtype of a \<@MatchesRegex> annotation if
each string matches at least one of the regular expressions.
\<@DoesNotMatchRegex(A)> is only a subtype of \<@DoesNotMatchRegex(B)> if the set of regular
expressions \<A> is a superset of the set of regular expressions \<B>.
Matching is done
via the
\href{https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/lang/String.html\#matches(java.lang.String)}{java.lang.String\#matches}
...
An \refqualclass{common/value/qual}{IntVal} annotation and an
...
Because \<int> can be cast to \<double>, an \<@IntVal> annotation is a
...
\begin{Verbatim}
  -processor org.checkerframework.common.value.ValueChecker
  -processorpath ${CHECKERFRAMEWORK}/checker/build/libs/checker-4.2.1-SNAPSHOT.jar:MY_PROJECT/build/
\end{Verbatim}
...
If the option \code{-AreportEvalWarns} is used, the Constant Value Checker issues a warning if it cannot load and run, at
...
instead of being resolved to a specific value annotation.
