...
for range annotations (\<@IntRange>, for instance), making them more precise.
You can supply the \code{-AignoreRangeOverflow} command-line option
when you are confident that no overflow will occur in the program (for
instance, when you have an external proof of that fact).
Figure~\ref{fig-value-ignore-overflow} gives an example.
...
\begin{Verbatim}
  ...
  if (i > 5) {
    ...
    i = i + 1;
    // Because of the possibility of overflow, i may be Integer.MIN_VALUE and
    // i's type is @IntRange(from=Integer.MIN_VALUE, to=Integer.MAX_VALUE).
...
for range types, which makes range types more precise.}
