...
If you are building on Mac OS and employ \ahref{https://brew.sh}{homebrew} to install packages, then the following commands install the aforementioned prerequisites:
...
Mac OS users also need to copy \<hevea.sty> into \<\$HOME/Library/texmf/tex/latex>, which is simply done by:
...
\begin{enumerate}
\item
{\bf [Ubuntu]} In the bash shell, putting the following command in your \code{.bashrc} file
...
\item
{\bf [Mac OSX 10.5 or later]} Add one of the following commands in your \code{.bash\_profile} or \code{.profile} file according with the version of your JDK.
\begin{smaller}
\begin{Verbatim}
...
\begin{smaller}
\begin{Verbatim}
  export JAVA_HOME=$(/usr/libexec/java_home -v 1.7) # JDK 7
\end{Verbatim}
\end{smaller}
