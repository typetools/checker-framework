...
% Don't remove the apostrophe here
\begin{Verbatim}
package 'org.checkerframework.checker.nullness.qual does not exist
\end{Verbatim}
...
\subsection{How to report problems with annotated libraries\label{reporting-bugs-annotated-libraries}}
...
then please open a pull request that adds the annotations.  You
can find the annotated JDK source in the
\ahref{https://github.com/typetools/checker-framework}{Checker Framework
  GitHub repository}, in directory
\ahref{https://github.com/typetools/checker-framework/tree/master/checker/jdk}{\<checker/jdk>}.
You can find the annotated source for other libraries in
...
Please don't open an issue tracker issue just to report that some library
class is not annotated.  Such issues will be closed, to avoid cluttering
the issue tracker.

An alternative to adding annotations to a library is to write stub files,
as described in Section~\ref{stub}.  This is a quick and expedient way to
add or chnage the annotations of an arbirary library.  Creating an
annotated library is a bit more work, but it is less error-prone, easier to
use, runs slightly faster, and it can help other users.
...
\subsection{Build the Stub Parser\label{stubparser-building}}
...
You do not need to add the Stub Parser to the path, as
the Checker Framework build finds it using relative paths.
