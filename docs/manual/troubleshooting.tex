...
Here is an \emph{incomplete} list of papers that use the Checker Framework;
...
If a paper is missing, please let us know so that we can add it.
...
using the Checker Framework.  Each paper gives details not available in
this manual, such as case studies, implementation details, and theoretical
foundations.
...
\item[Map keys (Chapter~\ref{map-key-checker})]
See the two papers about the Checker Framework, described above.

% Optional Checker: no paper yet.

\item[Interning (Chapter~\ref{interning-checker})]
See the two papers about the Checker Framework, described above.

\item[Called methods (Chapter~\ref{called-methods-checker})]
...
describes the Called Methods Checker.  (The paper uses an earlier name,
...
\item[Format Strings (Chapters~\ref{formatter-checker} and~\ref{i18n-formatter-checker})]
...
describes the Format String Checker and the Internationalization Format String Checker.
...
\item[Signedness (Chapter~\ref{signedness-checker})]
``Preventing signedness errors in numerical computations in
Java''~\cite{Mackie2016} (FSE 2016) describes the Signedness Checker.

\item[Returning the receiver (Chapter~\ref{returns-receiver-checker})]
See the ICSE 2020 about the Called Methods Checker, described above.

% \item[Reflection (Chapter~\ref{reflection-resolution})]
% TODO: What paper describes this?

\item[Third-party checkers]
  For papers about these checkers, please see the relevant subsection of Chapter~\ref{external-checkers}.
