...
\ahreforurl{https://github.com/typetools/checker-framework-gradle-plugin\#incompatibility-with-error-prone-234-and-earlier}{Gradle
...
\iffalse
\refqualclass{dataflow/qual}{SideEffectsOnly} (with no expression that can
affect \<currentOutgoing>),
\fi
...
annotations might appear in stub files (\code{.astub} files distributed
with the checker's source code)
or in annotated libraries (which appear
...
\noindent
A value can be assigned to a variable of type \<T extends @MyQualifier
...
least the following 4 parts: commands, inputs, output, and expectation.
...
\item[Commands]
  ...
  might be a set of commands to clone a repository, check out a branch, and
...
type-checker!  That is not a good use of your time to report, nor of the
...
\begin{description}
\item[Ubuntu]
\label{building-prerequisites-ubuntu}
  ...
  is a JDK version such as 17, 21, 25, etc.:
...
% Command taken from:
%   https://github.com/typetools/checker-framework/blob/master/checker/bin-devel/clone-related.sh
%BEGIN LATEX
\begin{smaller}
%END LATEX
\begin{Verbatim}
...
\item[macOS]
\label{building-prerequisites-macos}
...
Make a \<latex> directory and copy the \<hevea.sty> file into it.
(\url{https://hevea.inria.fr/} gives the current version number.)

\begin{Verbatim}
mkdir -p $HOME/Library/texmf/tex/latex
cp -p /opt/homebrew/Cellar/hevea/2.38/lib/hevea/hevea.sty $HOME/Library/texmf/tex/latex/
\end{Verbatim}
...
In the command, \emph{XX} is a JDK version such as 17, 21, 25, etc.
...
\item
If you wish to use Maven or Gradle, publish the Checker Framework artifacts
to a local Maven repository by running \code{./gradlew publishToMavenLocal}:
\begin{Verbatim}
  cd $CHECKERFRAMEWORK
...
(The list does not include papers that merely cite the Checker Framework.)
...
\item
  Verification games: making verification fun (FTfJP 2012)~\cite{DietlDEMWCPP2012}
\item
