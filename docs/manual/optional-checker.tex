...
\<Optional>'s pitfalls.
Most notably, the Optional Checker guarantees that your code will not suffer a
...
\item[\refqualclass{checker/optional/qual}{RequiresPresent}]
  ...
  specified expression to be a present Optional when this
...
\item[\refqualclass{checker/optional/qual}{EnsuresPresentIf}]
  ...
  Optional expression is present. See the Javadoc for examples of its use.
...
The Optional Checker guarantees that your code will not throw a \<NoSuchElementException>
...
on a \<@MaybePresent Optional> receiver, because the receiver may be absent and
each of these methods throws a \<NoSuchElementException> if the receiver is
empty at run time.
...
The Optional Checker considers both the return value and the
argument to be a present Optional after the \<castPresent>
method call.
