...
\item[\refqualclass{checker/optional/qual}{RequiresPresent}]
  ...
  that may require a \<@MaybePresent> field to be \<@Present>.
\item[\refqualclass{checker/optional/qual}{EnsuresPresent}]
  ...
  return) of the annotated method results in the given Optional expression being
...
\item[\refqualclass{checker/optional/qual}{EnsuresPresentIf}]
  ...
  Optional expression is present after the method returns; this is useful for
  a method that assigns a present Optional to a field, for example.
  With \<@EnsuresPresentIf>, if the annotated method returns the given boolean
  value (true or false), then the given Optional expression is present.
  See the Javadoc for examples of their use.
