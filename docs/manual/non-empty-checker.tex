...
The Non-Empty Checker has a second modality, in which it mostly acts like
the Optional Checker (\sectionpageref{optional-checker}).  If you pass the
\<-ArunAsOptionalChecker> command-line argument, then the Non-Empty Checker
runs the Optional Checker, then checks (only) explicitly-written
