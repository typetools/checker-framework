...
The Optional Checker~(\chapterpageref{optional-checker}) uses information from the
...
By default, the Non-Empty Checker verifies methods and
...
\item[\refqualclass{checker/nonempty/qual}{RequiresNonEmpty}]
  ...
  specified expression to be non-empty when the
...
\begin{Verbatim}
    ...
    countryCodes = List.of("CA", "US"); // Now has type @NonEmpty
\end{Verbatim}

A programmer can manually annotate code in cases where a collection,
iterator, iterable, or map is always known to be non-empty, but the type
system cannot infer that fact:

\begin{Verbatim}
    // This method always returns a non-empty map; there is always at least one user in the store
...
should never fail, and getting the first element of a \<@NonEmpty> list should
...
they have a length or size of at least 1 (i.e., they are non-empty).
...
of \<Optional>, even though misuse can lead to the same exception,
...
void parseRegions() {
 @SuppressWarnings("nonempty") // We know that `getRegionIds(x)` returns a non-empty list.
