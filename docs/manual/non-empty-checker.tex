...
Currently, the Non-Empty Checker operates as an auxiliary checker for the
Optional Checker (\sectionpageref{optional-checker}).
The Optional Checker uses information from the Non-Empty Checker to increase
its precision around \<Optional> values whose presence relies on whether a
container is non-empty (or empty).
