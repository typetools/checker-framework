...
The Checker Framework cannot always move an annotation written in a
declaration location to the proper type.  For example, if a user writes
...
\noindent
then the Checker Framework would try to move \<@Nullable> to before \<Map.Entry>,
but that does not make sense --- the \<Entry>, not the \<Map>, is nullable.
(See Section~\ref{common-problems-non-typechecking} for more details.)
So, the user needs to write
