...
It can double the compilation time --- or more, if you run many pluggable
type-checkers at once.  This is especially true if you run pluggable
type-checking on every file, instead of incremental compilation of just the files
...
Using a pluggable type-checker increases compile times by a factor of 2-5.
Slow compilation speed is probably the worst thing about the Checker
Framework.

If the Checker Framework is too slow for you to run on every compilation,
...
Here are some reasons that the Checker Framework is so slow.
\begin{itemize}
\item
  It has to do all the same work as the compiler does, such as resolving
  overloading/overriding/generics and type-checking.  It does so for
  richer, user-defined type systems whereas \<javac> hard-codes one type
  system.
\item
  Its implementation has some inefficiencies.  For example, when running a
  compound or aggregate checker, it makes multiple passes over the program
...
% LocalWords:  checkers'' covariance'' contravariance''
