...
\faqtocpara{\ref{faq-warnings-section}: How to handle warnings and errors}
...
Occasionally, a developer claims to make no errors that type-checking
...
\item[Composability]
A Java object has just one class at run time.  If you care about multiple
...
\item[Efficiency]
  ...
  primitives.  There is no run-time overhead due to extra dereferences
...
maintenance, or are most often incorrectly documented.  Focusing
...
checker.  For other properties, the benefits may not repay the effort.  You will be the best judge of this for your own code, of course.
...
non-determinism in the expression being tested and side effects to the
...
in some places where it is unsound, so that errors are thrown at run time.  The
...
are not a good use of your time to report, nor of the maintainers' time to
...
\item[Consistent behavior with or without pluggable type-checking]
  ...
  pluggable type-checking changed those types, then users would be
...
does not estimate relationships between two variables, except for specific
...
The lack of path-sensitivity can be viewed as a special case of the fact that
...
must be consistent with (a subtype of or equal to) the declaration of the
...
Framework handles declaration annotations in the meantime.
...
A final example is \<@Nullable> and \<@NonNull> annotations
that are intended to work both with modern tools
...
The
\href{https://docs.oracle.com/javase/specs/jls/se25/html/jls-9.html#jls-9.7.4}{Java
...
Java base type is a logical unit, which aids people reading the code.
...
own line instead of on the same line as the type they modify.  (Actually,
...
it handles two of the many type annotations --- \<@Nullable> and \<@NonNull> --- correctly,
but all other type annotations incorrectly!  This would be fixed by merging
...
If you define a new type qualifier --- for example, because you are using the
Subtyping Checker (Chapter~\ref{subtyping-checker}) or Fake Enum Checker
(Chapter~\ref{fenum-checker}), or because you have created a new checker
(Chapter~\ref{creating-a-checker}) --- then the above tools will format
your new type qualifiers incorrectly, like declaration annotations.  Here is
how to fix the formatting.

\begin{itemize}
\item
  For \ahref{https://github.com/diffplug/spotless}{Spotless}, change your
  configuration to indicate the new type qualifiers.  If you use the
  \ahref{https://github.com/diffplug/spotless/tree/main/plugin-gradle}{Spotless
  plugin for Gradle}, edit your \<build.gradle> file to change
  \<formatAnnotations()> to (for example)
  \<formatAnnotations().addTypeAnnotation("Growable").addTypeAnnotation("Modifiable")...>.
\item
  For
  \ahref{https://github.com/plume-lib/run-google-java-format}{run-google-java-format},
  make a pull request to change variable \<type\_annotations> in file
  \ahref{https://github.com/plume-lib/run-google-java-format/blob/master/fixup-google-java-format.py}{fixup-google-java-format.py}.
%% Write text about palantir-java-format when it is uncommented in the list above.
\end{itemize}
...
on Maven Central.  Once users are ready to upgrade to Java 8+ type
...
\sectionpageref{typestate-checker}), but typestate checkers have not been as
extensively tested.
...
ensure that code always calls an \<encrypt> routine in the appropriate
...
\item
  ...
  past minute.'')  To permit the Checker Framework to use up to 4 GB of
...
\item
  Its analysis is much richer.  \<javac> can take certain shortcuts that
...
% 87 as of 2023-04-07, 97 as of 2026-03-28.
about 100 open issues.
It is a proposal, not a standard or an official definition.
...
be
% 247 releases as of: Version 3.54.1 (2026-04-01)
...
\item
  ...
  example specific to the Nullness Checker (Chapter~\ref{nullness-checker}):
...
real-world code with good precision.
...
makes unchecked assumptions about getter methods, and that NullAway assumes
all objects are always fully initialized.  NullAway forces all generic
...
nullness'' or uses a library that uses ``unspecified nullness''.  Second, it
...
\href{https://www.eecs.ucf.edu/~leavens/JML/index.shtml}{JML}, the Java Modeling
