...
that clients of classes that use the builder pattern for object
...
construct objects, but it is error-prone.  For example, failing to provide
...
argument. The checker has built-in support for
...
(Section~\ref{called-methods-framework-details}).
...
command-line flag \<disableBuilderFrameworkSupports>.  For example, to disable both Lombok and AutoValue supports,
