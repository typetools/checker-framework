...
\item The \<-ACalledMethodsChecker\_disableBuilderFrameworkSupports> option disables automatic
 ...
 takes an argument, which is the name of the frameworks to disable. Permitted values:
 \<Lombok>, \<AutoValue>. See \chapterpageref{called-methods-framework-details}. By default,
...
\item The \<-ACalledMethodsChecker\_useValueChecker> option improves precision when analyzing
 code that uses the AWS SDK's \<DescribeImageRequest> API. See \chapterpageref{called-methods-other}.
