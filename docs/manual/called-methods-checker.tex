...
The checker also ignores the possibility that an exception might occur as part
of the call (even if the called method does not throw the exception):
that is, it assumes that the receiver of every method invocation
is non-null at each call-site, and that run-time errors (such as \<OutOfMemoryError>
and the like) do not occur. (You can verify that every receiver is non-null
...
describes the Called Methods Checker in more detail.  (The paper uses an
