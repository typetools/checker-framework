...
by running the Nullness Checker (\chapterpageref{nullness-checker}).
...
\item[\refqualclasswithparams{checker/calledmethods/qual}{CalledMethods}{String[] methodNames}]
  ...
  A typical case in which Lombok users must manually write this annotation is when performing extra
...
\item[\refqualclasswithparams{checker/calledmethods/qual}{CalledMethodsPredicate}{String expression}]
  ...
  \<@CalledMethodsPredicate("!m")> means ``it is not true that \<m> was
...
\item[\refqualclass{checker/calledmethods/qual}{EnsuresCalledMethods}]
  ...
  If \<m()> might have side-effects (i.e., it is not annotated
  as \refqualclass{dataflow/qual}{SideEffectFree},
  as \refqualclass{dataflow/qual}{Pure}\iffalse,
  or as \refqualclass{dataflow/qual}{SideEffectsOnly} with no expression
  that can affect the value of \<sock>\fi),
  ...
  \begin{itemize}
  \item add a \refqualclass{dataflow/qual}{SideEffectFree} or
  \refqualclass{dataflow/qual}{Pure} annotation to \<m()>, if \<m()> is
    in fact side-effect free or pure.
  \iffalse
  \item add a \refqualclass{dataflow/qual}{SideEffectsOnly} annotation if
  \<m()> has side effects but cannot affect the value of \<sock>.
  \fi
...
\item[\refqualclass{checker/calledmethods/qual}{EnsuresCalledMethodsVarargs}]
  ...
  currently cannot be verified, and an \<ensuresvarargs.unverified> error is always issued
...
\item[\refqualclass{checker/calledmethods/qual}{EnsuresCalledMethodsOnException}]
  ...
  \<@Owning> annotation.  See Section~\ref{resource-leak-owning-parameters-and-exceptions}.
...
\item[\refqualclass{checker/calledmethods/qual}{RequiresCalledMethods}]
  ...
  use a \<@CalledMethods> annotation on the parameter instead.
...
You can disable support for builder frameworks by specifying them in a
...
by Lombok and AutoValue. There are three places where annotations are usually assumed:
\begin{itemize}
...
\href{https://www.cve.org/CVERecord?id=CVE-2018-15869}{CVE-2018-15869}
