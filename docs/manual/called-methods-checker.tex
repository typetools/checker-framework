...
The checker also ignores the possibility that a run-time exception might occur:
in particular, it assumes that the program is free of null-pointer dereferences and
that the JVM itself does not suffer from a run-time error (such as an \<OutOfMemoryError>
or the like). (You can verify that all pointer dereferences are safe
