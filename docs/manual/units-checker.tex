...
of the metric unit newton and the imperial measure pound-force.
...
that you can multiply with an unqualified integer to get values
...
\item For \code{@Acceleration}:
  meters per second squared \refqualclass{checker/units/qual}{mPERs2}
...
\item For \code{@Current}:
  amperes \refqualclass{checker/units/qual}{A}

\item For \code{@Force}:
  newtons \refqualclass{checker/units/qual}{N}
  and the derived unit
...
\item For \code{@Luminance}:
  candelas \refqualclass{checker/units/qual}{cd}
...
\item For \code{@Substance}:
  moles \refqualclass{checker/units/qual}{mol}

\item For \code{@Temperature}:
  kelvins \refqualclass{checker/units/qual}{K}
  and the derived unit
  degrees Celsius \refqualclass{checker/units/qual}{C}
...
For example, to use nanoseconds as a unit, you could use
...
Static imports allow very natural notation; for
...
Multiplying a scalar by a unit type results in the same unit type.
...
initialize a variable of a unit type with a literal value.
...
% LocalWords:  UnitsRelations Aunits MyFile mm2 m2 km2 enum ElementType kN
%  LocalWords:  MixedUnits java mPERs2 api classpath bootclasspath RUNTIME
%%  LocalWords:  AunitsDirs myProject myLibrary Luminance processorpath m3
% LocalWords:  util mm3 km3
