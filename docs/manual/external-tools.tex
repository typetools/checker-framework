...
dependencies {
    ...
    ext.checkerFrameworkVersion = '3.6.1'
...
prebuilt_jar(
    ...
    binary_jar = 'checker-3.6.1.jar',
    visibility = [ 'PUBLIC' ]
)

prebuilt_jar(
    name = 'checker-qual',
    binary_jar = 'checker-qual-3.6.1.jar',
    visibility = [ 'PUBLIC' ]
)
...
other than \<\${HOME}/checker-framework-3.6.1/>.
...
\begin{itemize}
  \item
    Option 1:
    Add directory
    \code{.../checker-framework-3.6.1/checker/bin} to your path, \emph{before} any other
...
\begin{Verbatim}
  export CHECKERFRAMEWORK=${HOME}/checker-framework-3.6.1
...
\begin{Verbatim}
  ...
  # Windows
  set CHECKERFRAMEWORK = C:\Program Files\checker-framework-3.6.1\
...
\item \<checker-qual.jar>: \url{https://search.maven.org/artifact/org.checkerframework/checker-qual/3.6.1/jar}
\item \<checker.jar>: \url{https://search.maven.org/artifact/org.checkerframework/checker/3.6.1/jar}
\end{itemize}
...
automatically compile) your work periodically.

You may also wish to do the following:
Press Ctrl+Shift+A (Cmd+Shift+A on macOS) and search for Registry.
Open it to find and enable \<compiler.automake.allow.when.app.running>.
