...
\item Direct the Maven compiler plugin to use the desired checkers by
  creating two new profiles as shown below (the example uses the Nullness
  Checker).  If your POM file does not already use the
  \code{maven-compiler-plugin} plugin, you are done.  If your POM file
  already uses the plugin, copy the existing configuration into each
  profile.

\begin{mysmall}
\begin{alltt}
    <profile>
      <id>checkerframework-java8</id>
      <activation>
        <jdk>1.8</jdk>
      </activation>
      <build>
...
tests, wrap each {@code <configuration>...</configuration>} within
