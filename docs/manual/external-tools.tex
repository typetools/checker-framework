...
prebuilt_jar(
    ...
    binary_jar = 'checker-2.5.8.jar',
    visibility = [ 'PUBLIC' ]
)

prebuilt_jar(
    name = 'checker-qual',
    binary_jar = 'checker-qual-2.5.8.jar',
    visibility = [ 'PUBLIC' ]
)
...
\begin{itemize}
  \item
    Option 1:
    Add directory
    \code{.../checker-framework-2.5.8/checker/bin} to your path, \emph{before} any other
...
\begin{Verbatim}
  export CHECKERFRAMEWORK=${HOME}/checker-framework-2.5.8
...
\begin{Verbatim}
  ...
  # Windows
  set CHECKERFRAMEWORK = C:\Program Files\checker-framework-2.5.8\
...
Project Lombok (\url{https://projectlombok.org/}) is a widely-used Java library.
Starting with Lombok version 1.18.3 it is easy to use Lombok together with the
Checker Framework. When Lombok generates code, selected type annotations are
copied, for example, from fields to generated setter/getter/builder methods.
These copied type annotations are necessary to successfully type check code that
uses the Lombok-generated code.

Lombok automatically copies many standard Nullable/NonNull annotations, but the
...
All \<org.checkerframework> annotations, as well as any custom type use
annotations, must be added to \<lombok.copyableAnnotations>.

Directory \<docs/examples/lombok> contains an example Gradle project that
combines Lombok with the Checker Framework.

% Link to discussion thread and pull request:
% https://github.com/rzwitserloot/lombok/pull/1837

% TODO: see whether all Checker Framework annotations could be added to
% BASE_COPYABLE_ANNOTATIONS in lombok.
...
% LocalWords:  systemPath artifactID MacOS eclipsec
