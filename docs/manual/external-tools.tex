...
prebuilt_jar(
    ...
    binary_jar = 'checker-2.8.1.jar',
    visibility = [ 'PUBLIC' ]
)

prebuilt_jar(
    name = 'checker-qual',
    binary_jar = 'checker-qual-2.8.1.jar',
    visibility = [ 'PUBLIC' ]
)
...
checker on every compile for Java projects and/or modules.  If your project uses some other build tool, such as Gradle, then follow the instructions for that build tool.

To run a checker within IntelliJ IDEA:

\begin{enumerate}

\item Change the project sdk to 1.8 \ahref{https://www.jetbrains.com/help/idea/sdk.html#change-project-sdk}{following these instructions}.

\item Change the language lever for your project to 8
\ahref{https://www.jetbrains.com/help/idea/creating-and-managing-projects.html#project-structure-settings}{following these instructions}.

\item Add the annotated JDK library to the bootclasspath.
On \ahref{https://www.jetbrains.com/help/idea/java-compiler.html}{the Java compiler page}, in the
  ``Additional command line parameters'': section add:
  \<-Xbootclasspath/p:"<path to checker framework>/checker/dist/jdk8.jar">

\item Create an annotation profile
\ahref{https://www.jetbrains.com/help/idea/configuring-annotation-processing.html#create\_profile}{following these instructions}.
Associate any desired modules with that profile.
\ahref{https://www.jetbrains.com/help/idea/configuring-annotation-processing.html#associate}{See instructions}.
Configure that profile:
\begin{enumerate}
\item  Add checker.jar to the processor path.  (It's located in checker-framework/checker/dist)
\item  Add checkers to be run during compilation by writing the fully-qualified name of the checker in the ``Processor FQ Name'' section.

\end{enumerate}
\item Add checker-qual.jar, located in
\<\$CHECKERFRAMEWORK/checker/dist/checker-qual.jar>,  as a dependency to all
modules you wish to type check. (That should be those you specified in the
annotation profile above.)
\ahref{https://www.jetbrains.com/help/idea/creating-and-managing-projects.html#project-structure-settings}{Follow these instructions}.
...
\begin{itemize}
  \item
    Option 1:
    Add directory
    \code{.../checker-framework-2.8.1/checker/bin} to your path, \emph{before} any other
...
\begin{Verbatim}
  export CHECKERFRAMEWORK=${HOME}/checker-framework-2.8.1
...
\begin{Verbatim}
  ...
  # Windows
  set CHECKERFRAMEWORK = C:\Program Files\checker-framework-2.8.1\
