...
generates getter, setter, and builder methods.  For example, if you declare
a field:
...
Unfortunately, the generated code is lacking \<@Nullable> annotations on
the return type of \<getRole> and the argument type of \<setRole>.
You need to tell Lombok to copy type arguments from the field, so that
...
(October 2018),
Lombak does not yet support any of the Checker Framework annotations; see variable
...
% TODO: Make Lombok automatically copy all type annotations.
...
% Link to pull request that adds just a couple, to test the maintainers'
% receptiveness:  https://github.com/rzwitserloot/lombok/pull/2003
