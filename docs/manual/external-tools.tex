...
The Checker Framework is generally compatible with code generated by Lombok: if the
code you are typechecking depends on such code, the Checker Framework will work as
expected. Lombok copies annotations included with the Checker Framework distribution
automatically when using \<@Getter>, \<@Setter>, and \<@Builder>.

If you are using other annotations (i.e. annotations not defined
in the \<org.checkerframework:checker-qual> artifact), then you
can add some configuration to Lombok to enable it to copy those
annotations, too:
...
To typecheck classes that contain code generated by Lombok, you must run the
\href{https://projectlombok.org/features/delombok}{Delombok} tool included in your
Lombok distribution before running the Checker Framework on your code.
This restriction applies not only to the code actually generated by Lombok,
but also to all other code
in such classes. The Checker Framework
Gradle plugin does this for you automatically if you use the
\href{https://plugins.gradle.org/plugin/io.freefair.lombok}{io.freefair Lombok plugin}.
The Checker Framework will not typecheck any class that contains code generated by
Lombok if Lombok and the Checker Framework are run together, which could lead to
unsoundness (missed errors) in such classes.
