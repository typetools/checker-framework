...
prebuilt_jar(
    ...
    binary_jar = 'checker-2.10.1.jar',
    visibility = [ 'PUBLIC' ]
)

prebuilt_jar(
    name = 'checker-qual',
    binary_jar = 'checker-qual-2.10.1.jar',
    visibility = [ 'PUBLIC' ]
)
...
There are four ways to achieve this.  You can use any
one of them.  However, if you are using the Windows command shell, you must
use one of the last two.
...
\begin{itemize}
  \item
    Option 1:
    Add directory
    \code{.../checker-framework-2.10.1/checker/bin} to your path, \emph{before} any other
...
\begin{Verbatim}
  export CHECKERFRAMEWORK=${HOME}/checker-framework-2.10.1
...
\begin{Verbatim}
  ...
  # Windows
  set CHECKERFRAMEWORK = C:\Program Files\checker-framework-2.10.1\
...
\end{Verbatim}
   ...
   \<javac -J-Xbootclasspath/p:path-to-error-prone-javac.jar>
   or
   \<javac -J-Xbootclasspath/p:path-to-checker-framework/checker/dist/javac.jar>.
   This uses the javac from Error Prone.  Compared to the standard Java 8 javac,
...
\item If you are using Java 8, use the javac from Error Prone.  Compared to
  the standard Java 8 javac, it fixes some important bugs.
