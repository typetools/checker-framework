...
\item Other tools (Section~\ref{javac})
\end{itemize}
...
in the ``Additional command line parameters'': section, add:\\
  \code{-Xbootclasspath/p:"<path to checker framework>/checker/dist/jdk8.jar"}
...
\sectionAndLabel{Javac compiler (command line)}{javac-installation}
...
\begin{itemize}
  \item
    Option 1:
    Add directory
    \code{.../checker-framework-2.11.0/checker/bin} to your path, \emph{before} any other
...
%    Lombok runs.
%  * Add all Checker Framework annotations to BASE_COPYABLE_ANNOTATIONS in
%    the Lombok implementation.  But lombok.copyableAnnotations is still
%    needed for user-defined type annotations.
...
\item If you are using Java 8, use the javac from Error Prone.
...
\sectionAndLabel{Other tools}{javac}

This section explain how to use the Checker Framework with \<javac>.  This is
useful if you do not wish to use the \<javac> wrapper script described in
Section~\ref{javac-installation} or to help you adapt your build tool.  This
section assumes you have downloaded the Checker Framework release zip and set
the environment variable \<CHECKERFRAMEWORK> to point to the unziped directory.
You can also get all of the jars mentioned in this section from Maven Central:
...
Different arguments are required for Java 8, Section~\ref{java8-install}, and Java 9+, Section~\ref{java11-install}.

\subsectionAndLabel{Java 8}{java8-install}

This sections show what arguments to pass to javac to run the Checker Framework using JDK 8.
...
\begin{enumerate}
\item \<-J-Xbootclasspath/p:\$CHECKERFRAMEWORK/checker/dist/javac.jar>: the
ErrorProne javac.jar, the OpenJDK Java 9 compiler, must be on the JVM
bootclasspath. The \<-J> indicates that this is a JVM argument rather than a
compiler argument.
...
must be on the bootclasspath of the compiler.  (This is a compiler arguments, so do not add \<-J>.)
...
checker.jar must be on the processor path. Using this option means that the classpath will not be
searched for other annotation processors.
...
(Note, using this options means that javac will not search for annotation
processors, but rather run only those specified here.)

\item \<-source 8 -target 8>: Because using the ErrorProne \<javac.jar> turns
the Java 8 compiler into a Java 9 compiler, these options ensure that the
source code is Java 8 and that Java 8 bytecode is created.

\end{enumerate}

\subsectionAndLabel{Java 11}{java11-install}

This section shows what arguments to pass to javac to run the Checker Framework using JDK 11.  These
instructions should work on JDK 9 or newer, but we only test with JDK 11.
...
\<-J--add-opens=jdk.compiler/com.sun.tools.javac.comp=ALL-UNNAMED>: opens the
\<jdk.compiler/com.sun.tools.java.comp> package.  This is required because the
Checker Framework reflectivly accesses this package.  The rest of the arguments
are explained above.
...
Framework inserts annotations into bytecode even if none appear in source code,
...
% LocalWords:  AoutputArgsToFile qual jdk8 annotatedJdk Unhandled dex
% LocalWords:  annotationProcessors annotationProcessor JavaCompile Ctrl
% LocalWords:  targetJavaVersion GradleExamples gradle JavaVersion
% LocalWords:  systemPath artifactID MacOS eclipsec getter getRole setRole
% LocalWords:  config copyableAnnotations
