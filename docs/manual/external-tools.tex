...
\item Android Studio and the Android Gradle Plugin (Section~\ref{android-gradle})
...
All examples in this chapter are in the public domain, with no copyright or
...
dependencies {
    ...
    ext.checkerFrameworkVersion = "4.2.3"
...
the correct version of \code{javac}.
...
prebuilt_jar(
    ...
    binary_jar = 'checker-4.2.3.jar',
    visibility = [ 'PUBLIC' ]
)

prebuilt_jar(
    name = 'checker-qual',
    binary_jar = 'checker-qual-4.2.3.jar',
    visibility = [ 'PUBLIC' ]
)
...
However, the command lines that Buck prints with \<-v 8> are \emph{not} what
...
other than \<\${HOME}/checker-framework-\ReleaseVersion{}/>.
...
\begin{itemize}
  \item
    Option 1:
    Add directory
    \code{.../checker-framework-\ReleaseVersion{}/checker/bin} to your path, \emph{before} any other
...
\begin{Verbatim}
  export CHECKERFRAMEWORK=${HOME}/checker-framework-4.2.3
...
\begin{Verbatim}
  ...
  # Windows
  set CHECKERFRAMEWORK = C:\Program Files\checker-framework-4.2.3\
...
\begin{itemize}
\item \<checker-qual.jar>: \url{https://repo1.maven.org/maven2/org/checkerframework/checker-qual/4.2.3/checker-qual-4.2.3.jar}
\item \<checker-util.jar>: \url{https://repo1.maven.org/maven2/org/checkerframework/checker-util/4.2.3/checker-util-4.2.3.jar}
\item \<checker.jar>: \url{https://repo1.maven.org/maven2/org/checkerframework/checker/4.2.3/checker-4.2.3-all.jar}
\end{itemize}

These instructions should work on JDK 17 or later, but we only test with supported
versions of Java: JDK 17, JDK 21, and JDK 25.


\subsectionAndLabel{Non-modularized code}{javac-jdk11-non-modularized}
...
opens the \<jdk.compiler/com.sun.tools.javac.comp> package.  This is
...
required because the Checker Framework accesses non-exported members of these packages.

\end{enumerate}

If these options are missing, \<javac> may issue one of the following warnings:
\begin{Verbatim}
...
\subsectionAndLabel{Modularized code}{javac-jdk11-modularized}
...
\<checker-qual.jar> must be on the module path rather than the classpath, but
...
use the
\ahreforurl{https://github.com/typetools/checker-framework-gradle-plugin}{Checker
...
\item Change the project SDK to 17 or later, as explained at
...
\href{https://github.com/typetools/checker-framework-gradle-plugin}{Checker
  Framework Gradle plugin} does this for you automatically.


\subsectionAndLabel{Maven projects using Lombok}{lombok-maven}

The \href{https://anthonywhitford.com/lombok.maven/lombok-maven-plugin/}{Lombok Maven Plugin}
can be used to delombok source code for Maven projects. The
\href{https://anthonywhitford.com/lombok.maven/lombok-maven-plugin/faq.html#alt-src-setup}
...
\href{https://central.sonatype.com/search?q=org.checkerframework}{Maven
...
\item Direct the Maven compiler plugin to use the desired checkers by
  creating a new profile as shown below (the example uses the Nullness
  Checker).
...
or later.  Versions 1.19 and earlier are broken.  If necessary, you can
...
There are two approaches to running a checker in NetBeans:  modifying the
project properties, or using a custom Ant target.
