...
\begin{itemize}
  \item
    Option 1:
    Add directory
    \code{.../checker-framework-2.2.2/checker/bin} to your path, \emph{before} any other
...
\begin{Verbatim}
  export CHECKERFRAMEWORK=${HOME}/checker-framework-2.2.2
...
\begin{Verbatim}
  ...
  # Windows
  set CHECKERFRAMEWORK = C:\Program Files\checker-framework-2.2.2\
...
\item
  ...
  Note: It is a good idea to enclose the address in double quotation marks
  to escape special characters and spaces (to avoid errors). For example,
  \<-Xbootclasspath/p:"\$CHECKERFRAMEWORK/checker/dist/jdk8.jar">
...
\item
  ...
  a new Annotation Processor profile. Give this new profile a unique name
  of your choice, avoiding spaces and special characters. For example,
  "annotationProcessorProfile". Select the modules you would like to
...
% LocalWords:  IntelliJ Maia newdir classpath Unconfuse nullness Gradle FQ
...
%%  LocalWords:  typeAnnotationsJavac targetJavaVersion GradleExamples Ok
%%  LocalWords:  systemPath artifactID MacOS eclipsec init macrodef popup
%  LocalWords:  checkbox checkboxes
