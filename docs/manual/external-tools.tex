...
The Checker Framework should usually not be run together with Lombok because classes which
Lombok modifies (e.g. that include \<@Getter> or \<@Builder> annotations) will not be typechecked.
This restriction applies not only to the code generated by Lombok, but also to all other code
in such classes. We therefore strongly recommend running the
\href{https://projectlombok.org/features/delombok}{Delombok} tool included in your
Lombok distribution before running the Checker Framework on your code. The Checker Framework
