...
\sectionAndLabel{Only direct leakage, via a chain of assignments and operations}{confidential-direct-only}
...
\sectionAndLabel{Confidential values in generic types}{confidential-generics}

\code{@NonConfidential} is the default qualifier on an unannotated upper
bound of a type parameter.  Therefore, a generic class that has not been
annotated for the Confidential Checker cannot hold a confidential value:

\begin{Verbatim}
  // error: the type argument is not within the bound of List's type parameter
  List<@Confidential String> passwords;
\end{Verbatim}

To store confidential values in a generic class, the class's type parameter
must have an explicit upper bound of \<@UnknownConfidential> or
\<@PolyConfidential>, either in its source code or in a stub file
(\sectionpageref{stub}).
...
\code{@Confidential} and \code{@NonConfidential} values is most narrowly typed
as follows:
