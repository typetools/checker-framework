...
In certain cases, the analysis done by the Regex Checker can be utilised to
prevent some false positives. Consider the following snippet of code:

\begin{Verbatim}
  Pattern PATTERN = Pattern.compile("^.(.*).$");
  Matcher matcher = PATTERN.matcher(input);
  // :: error: (assignment.type.incompatible)
  @NonNull String a = matcher.group(1); // Will not throw a NullPointerException. False positive.
\end{Verbatim}

To prevent such false positives, you can run the Regex Checker as a sub-checker
of the Nullness Checker. To do this supply the -AenableRegexChecker flag.
