...
\item The passed array size is less than the collection size. Here are heuristics to
  ...
  \begin{itemize}
    \item an empty array initializer, e.g. \<c.toArray(new String[] \{\})>,

    \item array creation tree of size 0, e.g. \<c.toArray(new String[0])>, or
...
option, a call to \code{Collection.toArray} will return an array with a non-null
component type if the argument is a field accesses where the field declaration
has a \refqualclasswithparams{common/value/qual}{ArrayLen}{0} annotation.
