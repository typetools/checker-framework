...
\item
  ...
  the checker trusts \refqualclass{common/value/qual}{ArrayLen} annotations with
  length 0 when determining whether a call to \code{Collection.toArray} will
  return an array with non-null components.
  See Section~\ref{nullness-collection-toarray} for a discussion.
...
% Keep this in sync with the Javadoc of org.checkerframework.checker.nullness.CollectionToArrayHeuristics
...
\begin{itemize}
\item The receiver collection type argument is \<@NonNull>, and
\item The passed array size is less than the collection size. Here are heuristics to
  ...
  \begin{itemize}
    ...
    \item array creation tree of the collection size method invocation
      \<c.toArray(new String[c.size()])>.
\end{itemize}

\end{itemize}

Additionally, when the lint option \<-Alint=trustArrayLenZero> is provided,
field accesses where the field declaration has a \<@ArrayLen(0)> annotation are
considered when the Nullness Checker is estimating whether a call to
\code{Collection.toArray} will return an array with non-null components.
...
Note: From \sunjavadoc{java.base/java/util/Collection.html\#toArray(T[])}{Collection.toArray(T[])},
the nullness of the returned array doesn't depend on the passed array nullness.
