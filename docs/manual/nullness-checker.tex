...
\item
  ...
  If you supply the \code{-Alint=trustArrayLenZero} command-line option, then a
  call to \code{Collection.toArray} will return an array with a non-null
  component type if the argument is a field accesses where the field declaration
  has a \refqualclasswithparams{common/value/qual}{ArrayLen}{0} annotation. See
  Section~\ref{nullness-collection-toarray} for a discussion.
...
with a non-null component type by the Nullness Checker if the argument is a field
accesses where the field declaration has a \refqualclasswithparams{common/value/qual}{ArrayLen}{0}
annotation.
