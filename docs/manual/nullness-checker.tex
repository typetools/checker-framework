...
If the different nullness annotation is a type annotation, the Checker
Framework treats it exactly as if you had written the corresponding annotation
from the Nullness Checker, as described in
Figure~\ref{fig-nullness-refactoring}.  If the different annotation is a
declaration annotation, the Checker Framework converts them to type
annotations, in a way that is consistent with their semantics.  This means that
annotations on array and varargs declarations are treated inconsistenly with
the Checker Framework annotations.  For example,
\<@android.support.annotatoin.NonNull Object []> is treated as a ``non-null
array of objects'', rather than as ``array of non-null objects''.
