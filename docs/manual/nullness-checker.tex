...
\subsectionAndLabel{Utilising the Regex Checker analysis}{nullness-enable-regex-checker}

In general, the \<Matcher.group> method may return \<null>, so its return
type is annotated as \<@Nullable>.
In certain cases, the Regex Checker can determine that a particular group
exists, so some calls to \<Matcher.group> return a non-null value.
Here is an example:

\begin{Verbatim}
  Pattern PATTERN = Pattern.compile("^.(.*).$");
  Matcher matcher = PATTERN.matcher(input);
  @NonNull String a = matcher.group(1); // OK, result is non-null.
\end{Verbatim}

To prevent a false positive warning on the last line of code, you can run
the Regex Checker as a sub-checker of the Nullness Checker. To do this,
supply the \<-AenableRegexChecker> flag.
...
\hline
 ...
 ~org.jspecify.annotations.NullnessUnspecified~ \\ \hline
