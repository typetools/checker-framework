...
\item
  ...
  As a special case of a \refqualclass{checker/nullness/qual}{NonNull}
...
\item
  ...
  If you supply the \code{-AinvocationPreservesArgumentNullness} command-line
...
\item[\refqualclass{checker/nullness/qual}{MonotonicNonNull}]
  ...
  methods that are \refqualclass{dataflow/qual}{SideEffectFree}\iffalse or
  (for non-listed expressions) \refqualclass{dataflow/qual}{SideEffectsOnly}\fi, or that
...
code.  Figure~\ref{fig-initialization-hierarchy} shows their type hierarchy.  For
...
\item[\refqualclass{checker/nullness/qual}{KeyFor}]
    indicates that a value is a key for a given map --- that is, that
...
can then dereference the result of \code{Class.getComponentType} safely
...
null value in an array of non-null element type, use the command-line option
...
\<@MonotonicNonNull> array, initialize
...
returned array's component type is \<@NonNull> if the following conditions
hold:
...
Note: The nullness of the returned array doesn't depend on the nullness of the passed array.
...
(Section~\ref{type-refinement}), which infers more specific types for
...
\item
  ...
  \href{https://plse.cs.washington.edu/daikon/download/doc/daikon.html#AnnotateNullable}{AnnotateNullable}
  tool of the \href{https://plse.cs.washington.edu/daikon/}{Daikon invariant
    detector}.
...
The Nullness Checker considers both the return value and the
...
crash with a \<NullPointerException>.  A formal parameter
...
\item
The
\href{https://plse.cs.washington.edu/daikon/}{Daikon invariant detector}.
...
there is little point using the nullness analysis of a bug detector such as
...
\subsectionAndLabel{Incompatibility note about FindBugs and SpotBugs \code{@Nullable}}{findbugs-nullable}

FindBugs and SpotBugs have a non-standard definition of
\<@Nullable>.  This treatment is not documented in the annotation's own
...
FindBugs and SpotBugs made a poor choice of names.  The choice of names should have made a
...
should also have been consistent with other tools, and intuitively clear to
...
\subsectionAndLabel{Relationship to \code{Optional<T>}}{nullness-vs-optional}
...
lead you to choose to use regular Java references rather than \<Optional>.
...
\item
  ...
  and in either case your code crashes.
\item
...
\caption{Partial type hierarchy for the Initialization type system.
  ...
  the right side of the figure illustrates their type hierarchy in more detail.}
...
\item[\refqualclass{checker/initialization/qual}{UnknownInitialization}]
  ...
  field declared in \<Foo> or its superclasses is initialized, but other
  fields might not be.
...
\href{https://docs.oracle.com/javase/specs/jls/se25/html/jls-16.html}{JLS ch.16}),
...
\href{https://docs.oracle.com/javase/specs/jls/se25/html/jls-4.html#jls-4.12.4}{JLS \S 4.12.4}) because fields
...
helper routine that is called by a constructor.  For example:
...
class A {
   ...
   public A() {
       ...
       // If we were to type 'this' as @Initialized, then the following call is valid:
       doSomething();
   }
   void doSomething() {}
}
...
an object is uninitialized until initialization completes, and then it is initialized.  The full truth is a bit more complex:  during the
...
the initialized and uninitialized classes.
The \<@UnderInitialization(\emph{classliteral})> annotation
...
considered:  the rules governing reading from and writing to objects that are
currently under initialization (both reading from fields of objects under
initialization and writing objects under initialization to fields).
By default, only fully-initialized objects can be stored in a field of
another object.  If this were the only option, then it would not be possible
...
In this case, the rules for reading from and writing to that field become a little
...
\refqualclass{checker/initialization/qual}{UnderInitialization}, then
...
\begin{Verbatim}
  ...
  class Node<T> {
    ...
    Node(@UnderInitialization List<T> parent) {
      this.parent = parent;
      this.prev = this;
      this.next = this;
    }
...
% \urldef{\jlsconstructorbodyurl}{\url}{https://docs.oracle.com/javase/specs/jls/se25/html/jls-8.html#jls-8.8.7}
...
\<@Initialized> and \<@UnderInitialization>.
\end{sloppypar}
...
A method with an uninitialized receiver may assume that a few fields (but not all
...
% LocalWords:  clearProperty AassumeKeyFor WeakReference checkers''
