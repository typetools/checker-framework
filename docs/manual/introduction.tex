...
\ahref{https://search.maven.org/search?q=org.checkerframework.annotatedlib}{in
  the Maven Central Repository}, you can contribute by annotating it, which will
...
\item
  ...
  \<-AstubWarnNote>,
  Warn about problems with stub files; see Section~\ref{stub-using}.
...
\item
  Don't type-check clients.  JUnit's \<assertNotNull()> is typically called
  only in test code, not in your main program.  If you type-check only your
  main program, then the annotation on \<assertNotNull()> is irrelevant.
\end{itemize}
