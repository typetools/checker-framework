...
\href{https://eisop.uwaterloo.ca/live/}{Checker Framework Live Demo} webpage.
\item
...
\textbf{Requirement:}
You must have a JDK (version 17 or later) installed.
...
\item
  ...
  \\
  %END LATEX
  \url{https://checkerframework.org/checker-framework-4.2.3.zip}
...
\item The programmer writes annotations, such as \refqualclass{checker/nullness/qual}{NonNull} and
  ...
  annotations that are consistent with their code:  see Chapter~\ref{type-inference}.)
...
Two concrete examples of using \code{-processor} to run the Nullness Checker are
...
created by another annotation processor.  It may also include other Java
files of yours, if they are more recent than the corresponding \code{.class} file.
...
\<-Astubs=checker.jar/\emph{stubfilename.astub}>.
...
\item \<-AcheckPurityAnnotations>
  Check the bodies of methods and constructors marked
...
\fi
  ...
  to ensure the method or constructor satisfies the annotation.  By default,
  the Checker Framework unsoundly trusts the annotation.  See
  Section~\ref{type-refinement-purity}.
...
\item \<-AinferOutputDirectory>
  The directory into which to write inference output (e.g., \<.ajava>
  files).  It defaults to \<build/whole-program-inference>.  When using
  a build system (e.g., Gradle), the default may be interpreted relative to
  a build system directory (e.g., \<\$HOME/.gradle/workers>) rather than the
  project root directory, so you may need to provide an absolute path.
...
\item \<-AwarnUnneededSuppressions>
  Issue an \<unneeded.suppression> warning for each \<@SuppressWarnings>
  ...
  not by \<@SuppressWarnings("\emph{checkername}")> and not by a partial
  message key such as \<@SuppressWarnings("suppression")>.
...
\item \<-ArequirePrefixInWarningSuppressions>
  ...
  name.  Otherwise, the warning suppression annotation does not
...
\item
  ...
  \item
    Dynamically generated code, such as code generated by Spring or MyBatis.
...
pointer exception --- \<x> is non-null if control flows to the second
statement.
...
annotations for the Nullness Checker (Chapter~\ref{nullness-checker}), then
...
The specification should state all facts that are relevant to callers.
...
callers, a more automated approach is to save the checker output before
...
Their formal parameter types are annotated as \<@NonNull>, because otherwise the
...
\code{com/google/common/collect/Multiset.java} (the annotation was
subsequently changed to the more correct \code{@ParametricNullness}):
\end{sloppypar}
...
the same Multiset \code{ms}.  The only way the client can be sure not to throw an exception is to pass
...
Write an explanation of why your code is correct and why it
...
\iffalse
\item
  If your proof includes ``method \<m> modifies nothing but field \<f> of
  its receiver'', then annotate \<m>'s declaration with
  \refqualclasswithparams{dataflow/qual}{SideEffectsOnly}{"this.f"}.  The
  annotation's required argument lists every expression that \<m> might
  modify.
\fi
\item
...
even indices are always non-\<null>, but elements stored at odd indices
...
\item To annotate the library:
  ...
  \item
    ...
    (Section~\ref{suppresswarnings-annotation}) or not running the
...
% LocalWords:  bashrc IDE xml buildfile enum API elts INF sql WPI wget
...
% LocalWords:  NullnessChecker commandlineargfile AnnotatedFor Xmx4g
...
% LocalWords:  AinferOutputDirectory
