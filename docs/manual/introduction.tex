...
\textbf{Requirement:}
% Keep in sync with ../../build.gradle and with SourceChecker.init (in checker-framework/framework/src/main/java/org/checkerframework/framework/source/SourceChecker.java).
The Checker Framework is tested with \textbf{JDK 8, JDK 11, 17, and 18}, so
you need to have one of those installed.
(It seems to run on every JDK version $\ge$ 8, but isn't tested on all of
them.  To run the Checker Framework under other JDK versions without a warning, see
Section~\ref{permitUnsupportedJdkVersion}.)
The Checker Framework processes code written for any of those versions.
...
\item
  ...
  \\
  %END LATEX
  \url{https://checkerframework.org/checker-framework-3.22.2.zip}
