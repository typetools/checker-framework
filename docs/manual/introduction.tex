...
It can sometimes be necessary to pass options to only a particular checker.
To achieve this, an option can be prefixed either with the canonical or simple name
...
\hspace*{1cm}\<-ANullnessChecker\_lint=redundantNullComparison>\\
\hspace*{1cm}\<-Aorg.checkerframework.checker.guieffect.GuiEffectChecker\_lint=debugSpew>\\
to pass different lint options to the Nullness and GUI Effect Checkers.
...
When checking a method, uses only the specification, not the
