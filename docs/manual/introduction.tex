...
\item \<-Ainfer>
  ...
  Using \<-Ainfer> produces \<.jaif> files.
...
\item \<-AmergeStubsWithSource>
  If both a stub file and a source file for a class are available, use the
  greatest lower bound of their annotations. The default behavior (without this
  flag) is to ignore types from the stub file if source is available. GLB is
  used to support using this flag in combination with \<-Ainfer=stubs>.
