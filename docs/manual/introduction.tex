...
\item \<-Ainfer>
  ...
  Section~\ref{whole-program-inference}. This argument optionally
  takes an argument to specify the format of its output. By default,
  it outputs to .jaif files. "-Ainfer=stubs" will cause it to write
  its outputs to .astub files, instead.
...
\item \<-AmergeStubsWithSource>
  If both a stub file and a source file for a class are available,
  use the stronger type from the two. This flag overrides the default
  behavior, which is to ignore the stub file.
