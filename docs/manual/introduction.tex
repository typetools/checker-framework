...
\textbf{Requirement:}
You must have \textbf{JDK 7 or later} installed.  You can get the JDK from
...
\item
  ...
  \\
  %END LATEX
  \url{https://checkerframework.org/checker-framework-2.1.10.zip}
...
\item
  ...
  alternative.  It is tempting to think that your code is wrong and the
...
\item
  ...
  If that is not possible, suppress the warning; be sure to include a code
...
above categories it falls into.  Here is an effective way to do so:
\begin{enumerate}
\item
  Write an informal argument explaining why your code is correct and it
  never suffers the error at run time.  In other words, this is an argument
  for why the type-checker's warning is incorrect.

  Don't skip any steps in your argument.  Don't let your reasoning rely on
  facts that you do not write down explicitly.  For example, remember that
  calling a method might change the values of object fields; your argument
  might need to state that certain methods have no side effects.

  You might find that you cannot make this argument, because there is a bug
  in your code.  In that case, fix the bug.

\item
  Try to translate the argument into annotations.

  For example, if your argument includes ``variable \<x> is never \<null>
  at run time'', then ensure that the variable's type is annotated with
  \refqualclass{checker/nullness/qual}{NonNull}.  Or if your argument
  includes, ``method \<foo> always returns a legal regular expression'',
  then ensure that \<foo>'s return type is annotated with
  \refqualclass{checker/regex/qual}{Regex}.  If method \<foo> is in a
  library that your code uses, see \chapterpageref{annotating-libraries},
  Annotating Libraries.
  These are examples of correcting weaknesses in the annotations you wrote.

  If there are complex facts in your argument that cannot be expressed as
  ...
  %
  Second, you can rewrite the code to make the argument simpler;
  in the above example, it might be better to use a list of pairs
  rather than a heterogeneous list.

\item
  At this point, all the steps in your argument have been formalized as
  annotations.  Re-run the checker and repeat the process for any new or
  remaining warnings.

  If every step of your argument can be expressed in annotations, but the
...
Although you will mostly rely on manual code examination, you may also
find it useful to write test cases for your code or do other kinds of
analysis.
...
% LocalWords:  int XDTA spacesincomments newdir Awarns signedness urgin
