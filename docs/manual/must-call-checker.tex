...
The primary client
of the Must Call Checker is the Resource Leak Checker (\chapterpageref{resource-leak-checker}),
...
Running both may cause warning suppressions for the Must Call Checker not to
work.
...
call \<foo>, \<bar>, both, or neither; it cannot represent an object that needs
...
Resource Leak Checker (\chapterpageref{resource-leak-checker}). In particular, any location that
the Resource Leak Checker would consider ``non-owning'' (e.g., unannotated parameters and \<@NotOwning>
returns) is treated as \<@MustCall(\{\})> by the Must Call Checker. This can lead to surprising
...
\item remove the explicit bound from \<Partitioner>, changing its declaration to
  \code{interface Partitioner<ELEMENT, CLASS>}.
\end{itemize}
