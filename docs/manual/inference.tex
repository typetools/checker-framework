...
This chapter is about tools that you can run to infer annotations for your
entire program's method signatures and fields, before running a type-checker.
...
Type inference can either modify your program to insert annotations, or it
can store the annotations in a separate file.  The separate file is read by
the type-checker, but you lose the documentation benefits of the
annotations for programmers who read the source code.

Type inference is a static analysis --- it does not run your program, but
merely analyzes your program's source code.
...
library.  Type inference outputs the most specific type qualifiers that are
consistent with all the source code it is given.  If the given code can never
passes \<null> to method \<m>, type inference will infer that \<m>'s formal
parameter type is \<@NonNull>, even if \<m> was designed to be able to take
\<null> as an argument.  Similarly, if all the methods that override
\<m> return a non-null value, type inference will infer that \<m>'s return
type has \<@NonNull> type, even if some other overriding method can return
\<null>.

The goal of type inference is to find a set of annotations that make your
code type-check:  the annotations are consistent with the source code and
with one another.  Type inference can fail at this task in no such set
exists, because your program contains a bug or contains tricky code that is
beyond the capabilities of the type checker.

For both of these reasons, you may wish to double-check inference results before
using them, or you may need to manually revise some of the inferred annotations.

Type inference is costly:  it usually takes several times longer than
type-checking does.  However, it only needs to be run once, after which
programmers can use and possibly modify the results.
...
work with CFI\@.  The CFI repository
(\url{{https://github.com/opprop/checker-framework-inference}} contains
rewritten versions of some of the type systems that are distributed with
...
\sectionAndLabel{Shared requirements for whole-program inference scripts that do not insert annotations into source code}{wpi-shared-requirements}
...
\sectionAndLabel{Whole-program inference ignores some code}{whole-program-inference-ignores-some-code}
...
\sectionAndLabel{Manually checking whole-program inference results}{whole-program-inference-manual-checking}
...
\subsectionAndLabel{Poor whole-program inference results due to non-representative uses}{whole-program-inference-non-representative-uses}
