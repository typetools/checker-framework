...
be modified for some reason, if the type-checking is ``one-off'' (type-checking will
...
\begin{itemize}
    \item
    To run whole-program inference on a single project, with a few changes to
    its buildfile but no changes to its source code, use the \<wpi2.sh> script
    (Section~\ref{wpi-two}).

    \item
    To run whole-program inference on a single project without modifying either its
    buildfile or its source code,
...
(Sections~\ref{wpi-two}--\ref{wpi-insert}), then concludes with discussion
...
(\<-Ainfer> with no argument does not produce the \<.ajava> files that
\<wpi2.sh> requires.)
...
Do not use \<-AinferOutputOriginal> with \<wpi2.sh>.  It writes, into the
inference output directory, a copy of each source file with no inferred
annotations.  \<wpi2.sh> moves that directory to
\<whole-program-inference-output>, which is the \<-Aajava> argument, so the
compiler would read those un-annotated copies as annotation input.

The two directories must be absolute paths, because the build system might
run the compiler in a different directory than the one in which you run
\<wpi2.sh>.  They must also be the same for every subproject of a
multi-project build, because \<wpi2.sh> manages a single pair of
directories:  \<whole-program-inference-new> and
\<whole-program-inference-output>, both relative to the directory in which
you run \<wpi2.sh>.  That is why the example uses \<project.rootDir> rather
than \<project.projectDir>; the two are the same in a single-project build,
but only \<project.rootDir> is the same in every subproject of a
multi-project build.

Do not put either directory under your build system's output directory, such
as \<build/> or \<target/>.  Your build system's \<clean> task would delete
the inference output, possibly in the middle of a build:  a build system is
free to run \<clean> after it has already compiled some of the project's
source files.


\subsectionAndLabel{Running \<wpi2.sh>}{wpi-two-command}

Second, run \<wpi2.sh>.  Its arguments are the command that runs the Checker
Framework, and that command's arguments.
...
repeatedly, and Gradle does nothing if the old (but still up-to-date)
compilation output is present.  If the command does not recompile every
source file, then \<wpi2.sh> halts with an error rather than discarding the
annotations that it inferred for the files that were not recompiled.

The output of \<wpi2.sh> is a top-level directory
\<whole-program-inference-output>.  For each of your project's source files
for which the Checker Framework inferred an annotation, it contains a file
\<\emph{package}/\emph{ClassName}-\emph{checkername}.ajava>, where
\emph{checkername} is the fully-qualified name of the checker that ran.  A
source file for which nothing was inferred has no \<.ajava> file.
\<wpi2.sh> uses two other top-level directories:
\<whole-program-inference-new>, which the compiler writes each iteration's
results to and which \<wpi2.sh> removes when inference converges, and \<whole-program-inference-diffs>, which contains one diff
file per iteration (\<iteration-1.diff>, \<iteration-2.diff>, etc.) for
diagnostic purposes.  You may wish to add \<whole-program-inference-*> to
your \<.gitignore> file.

If \<wpi2.sh> fails --- for example, because the Checker Framework crashed or
your buildfile is misconfigured --- then the annotations that it inferred so
far remain in \<whole-program-inference-output>.  A subsequent run of
\<wpi2.sh> continues from them rather than starting over; it retains the
earlier run's diff files and numbers the new ones after them.  To start
inference from scratch, remove \<whole-program-inference-output> before
running \<wpi2.sh>.

\<wpi2.sh> runs the command at most 10 times.  If inference has not converged
by then, \<wpi2.sh> reports that fact and exits; set the
\<WPI2\_MAX\_ITERATIONS> environment variable to a positive integer to change
the bound.
...
temporary directory whose name appears in the \<dljc-out/wpi-stdout.log> file;
...
\item At least one of the \verb|JAVA_HOME|, \verb|JAVA8_HOME|,
  \verb|JAVA11_HOME|, \verb|JAVA17_HOME|, \verb|JAVA21_HOME|,
  \verb|JAVA24_HOME|, \verb|JAVA25_HOME|, or \verb|JAVA26_HOME|
...
\item If set, the \verb|JAVA24_HOME| environment variable must point to a Java 24 JDK.
\item If set, the \verb|JAVA25_HOME| environment variable must point to a Java 25 JDK.
\item If set, the \verb|JAVA26_HOME| environment variable must point to a Java 26 JDK.
\item The \<CHECKERFRAMEWORK> environment variable must point to a built copy of the Checker Framework.
...
\<docs/examples/wpi-many/securerandom.list> is the standard output
...
that the project type-checks without errors afterward, or even type-checks at
all --- just that the Checker Framework attempted to type-check the project and
...
\item[-s]
  If this flag is present, then projects that are not buildable --- for which
  no supported build file is present or for which running the standard build
  commands fails --- are skipped on future runs but are \emph{not} deleted immediately
...
Each script adds the inferred annotations to the next run, so that the checker takes them into
...
not infer enough annotations, or because your program cannot type-check
