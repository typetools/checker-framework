...
There are three ways to run whole-program inference: via the
\<checker-framework/checker/bin/wpi.sh> script, via the \<checker-framework/checker/bin/wpi-many.sh>
scripts, or via the \<checker-framework/checker/bin/infer-and-annotate.sh> script.
In most cases, you should choose \<wpi.sh> if you are running whole-program inference
on a single project, and \<wpi-many.sh> if you are running an experiment on many
projects that you will download from GitHub or another repository. You should
only use \<infer-and-annotate.sh> if you know that you need to use \<.jaif> files
...
\begin{enumerate}
\item Use \<checker-framework/checker/bin/query-github.sh> to search GitHub for candidate repositories.
File \<checker-framework/checker/bin/securerandom.query> is an example query, and file
\<checker-framework/checker/bin/securerandom.list>
was created by running \<checker-framework/checker/bin/query-github.sh securerandom.query 100>. If you do
not want to use GitHub, construct a file yourself that matches the format of
the file \<checker-framework/checker/bin/securerandom.list>.

\item Use \<checker-framework/checker/bin/wpi-many.sh> to run whole-program inference on every
...
\<checker-framework/checker/bin/custom-checker-example.sh> is a no-arguments
script that serves as an example of how to use \<checker-framework/checker/bin/wpi-many.sh>.
\end{itemize}
...
\item Use \<checker-framework/checker/bin/wpi-summary.sh> to summarize the logs in the output results directory.
Use its output to guide your analysis of the results of running \<checker-framework/checker/bin/wpi-many.sh>:
...
Then, re-run your experiments, supplying the \<-u "\$yourGithubId"> option to \<checker-framework/checker/bin/wpi-many.sh>.
\<checker-framework/checker/bin/wpi-many.sh> will perform inference on your forked version rather than
the original.
...
\subsectionAndLabel{Running whole-program inference on a single projects}{wpi-one}

Run \<checker-framework/checker/bin/wpi.sh>.
Its use is similar to \<checker-framework/checker/bin/wpi-many.sh>, described above. However,
its input is a single directory on your local machine rather than
a list of repository URLs and hashes. \<checker-framework/checker/bin/wpi-many.sh> uses this script internally
after checking out projects.

\subsectionAndLabel{Running whole-program inference using JAIF files}{wpi-jaif}

\textbf{Note: this section describes legacy instructions to use the infer-and-annotate.sh script. Unless
you know you need to use .jaif-based WPI, you should follow the instructions in the previous two sections
