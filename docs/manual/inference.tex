...
\item[DLJC-ARGS]
  ...
  what type-checker to run.  Usage of dljc is:
%% To obtain this usage string, which we should update periodically:
%  git clone -q --depth 1 https://github.com/kelloggm/do-like-javac
%  cd do-like-javac
%  python2 dljc --help
%% Even better, the do-like-javac repository could document the
%% command-line arguments, and this could just link to that documentation.
\begin{Verbatim}
dljc [-o <directory>] [--log_to_stderr] [-t <tool>]
     [--timeout <seconds>] [--guess] [--quiet] [--cache] [-c <checker>]
     [--stubs <stubs>] [-l <lib_dir>] [--jdkVersion <jdkVersion>]
     [--quals <quals>] [-s <solver>] [-afud <afud>] [-m <mode>]
...
This section explains how the \<wpi.sh> script works.  If you
