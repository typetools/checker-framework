...
The null literal is implicitly annotated with \code{@UnknownKeyFor}, because
\code{null} might be a key for any map.

Lower bounds are defaulted to \code{@UnknownKeyFor}.  This reduces false
positives when \code{null} is assigned to a variable whose type is a type
variable, but it does require you to take action if a type variable will be
instantiated with \code{@KeyFor} for a particular map.  In that case, you
must explicitly annotate the lower bound of that type variable with
\code{@KeyForBottom}.  For example,

\begin{Verbatim}
public abstract class MyKeySet<@KeyForBottom K> implements Set<K> { }
\end{Verbatim}
...
%%  LocalWords:  mapSet keySet km threeLetterWordSubset JT
