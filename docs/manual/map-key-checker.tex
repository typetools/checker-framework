...
Lower bounds are defaulted to \code{@KeyForBottom}.
This reduces the number of false
positives, but it does require you to take action if you expect a
type variable to be instantiated with \code{@KeyFor} for a particular map.
In that case,
you must explicitly annotate the lower bound of that type variable with
\code{@KeyForBottom}.  For example, \code{List<@KeyForBottom T>}.
