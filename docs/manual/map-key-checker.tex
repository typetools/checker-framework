...
\<@Nullable V get(Object key)>.  (Changing the JDK declaration would not
...
changes a method's return type.  This is not the same as assuming that the
return value is \<@NonNull>, because the map's value type might be
\<@Nullable>, as in \code{Map<String, @Nullable Integer>}.
