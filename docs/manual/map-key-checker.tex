...
The \code{null} literal is implicitly annotated with \code{@UnknownKeyFor}
If \code{null} were \code{@UnknownKeyFor}, that would mean that
...
If lower bounds are defaulted to \code{@KeyForBottom},
then whenever \<null> is assigned to a variable whose type is a type variable,
programmers
must write a \<UnknownKeyFor> annotation on either the type variable
declaration or on variable declarations, as shown in \<MyClass1> and
\<MyClass2>.

If lower bounds are defaulted to \code{@UnknownKeyFor}, then
whenever a client might use a \<@KeyFor> type argument,
