...
\code{null} is a key for every map.  In order to reduce the number of false
postives, lower bounds are defaulted to \code{@KeyForBottom}.  If you expect a
type variable to be instantiated with \code{@KeyFor} for a particular map, then
you must explictly annotated the lower bound of that type variable with
\code{@KeyForBottom}.  For example, \code{<@KeyForBottom T>}.
