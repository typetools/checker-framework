...
of a method which ensures that a certain expression is a key for a map:

\begin{description}
\item[\refqualclasswithparams{checker/index/qual}{EnsuresKeyFor}{String[] value, String[] targetValue}]
  ...
  maps. More preciesly, the expression has the \code{@KeyFor} qualifier
  with the \code{value} arguments taken from the \code{targetValue} element
  of this annotation.
\item[\refqualclasswithparams{checker/index/qual}{EnsuresKeyForIf}{String[] expression, boolean result, String[] targetValue}]
...
%%  LocalWords:  MyClass2 MyClass3 s1 MySet2 s2
