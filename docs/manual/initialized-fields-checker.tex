...
The Initialized Fields Checker warns you if you have not initialized a
field.
If you supply the \<-AcheckInitializedFields=...> command-line argument,
then warnings are issued only if
the default value assigned by Java (0, true, or null) is not
consistent with the field's annotation, for the given type systems.

The Initialized Fields Checker works by tracking the fields that have been
initialized so far, and issuing a warning at the end of any constructor if
not all the fields of this class have been initialized.
You usually run the Initialized Fields Checker together with another
checker that ensures that the values assigned to fields are legal.
...
The Initialized Fields type system uses the following annotations:
\begin{description}
