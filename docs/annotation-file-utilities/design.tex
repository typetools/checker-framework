...
document uses AFU in the singular, in reference to the source-insertion
...
so there will be no need to refer back to the source itself.
...
The current AFU transforms the scene into a set of
...
strategy not only obviates an additional pass but also avoids much useless
...
\texttt{com.sun.source.tree.Tree} that provides needed bits from
...
\item  The compilation unit node will store any text that precedes the
...
\begin{jcode}
public abstract class AAST implements com.sun.source.tree.Tree {
  ...
  public void insertAnnotation(TreePath path, Annotation anno);
}
