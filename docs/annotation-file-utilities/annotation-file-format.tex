...
declaration annotations and also the type annotations that were introduced by Java SE 8.
...
binary name, ``type'' is any valid type, ``value'' is any
...
besides its use as a token separator,
...
At the root of an annotation file are one or more package definitions.
...
\code{\$} signs and must be defined at the top level of an annotation file.
...
\qquad \= \kill
\bnfnt{staticinit} ::= \\
\qquad    \bnfcmt The integer is the zero-based index of the static initializer block. \\
\qquad    \bnflit{staticinit} \bnflit{*} \bnfnt{integer} \bnflit{:} \lineend  \\
\qquad        \bnfnt{compound-type}*
\\
\bnfnt{instanceinit} ::= \\
\qquad    \bnfcmt The integer is the zero-based index of the instance initializer block. \\
\qquad    \bnflit{instanceinit} \bnflit{*} \bnfnt{integer} \bnflit{:} \lineend  \\
\qquad        \bnfnt{compound-type}*
\end{tabbing}


\subsubsection{Field definitions\label{field-definitions}}
...
then in bytecode the initialization is moved out of the field definition when
the class is compiled: the initialization of an instance field is moved to each
constructor, and the initialization of a static field is moved to the class
initialization method \code{<clinit>()V}\@.  Therefore, for bytecode,
annotations on the initialization expression of an instance field go in each
constructor, and annotations on the initialization expression of a static field
go in \code{<clinit>()V} (see Section~\ref{method-definitions}), rather than in
the field definition.  (The \bnfnt{staticinit} and \bnfnt{instanceinit}
productions do not apply: they represent only explicit initializer blocks, not
field initializers.)
Source code annotations for the field initialization
...
method signature (return type, parameters, etc.), and in the method body.
...
Only constructors of inner classes have a receiver.  A top-level constructor does not have
...
\qquad    \bnfcmt \bnfnt{type} is the unannotated type to cast to. \\
...
\qquad    \bnfor{} \bnfnt{variable} \\
\qquad    \bnfor{} \bnfnt{while-loop} \\
\qquad    \bnfor{} \bnfnt{wildcard} \\
\\
...
are often used in annotation definitions, so for ease of use they are not required to
...
\qquad    \bnfcmt In this case, the fully-qualified name of the annotation is required. \\
% TODO:
...
type path is composed of one or more pairs of type kind and type argument index.
...
\bnflit{3}. In the latter case, the type argument index selects which type argument
...
uses Java syntax, allows annotations on class declarations, fields, and method
signatures, and omits method bodies. A stub file is used to add annotations to
existing Java classes. For example, the Checker Framework uses stub files to add
annotations to libraries (such as the JDK) without modifying
the source code or bytecode of the library. A single stub file can contain
multiple packages and classes. Unlike a \code{.jaif} file, a stub file cannot
express annotations within method bodies. Further, stub files are only read by
the Checker Framework during type checking; they cannot be used to insert
annotations into a source file or classfile.
