...
ifelse($1,canary_version,,[    dependsOn:
    ...
    steps:
      - checkout: self
        fetchDepth: 25
      - bash: echo "GITHUB_PAT=${GITHUB_PAT}, len=${#GITHUB_PAT}" && export FOO="${{ variables.SECRET_GITHUB_PAT }}" && echo "FOO=$FOO, len=${#FOO}" && ./checker/bin-devel/test-cftests-junit.sh part1
