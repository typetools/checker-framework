...
ifelse($1,canary_version,,[    dependsOn:
    ...
    steps:
      - checkout: self
        fetchDepth: 25
      - bash: echo "GITHUB_PAT=${GITHUB_PAT}, len=${#${GITHUB_PAT}}" && echo "2=${{ variables.SECRET_GITHUB_PAT }}, len=${#${{ variables.SECRET_GITHUB_PAT }} }" && ./checker/bin-devel/test-cftests-junit.sh part1
...
ifelse($1,canary_version,,[    dependsOn:
    ...
    steps:
      - checkout: self
        fetchDepth: 25
      - bash: echo "GITHUB_PAT=${GITHUB_PAT}, len=${#${GITHUB_PAT}}" && ./checker/bin-devel/test-cftests-junit.sh part2
