...
public class FIsAReducer implements AFReducer {
     ...
     *
     * <p>The visit method will determine if the given constraint should either: a) be discarded -
     * in this case, the visitor just returns b) reduced to a simpler constraint or set of
     * constraints - in this case, the new constraint or set of constraints is added to
     * newConstraints
     *
     * <p>From the JLS, in general there are 2 rules that govern F = A constraints: If F = Tj, then
     * the constraint Tj = A is implied. If F = U[], where the type U involves Tj, then if A is an
     * array type V[], or a type variable with an upper bound that is an array type V[], where V is
     * a reference type, this algorithm is applied recursively to the constraint {@code V >> U}.
