...
public class ElementAnnotationUtil {
     ...
     *
     * <p>a) If an Annotation is in front of a extends or super bounded wildcard, it applies to the
     ...
     * }</pre>
     *
     * c) If an Annotation is on an unbounded wildcard there are two subcases. c.1 The user wrote
     * the annotation explicitly - these annotations apply to both bounds e.g. the user wrote
     *
     * <pre>{@code
     * <@C ?> - the annotation is placed on the extends/super bounds
     * }</pre>
     *
     * c.2 Previous calls to getAnnotatedType have annotated this wildcard with BOTH bounds e.g. the
     * user wrote {@code <?>} but the checker framework added {@code <@C ? extends @D Object>} to
     * the corresponding element {@code <?>} - @C is placed on the lower bound and @D is placed on
     * the upper bound This case is treated just like annotations in cases a/b.
     */
