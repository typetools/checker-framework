...
//import org.checkerframework.framework.util.AgreementWriter;
import org.checkerframework.framework.util.AnnotatedTypes;
import org.checkerframework.framework.util.QualifierPolymorphism;
//import org.checkerframework.framework.util.SubtypeWriter;
 ...
 * purposes.
 */
public class TypeHierarchy_orig {
...
//    private SubtypingVisitor subtypingVisitor;
    /**
     ...
     */
    public TypeHierarchy_orig(BaseTypeChecker checker, QualifierHierarchy qualifierHierarchy) {
        this.qualifierHierarchy = qualifierHierarchy;
        this.visited = new HashSet<AnnotatedTypeMirror>();
        this.checker = checker;
//        subtypingVisitor = new SubtypingVisitor(checker, qualifierHierarchy,
//                new EqualityComparer(),
//                checker.hasOption("ignoreRawTypeArguments"), checker.hasOption("invariantArrays"));
    }



    public boolean isSubtype(AnnotatedTypeMirror rhs, AnnotatedTypeMirror lhs) {
        boolean oldResult = origIsSubtype(rhs, lhs);
 //       try {
//            boolean newResult = subtypingVisitor.isSubtype(rhs, lhs);
//            if( newResult != oldResult ) {
                //AgreementWriter.writeDisagreement(rhs, lhs, oldResult, false, this.checker);
                oldResult = origIsSubtype(rhs, lhs);
//                newResult = subtypingVisitor.isSubtype(rhs, lhs);
//            }
//        } catch(Throable thr) {
//           // AgreementWriter.writeDisagreement(rhs, lhs, oldResult, true, this.checker);
//           // oldResult = origIsSubtype(rhs, lhs);
//           // boolean newResult = subtypingVisitor.isSubtype(rhs, lhs);
//           throw exc;
//        }
     ...
     */
    public boolean origIsSubtype(AnnotatedTypeMirror rhs, AnnotatedTypeMirror lhs) {
        try {
...
//        SubtypeWriter.writeSubtype(rhs, lhs);
        /*
        ...
        */
        ...
        if (lhs.getKind() == TypeKind.ARRAY && rhsBase.getKind() == TypeKind.ARRAY) {
            ...
            // System.out.printf("lhsBase (%s underlying=%s), rhsBase (%s underlying=%s), equals=%s%n", lhsBase.hashCode(), lhsBase.getUnderlyingType(), rhsBase.hashCode(), rhsBase.getUnderlyingType(), lhsBase.equals(rhsBase));
             //todo: I think this is here solely for the case in which the typevars are not referring to the same type
