...
public class Range {
   ...
   *
   * @param collection the min and max value found in it will be used as from and to values in the
   *     range
   */
  ...
  public Range charRange() {
    ...
    if (ignoreOverflow) {
      return new Range(Math.max(from, Character.MIN_VALUE), Math.min(to, Character.MAX_VALUE));
    }
