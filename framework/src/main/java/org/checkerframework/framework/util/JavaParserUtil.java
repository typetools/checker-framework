...
public final class JavaParserUtil {
  ...
  private static @Nullable TypeElement resolveTypeNameMemoized(
    ...
    TypeElement result = resolveName(elements, type, cache, nodeCache).typeElement();
   ...
   * #resolveTypeVariableName(Elements, ClassOrInterfaceType, Map)}.
   *
   * <p>Returns null if the TypeMirror cannot be determined. In addition to the types for which
   * {@link #typeToTypeMirror(Elements, Types, Type, Map)} returns null, that is a type variable
   * whose upper bound is an intersection type, which {@code Types} cannot create.
   *
   * @param elements used for looking up names
   * @param types used for creating types
   * @param type a JavaParser type
  ...
  public static @Nullable TypeMirror typeToTypeMirrorErasingTypeVariables(
    ...
    if (type instanceof ClassOrInterfaceType classType) {
      ...
      if (classType.getNameWithScope().indexOf('.') != -1) {
        // A type variable has no member types, so a qualified name does not name a type variable;
        // `resolveName` resolved only the name's first component.
        return null;
      }
   ...
   * @param cache maps a name to the type it names, or to null if it names no type; this method both
   *     reads and writes it
   * @return the TypeMirror for the upper bound of {@code typeParameter}, or null
   */
