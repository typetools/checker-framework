 ...
 *
 * <p>Conservative defaults are only applied if they are enabled via the {@code
 * -AuseConservativeDefaultsForUncheckedCode} command-line option. They can be enabled for source
 * and bytecode separately. If the conservative code defaults are enabled for unchecked source code,
 * they will only be applied to source code not annotated with {@link AnnotatedFor} for the checker
 * being executed.
 *
 ...
 * @see ConservativeDefaultQualifierInHierarchy
...
public @interface ConservativeDefaultFor {
