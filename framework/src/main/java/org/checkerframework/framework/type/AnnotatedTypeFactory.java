        ...
        if (root != null && checker.hasOption("ajava")) {
            ...
            for (String ajavaLocation : checker.getOption("ajava").split(File.pathSeparator)) {
                ...
                if (ajavaFileInDir.exists()) {
                    ...
                    // correct one
                    // for class foo.Bar? It depends on whether there is a foo.foo.Bar or a
                    // baz.foo.Bar
                    // elsewhere in the project. For that reason, parsing using a specific file is
                    // done at the
                    // **end** of the loop, and if there is more than one match no file is parsed
                    // for this
                    // class and a warning is issued instead. The user can disambiguate by supplying
                    // a root
