...
# 4. Extra processor arguments which will be passed to the checker

# Example of usage:
#./infer-and-annotate.sh org.checkerframework.common.value.ValueChecker $PROJECT_CP project_src_folder/ $EXTRA_PROCESSOR_ARGS
...
function run_cf_javac() {
    ...
    for f in $(find $source_files_dir/ -name "*.java" -print0); do
